...
const approvalPinAttempts = new Map();
const approvalIdempotencyKeys = new Map();
const APPROVAL_PIN_WINDOW_MS = Number(process.env.MISSION_APPROVAL_PIN_WINDOW_MS || 10 * 60 * 1000);
const APPROVAL_PIN_MAX_ATTEMPTS = Number(process.env.MISSION_APPROVAL_PIN_MAX_ATTEMPTS || 5);
const APPROVAL_IDEMPOTENCY_TTL_MS = Number(
  process.env.MISSION_APPROVAL_IDEMPOTENCY_TTL_MS || 5 * 60 * 1000
);

const cleanupExpiredEntries = (store, nowMs) => {
  for (const [key, value] of store.entries()) {
    if ((value?.expiresAt || 0) <= nowMs) {
      store.delete(key);
    }
  }
};

const safeStringCompare = (left, right) => {
...
export const verifyConfiguredApprovalPin = async (providedPin) => {
  const configuredPinHash = process.env.MISSION_APPROVAL_PIN_HASH || '';
  const configuredPinRaw = process.env.MISSION_APPROVAL_PIN || '';
  if (!configuredPinHash && !configuredPinRaw) {
  ...
  if (configuredPinHash) {
    try {
      const valid = await bcrypt.compare(String(providedPin), String(configuredPinHash));
      return { required: true, valid };
    } catch {
      return { required: true, valid: false };
    }
  }
  return { required: true, valid: safeStringCompare(providedPin, configuredPinRaw) };
};

export const checkPinRateLimit = (attemptKey) => {
  const nowMs = Date.now();
  cleanupExpiredEntries(approvalPinAttempts, nowMs);
  const entry = approvalPinAttempts.get(attemptKey);
  if (!entry) return { limited: false, remaining: APPROVAL_PIN_MAX_ATTEMPTS };
  if ((entry.failures || 0) >= APPROVAL_PIN_MAX_ATTEMPTS) {
    return { limited: true, remaining: 0 };
  }
  return {
    limited: false,
    remaining: Math.max(0, APPROVAL_PIN_MAX_ATTEMPTS - (entry.failures || 0)),
  };
};

export const registerPinFailure = (attemptKey) => {
  const nowMs = Date.now();
  const current = approvalPinAttempts.get(attemptKey);
  const failures = (current?.failures || 0) + 1;
  approvalPinAttempts.set(attemptKey, {
    failures,
    expiresAt: nowMs + APPROVAL_PIN_WINDOW_MS,
  });
};

export const clearPinFailures = (attemptKey) => {
  approvalPinAttempts.delete(attemptKey);
};

export const reserveIdempotencyKey = (userId, missionId, idempotencyKey) => {
  if (!idempotencyKey || typeof idempotencyKey !== 'string') return { duplicate: false };
  const nowMs = Date.now();
  cleanupExpiredEntries(approvalIdempotencyKeys, nowMs);
  const cacheKey = `${userId}:${missionId}:${idempotencyKey}`;
  if (approvalIdempotencyKeys.has(cacheKey)) {
    return { duplicate: true };
  }
  approvalIdempotencyKeys.set(cacheKey, { expiresAt: nowMs + APPROVAL_IDEMPOTENCY_TTL_MS });
...
const drawPdfLogo = (doc, logoPath, x = 42, y = 30, width = 126, height = 48) => {
  ...
  doc
    ...
    .font('Helvetica-Bold')
    .fontSize(18)
    .text('PROQUELEC', x, y + 6);
};

const drawPdfSectionTitle = (doc, title, y) => {
  doc.fillColor('#16324f').font('Helvetica-Bold').fontSize(11).text(title.toUpperCase(), 42, y);
  doc
    .moveTo(42, y + 16)
    .lineTo(552, y + 16)
    .strokeColor('#dbeafe')
...
const drawMissionTable = (doc, rows, startY) => {
  ...
  const drawRow = (label, value, height = 36) => {
    ...
    doc
      .rect(left + 112, y, width - 112, height)
      .strokeColor('#d9d9d9')
      .stroke();
    doc
      .fillColor('#4b5563')
...
const drawApprovalAuditTable = (doc, steps, startY) => {
  ...
  if (!steps.length) {
    ...
    doc
      ...
      .font('Helvetica')
      .fontSize(9)
      .text('Aucune etape d approbation detaillee disponible.', left + 12, y + 12);
    return y + 44;
  }
...
const buildMissionDocumentBuffer = async (mission) => {
  ...
  return await new Promise((resolve, reject) => {
    ...
    doc
      ...
      .font('Helvetica-Bold')
      .fontSize(11)
      .text('Sont autorises a se rendre en mission :', 42, y);
    y += 24;
    ...
    y = drawMissionTable(
      ...
      [
        ...
        {
          label: 'Date Mission',
          value: formatDateFr(mission.startDate || missionData.startDate),
          height: 36,
        },
        ...
        { label: 'Objet de la mission', value: mission.title || missionData.purpose, height: 46 },
        {
          ...
          value: missionData.transport || 'Vehicule de service',
          height: 36,
        },
        {
          label: 'Itineraire Aller',
          value: mission.description || missionData.itineraryAller,
          height: 36,
        },
        { label: 'Itineraire Retour', value: missionData.itineraryRetour, height: 36 },
        {
          label: 'Retour Prevu le',
          value: formatDateFr(mission.endDate || missionData.endDate),
          height: 36,
        },
      ],
      y
    ...
    doc
      ...
      .font('Helvetica-Oblique')
      .fontSize(6)
      .text('SCANNEZ POUR VERIFIER', 42, signatureY + 68, {
        width: 90,
      });
    ...
    doc
      ...
      .font('Helvetica-Bold')
      .fontSize(11)
      .text('Validation finale', 372, signatureY + 8, {
        width: 140,
        align: 'center',
      });
    doc
      .moveTo(330, signatureY + 72)
      .lineTo(530, signatureY + 72)
      .strokeColor('#bdbdbd')
      .lineWidth(0.7)
      .stroke();
    doc
      ...
      .font('Helvetica-Oblique')
      .fontSize(8)
      .text('Signature DG', 330, signatureY + 78, {
        width: 200,
        align: 'center',
      });
    ...
    doc
      ...
      .font('Helvetica-Bold')
      .fontSize(15)
      .text('DECOMPTE FRAIS DE MISSION', 42, 104, {
        width: 510,
        align: 'center',
      });
    doc.moveTo(124, 126).lineTo(510, 126).strokeColor('#4f46e5').lineWidth(2).stroke();
    ...
    if (members.length === 0) {
      ...
      doc
        ...
        .font('Helvetica')
        .fontSize(9)
        .text('Aucun beneficiaire renseigne.', 58, page2Y + 10);
      page2Y += 34;
    } else {
      members.forEach((member) => {
        if (page2Y > 640) {
          ...
          doc
            ...
            .font('Helvetica-Bold')
            .fontSize(13)
            .text('DECOMPTE FRAIS DE MISSION - SUITE', 42, 104, {
              width: 510,
              align: 'center',
            });
          doc.moveTo(124, 126).lineTo(510, 126).strokeColor('#4f46e5').lineWidth(2).stroke();
        ...
        doc.text(`${daily.toLocaleString('fr-FR')} FCFA`, 228, page2Y + 10, {
          width: 110,
          align: 'right',
        });
        doc.text(String(days), 352, page2Y + 10, { width: 80, align: 'right' });
        doc.text(`${total.toLocaleString('fr-FR')} FCFA`, 448, page2Y + 10, {
          width: 86,
          align: 'right',
        });
        page2Y += 34;
    ...
    doc
      ...
      .font('Helvetica-Bold')
      .fontSize(8)
      .text('Montant total en FCFA', 346, page2Y + 9);
    doc
      .fillColor('#111827')
      .font('Helvetica-Bold')
      .fontSize(14)
      .text(`${grandTotal.toLocaleString('fr-FR')} FCFA`, 346, page2Y + 22, {
        width: 188,
        align: 'right',
      });
    doc
      ...
      .font('Helvetica-Bold')
      .fontSize(11)
      .text('Validation finale', 372, 642, {
        width: 140,
        align: 'center',
      });
    ...
    doc
      ...
      .font('Helvetica-Bold')
      .fontSize(15)
      .text('ANNEXE DE VERIFICATION', 42, 104, {
        width: 510,
        align: 'center',
      });
    doc.moveTo(124, 126).lineTo(510, 126).strokeColor('#4f46e5').lineWidth(2).stroke();
    ...
    doc
      ...
      .font('Helvetica-Bold')
      .fontSize(11)
      .text('Verification publique du document', 176, page3Y + 28, {
        width: 330,
      });
    doc
      .fillColor('#475569')
    ...
    doc
      ...
      .font('Helvetica-Bold')
      .fontSize(8)
      .text('Numero OM', 176, page3Y + 86);
    doc
      .fillColor('#111827')
      .font('Helvetica')
      .fontSize(10)
      .text(orderNumber, 176, page3Y + 98, {
        width: 150,
      });
    doc
      .fillColor('#64748b')
      .font('Helvetica-Bold')
      .fontSize(8)
      .text('Empreinte integrite', 330, page3Y + 86);
    doc
      .fillColor('#111827')
      .font('Helvetica')
      .fontSize(8)
      .text(workflow?.integrityHash || '—', 330, page3Y + 98, {
        width: 190,
        lineGap: 1,
      });
    ...
    doc
      ...
      .font('Helvetica-Bold')
      .fontSize(12)
      .text('Historique d approbation', 42, page3Y);
    page3Y += 22;
    ...
    doc
      ...
      .font('Helvetica-Bold')
      .fontSize(11)
      .text('Signature Direction Generale', 346, 650, {
        width: 170,
        align: 'center',
      });
    doc.moveTo(330, 712).lineTo(530, 712).strokeColor('#bdbdbd').lineWidth(0.7).stroke();
    doc
      ...
      .font('Helvetica-Oblique')
      .fontSize(8)
      .text('Cachet et signature', 330, 718, {
        width: 200,
        align: 'center',
      });
    drawPdfFooter(doc, orderNumber, publicUrl);
...
export const verifyMissionPublic = async (req, res) => {
  try {
    ...
    if (h && h !== shortHash) {
      logger.warn(
        `[MISSION_SECURITY] Hash mismatch for mission ${mission.id}. Provided: ${h}, Expected: ${shortHash}`
      );
    }
...
export const downloadMissionCertifiedDocumentPublic = async (req, res) => {
  try {
    ...
    tracerAction(
      mission.organizationId,
      null,
      'MISSION_CERTIFIED_DOCUMENT_DOWNLOADED',
      'Mission',
      mission.id,
      {
        identifier,
...
export const sendMissionDocumentEmail = async (req, res) => {
  try {
    ...
    await sendMail({
      ...
      body:
        body ||
        `Veuillez trouver ci-joint votre ordre de mission officiel n° ${mission.orderNumber}. <br/><br/>Ce document est certifié numériquement.`,
      attachments: [
        {
...
export const getMissions = async (req, res) => {
  try {
    ...
    const isSystemAdmin = req.user?.email === SUPER_ADMIN_EMAIL;
...
export const getMissionStats = async (req, res) => {
  try {
    ...
    const whereClause = {
      ...
      deletedAt: null,
      ...(projectId && { projectId }),
      ...(req.user.email !== SUPER_ADMIN_EMAIL ? { createdBy: req.user.id } : {}),
    };
...
export const createMission = async (req, res) => {
  try {
    ...
    if (!organizationId) {
      return res
        .status(400)
        .json({ error: 'Organization ID manquante dans le contexte utilisateur' });
    }
...
export const updateMission = async (req, res) => {
  try {
    ...
    if (
      ...
      return res
        .status(403)
        .json({ error: 'Seule la Direction Générale peut certifier une mission.' });
    }
    ...
    if (status === 'soumise' || status === 'en_attente_validation') {
      ...
      if (isSubmissionTransition) {
        try {
          await tracerAction(organizationId, userId, 'MISSION_SUBMITTED', 'Mission', id, {
            title,
            status,
          });
        } catch (e) {
        ...
        try {
          ...
          for (const email of emails) {
            missionNotificationService
              .notifySubmission(mission, email)
  ...
  try {
    ...
    const mission = await prisma.mission.findFirst({
      where: {
        ...
        ...(!isAdmin && req.user.email !== SUPER_ADMIN_EMAIL ? { createdBy: userId } : {}),
      },
    });
...
export const purgeMissions = async (req, res) => {
  try {
    ...
    if (userRole !== 'ADMIN_PROQUELEC' && req.user.email !== SUPER_ADMIN_EMAIL) {
      return res
        .status(403)
        .json({ error: 'Action non autorisée. Réservée au Super Administrateur.' });
    }
    ...
    logger.warn(
      `🔥 [MISSION_PURGE] ${count} missions have been purged by ${userId} (Org: ${organizationId})`
    );
  ...
  try {
    ...
    const original = await prisma.mission.findFirst({
      where: {
        ...
        organizationId,
        deletedAt: null,
        ...(req.user.email !== SUPER_ADMIN_EMAIL ? { createdBy: userId } : {}),
      },
    });
    ...
    try {
      await tracerAction(organizationId, userId, 'MISSION_DUPLICATE', 'Mission', id, {
        newId: copy.id,
      });
    } catch (e) {
...
async function _getWorkflowConfig(organizationId) {
  ...
  return [{ role: 'DIRECTEUR', sequence: 1, label: 'Directeur Général' }];
}
...
async function findMissionStakeholderEmails(organizationId, roleNames) {
  const normalizedRoles = (Array.isArray(roleNames) ? roleNames : [roleNames]).flatMap(
    (roleName) => {
  ...
  return [
    ...new Set(stakeholders.map((user) => user.notificationEmail || user.email).filter(Boolean)),
  ];
}
...
export const approveMissionStep = async (req, res) => {
  try {
    const { missionId } = req.params;
    const { role, comment, pin } = req.body;
    const { organizationId, id: userId, role: rawUserRole } = req.user;
    const userRole = rawUserRole?.toUpperCase();
    ...
    const idempotencyReservation = reserveIdempotencyKey(userId, missionId, idempotencyKey);
    if (idempotencyReservation.duplicate) {
      return res
        .status(409)
        .json({ error: 'Duplicate approval request detected (idempotency key).' });
    }
    ...
    // ==============================================
    const isSelfCreatedDirector =
      (userRole === 'DIRECTEUR' || userRole === 'DG_PROQUELEC') && mission.createdBy === userId;
    const isAdmin = userRole === 'ADMIN_PROQUELEC' || userRole === 'ADMIN';

    const privilegedApproval =
      ['DIRECTEUR', 'ADMIN', 'ADMIN_PROQUELEC'].includes(normalizedRole) ||
      ['DIRECTEUR', 'DG_PROQUELEC', 'ADMIN', 'ADMIN_PROQUELEC'].includes(userRole);
    if (privilegedApproval) {
      const attemptKey = `${userId}:${req.ip || 'unknown-ip'}`;
      const limitState = checkPinRateLimit(attemptKey);
      if (limitState.limited) {
        return res
          .status(429)
          .json({ error: 'Trop de tentatives PIN invalides. Réessayez plus tard.' });
      }

      const pinCheck = await verifyConfiguredApprovalPin(pin);
      if (pinCheck.required && !pinCheck.valid) {
        registerPinFailure(attemptKey);
        return res.status(403).json({ error: 'PIN de validation incorrect.' });
      }
      clearPinFailures(attemptKey);
    }

    if (
      (isAdmin && (normalizedRole === 'ADMIN' || normalizedRole === 'ADMIN_PROQUELEC')) ||
      isSelfCreatedDirector
    ) {
        ...
        workflow.approvalSteps.map((step) =>
          prisma.missionApprovalStep.update({
            ...
            data: {
              ...
              comment:
                comment ||
                (isAdmin ? `Admin approval` : `Auto-validation par le Directeur (Créateur)`),
      ...
      const updatedWorkflow = await prisma.missionApprovalWorkflow.update({
        ...
        data: {
          ...
          integrityHash: crypto
            .createHash('sha256')
            .update(JSON.stringify({ missionId, orderNumber, timestamp: new Date() }))
            .digest('hex'),
        },
        include: {
          approvalSteps: { orderBy: { sequence: 'asc' } },
      ...
      missionNotificationService
        .notifyFullApproval(mission, orderNumber, mission.createdBy)
    ...
    const canApproveThisRole =
      ...
      (normalizedRole === 'DIRECTEUR' && userRole === 'DG_PROQUELEC');
    ...
    if (!isAdmin && mission.createdBy === userId && step.sequence > 1) {
      return res
        ...
        .json({
          error:
            "Sécurité : Vous ne pouvez pas approuver les étapes supérieures d'une mission que vous avez créée.",
        });
    }
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      if (nextStep) {
        ...
        missionNotificationService
          .notifyNextStep(mission, nextStep.role, organizationId)
        ...
        if (initiator?.email) {
          missionNotificationService
            .notifyFullApproval(updatedMission, orderNumber, initiatorId)
    ...
    if (result.nextStep) {
      missionNotificationService
        .notifyNextStep(mission, result.nextStep.role, organizationId)
        .catch((err) => logger.error('Next step notification failed:', err));
    } else {
      missionNotificationService
        .notifyFullApproval(mission, result.orderNumber, mission.createdBy)
...
export const rejectMissionStep = async (req, res) => {
  try {
    ...
    if (idempotencyReservation.duplicate) {
      return res
        .status(409)
        .json({ error: 'Duplicate rejection request detected (idempotency key).' });
    }
    ...
    missionNotificationService
      .notifyRejection(
        mission,
        role,
        `[${normalizedCategory}] ${normalizedReason}`,
        mission.createdBy
      )
...
export const overrideOrderNumber = async (req, res) => {
  try {
    ...
    if (!mission.orderNumber) {
      return res
        .status(400)
        .json({ error: 'Mission has not been approved yet, cannot override orderNumber' });
    }
  ...
  try {
    ...
    if (!isArchiveQuery) {
      await Promise.all(
        missions
          ...
          .map(async (mission) => {
            try {
              mission.approvalWorkflow = await ensureMissionApprovalWorkflow(
                mission.id,
                organizationId
              );
              emitMissionRealtimeEvent('mission:submitted', mission, organizationId, {
    ...
    const pendingDurationsHours = filtered
      ...
      .map((m) =>
        Math.max(
          0,
          Math.round((nowMs - new Date(m.updatedAt || m.createdAt).getTime()) / 36_000) / 100
        )
      );
    ...
    const avgPendingHours =
      pendingDurationsHours.length > 0
        ? Math.round(
            (pendingDurationsHours.reduce((sum, h) => sum + h, 0) / pendingDurationsHours.length) *
              10
          ) / 10
        : 0;
    ...
    const approvalDurations = approvedWorkflows
      ...
      .map(
        (wf) =>
          Math.max(
            0,
            new Date(wf.orderNumberGeneratedAt).getTime() - new Date(wf.createdAt).getTime()
          ) / 36e5
      );
    const avgApprovalHours =
      approvalDurations.length > 0
        ? Math.round(
            (approvalDurations.reduce((sum, h) => sum + h, 0) / approvalDurations.length) * 10
          ) / 10
        : 0;
