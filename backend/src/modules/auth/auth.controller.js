    ...
    if (user && user.passwordHash && (await bcrypt.compare(password, user.passwordHash))) {
      if (user.requires2FA) {
        ...
        const isValidAnswer = await bcrypt.compare(
          req.body.twoFactorCode.toLowerCase(),
      ...
      // Audit Log - CONNEXION_REUSSIE (après vérification 2FA si applicable)
      await tracerAction({
        userId: user.id,
        organizationId: user.organizationId,
...
export const resetPassword = async (req, res) => {
  try {
    ...
    if (!verified) {
      return res
        .status(400)
        .json({ error: 'Code de récupération ou réponse de sécurité incorrecte' });
    }
