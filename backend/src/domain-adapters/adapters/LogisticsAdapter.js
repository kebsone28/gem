...
export class LogisticsAdapter {
    ...
    deriveStatus(entity) {
        ...
        const criticalItems = (stock || []).filter((item) => item.minAlert !== undefined && item.qty <= item.minAlert);
        if (criticalItems.length >= 3)
            return 'critical_shortage';
        if (fulfillmentRate !== null && fulfillmentRate < 60)
            return 'understocked';
        if (capacity && stock) {
            const totalQty = stock.reduce((sum, item) => sum + (item.qty || 0), 0);
            if (totalQty >= capacity * 0.95)
                return 'full';
        }
        return 'operational';
    }
    generateAlerts(entity) {
        ...
        const lowStockItems = (d.stock || []).filter((item) => item.minAlert !== undefined && item.qty <= item.minAlert);
