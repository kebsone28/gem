...
export class AgricultureAdapter implements DomainAdapter {
  ...
  deriveStatus(entity: any): string {
    ...
    if (actualYield !== null && actualYield !== undefined) return 'harvested';
    if (!currentCrop) return 'prepared';

    if (expectedHarvest && new Date(expectedHarvest) < now) return 'ready';
    if (plantedDate && new Date(plantedDate) <= now) return 'growing';
    if (plantedDate) return 'planted';
