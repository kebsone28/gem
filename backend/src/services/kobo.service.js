...
export async function fetchKoboSubmissions(token, assetUid, since = null) {
  ...
  if (!finalToken || !finalAssetUid) {
    const error = new Error(
      'Configuration Kobo manquante (Token ou ID de formulaire non renseigné dans les paramètres du projet).'
    );
      ...
      try {
        ...
        if (!response.ok) {
          ...
          if (response.status === 503)
            specificMsg =
              'Le serveur KoBoToolbox est temporairement indisponible (Erreur 503). Veuillez réessayer plus tard.';
          if (response.status === 401)
            specificMsg = 'Identifiants KoBo invalides (Token API incorrect).';
          if (response.status === 404)
            specificMsg = "Formulaire KoBo introuvable (Vérifiez l'Asset UID).";
        ...
        retries--;
        logger.warn(
          `[KOBO-SYNC] ⚠️ Échec du téléchargement Kobo (reste ${retries} tentatives) : ${error.message}`
        );
        if (retries === 0) {
          const finalError = new Error(
            `KoboToolbox API error après 3 tentatives : ${error.message}`
          );
 ...
 */
async function mapSubmissionToHousehold(
  submission,
  organizationId,
  defaultZoneId,
  projectId,
  ...
  household.assignedTeams = submission['TYPE_DE_VISITE/role']
    ? [
        ...
        }[String(submission['TYPE_DE_VISITE/role']).toLowerCase()] ||
          submission['TYPE_DE_VISITE/role'],
      ]
  ...
  if (household.latitude && household.longitude && household.region) {
    const validation = validateGPSRegion(
      household.latitude,
      household.longitude,
      household.region,
      submission['_id']
 ...
 */
export async function syncKoboToDatabase(
  organizationId,
  fallbackZoneId,
  since = null,
  projectId = null,
  ...
  if (!targetProjectId) {
    ...
    if (zone) {
      ...
      if (project) {
        ...
        logger.info(
          `[KOBO-SYNC] 💡 fallbackZoneId was actually a projectId [${project.name}]. Resolving default zone...`
        );
  ...
  for (const submission of submissions) {
    try {
      ...
      // 2. Second attempt: Match by numeroordre (Business identifier)
        ...
        try {
          ...
          if (!existingHousehold && numeroDemande.endsWith('0')) {
            ...
            if (existingHousehold) {
              logger.info(
                `[KOBO-SYNC] 💡 Fuzzy match successful: ${numeroDemande} -> ${fallbackNumero}`
              );
            }
          }
          ...
          logger.warn(
            `[KOBO-SYNC] Could not search for existing household by numeroordre [${numeroDemande}]:`,
            e.message
          );
        }
      }
      ...
      const household = await mapSubmissionToHousehold(
        submission,
        organizationId,
        zoneId,
        targetProjectId,
        ...
        for (const rule of decisionRules) {
          ...
          if (isMatch) {
            if (rule.action === 'SET_STATUS' && rule.status) {
              logger.info(
                `[KOBO-DECISION] 🤖 Auto-setting status to [${rule.status}] for household ${numeroDemande} due to rule on [${rule.field}]`
              );
      ...
      for (const [key, value] of Object.entries(submission)) {
        if (typeof value === 'string') {
          ...
          if (
            anomalyKeywords.some((kw) => normalizedValue === kw || normalizedValue === kw + '_')
          ) {
      ...
      const ncPairing = {
        'etape_controleur/group_hx7ae46/DISJONCTEUR_GENERAL_EN_TETE_D_':
          'etape_controleur/group_hx7ae46/OBSERVATIONS_',
        'etape_controleur/group_hx7ae46/ENSEMBLE_DE_L_INSTALLATION_PRO':
          'etape_controleur/group_hx7ae46/OBSERVATIONS__001',
        'etape_controleur/group_hx7ae46/PROTECTION_L_ORIGINE_DE_CHAQ':
          'etape_controleur/group_hx7ae46/OBSERVATIONS_002',
        'etape_controleur/group_hx7ae46/S_PARATION_DES_CIRCUITS_Lumi_':
          'etape_controleur/group_hx7ae46/OBSERVATIONS__002',
        'etape_controleur/group_hx7ae46/PROTECTION_CONTRE_LES_CONTACTS':
          'etape_controleur/group_hx7ae46/OBSERVATIONS__003',
        'etape_controleur/group_hx7ae46/MISE_EN_OEUVRE_MAT_RIEL_ET_APP':
          'etape_controleur/group_hx7ae46/OBSERVATIONS__004',
        'etape_controleur/group_hx7ae46/CONTINUITE_DE_LA_PROTECTION_ME':
          'etape_controleur/group_hx7ae46/OBSERVATIONS__005',
        'etape_controleur/group_hx7ae46/MISE_EN_UVRE_DU_R_SEAU_DE_TER':
          'etape_controleur/group_hx7ae46/OBSERVATIONS__006',
        'etape_controleur/group_hx7ae46/VALEUR_DE_LA_RESISTANCE_DE_TER':
          'etape_controleur/group_hx7ae46/OBSERVATIONS__007',
        'etape_controleur/group_wr05k35/Position_du_branchement':
          'etape_controleur/group_wr05k35/Observations_sur_la_ition_du_branchement',
        'etape_controleur/group_wr05k35/Hauteur_branchement':
          'etape_controleur/group_wr05k35/Observations',
        'etape_controleur/group_wr05k35/Hauteur_coffret':
          'etape_controleur/group_wr05k35/Observations_001',
        'etape_controleur/group_wr05k35/Etat_du_coupe_circuit':
          'etape_controleur/group_wr05k35/OBSERVATION_001',
        'etape_controleur/group_wr05k35/Continuit_PVC':
          'etape_controleur/group_wr05k35/OBSERVATION_002',
        'etape_controleur/group_wr05k35/Mise_en_oeuvre':
          'etape_controleur/group_wr05k35/OBSERVATION_003',
      };
      ...
      if (
        matchType === 'NUMERO_ORDRE' &&
        existingHousehold.koboSubmissionId &&
        existingHousehold.koboSubmissionId !== koboSubmissionId
      ) {
        household.alerts = household.alerts || [];
        household.alerts.push({
      ...
      const normalizedExisting = existingHousehold
        ? normalizeLegacyHousehold(existingHousehold)
        : null;
      ...
      const updateData = {
        ...
        projectId: household.projectId || targetProjectId || undefined,
        ...
        ...(!overrides.includes('departement')
          ? { departement: household.departement || undefined }
          : {}),

        // CRITICAL: On ne remplace pas le village s'il est vide dans Kobo
        ...(!overrides.includes('village') && household.village
          ? { village: household.village }
          : {}),

        // GPS: On respecte le verrouillage Admin
        ...(!overrides.includes('latitude') && household.latitude
          ? { latitude: household.latitude }
          : {}),
        ...(!overrides.includes('longitude') && household.longitude
        ...
        koboSync: {
          ...
          livreurDate:
            household.koboSync.livreurDate || normalizedExisting?.koboSync?.livreurDate || null,
        },

        ...(!overrides.includes('constructionData')
          ? {
              constructionData: {
                ...(typeof normalizedExisting?.constructionData === 'object'
                  ? normalizedExisting.constructionData
                  : {}),
        ...
        alerts: {
          set: [
            ...
            (v, i, a) => a.findIndex((t) => t.message === v.message && t.type === v.type) === i
          ), // Unique
        },
      ...
      if (existingHousehold) {
        ...
        logger.info(
          `[KOBO-SYNC] 🔄 UPDATE existing household: ${existingHousehold.id} (Match Type: ${matchType})`
        );
      ...
      if (
        household.location &&
        Array.isArray(household.location.coordinates) &&
