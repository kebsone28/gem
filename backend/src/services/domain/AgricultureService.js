...
export class AgricultureService {
     ...
     */
    static async calculatePredictedYield(fieldId) {
        const field = await prisma.field.findUnique({ where: { id: fieldId } });
        if (!field)
            throw new Error('Field not found');
        const d = field.domainData;
        if (!d || !d.cropType || !d.area)
            return null;
        // Constantes agronomiques fictives pour le modèle
        const cropYieldPotential = {
            // Céréales
        ...
        if (d.soilType === 'sandy')
            soilFactor = 0.8;
        let waterFactor = 1.0;
        if (d.waterSource === 'irrigation_drip')
            waterFactor = 1.2;
        if (d.waterSource === 'rainfed')
            waterFactor = 0.7;
     ...
     */
    static async calculateWaterRequirements(fieldId) {
        const field = await prisma.field.findUnique({ where: { id: fieldId } });
        if (!field)
            throw new Error('Field not found');
        const d = field.domainData;
     ...
     */
    static async getCropRotationRecommendations(fieldId) {
        const field = await prisma.field.findUnique({ where: { id: fieldId } });
        if (!field)
            throw new Error('Field not found');
        const d = field.domainData;
     ...
     */
    static async calculateFertilizerNeeds(fieldId) {
        const field = await prisma.field.findUnique({ where: { id: fieldId } });
        if (!field)
            throw new Error('Field not found');
        const d = field.domainData;
        if (!d.cropType || !d.area)
            return null;
        ...
        if (d.soilType === 'sandy')
            soilRetention = 1.2; // Lessivage rapide, besoin accru
        if (d.soilType === 'clay')
            soilRetention = 0.9; // Bonne rétention
     ...
     */
    static async assessDiseaseRisk(fieldId) {
        const field = await prisma.field.findUnique({ where: { id: fieldId } });
        if (!field)
            throw new Error('Field not found');
        const d = field.domainData;
     ...
     */
    static async createField(organizationId, projectId, rawData) {
