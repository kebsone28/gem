...
export class MigrationService {
    ...
    _mapStatus(legacyStatus) {
        if (typeof window !== 'undefined' && window.normalizeStatus) {
            return window.normalizeStatus(legacyStatus);
        }
        const map = {
            'En attente': 'Non débuté',
            'En cours': 'Murs: En cours',
            'Terminé': 'Réception: Validée',
            'Bloqué': 'Problème',
            'Validé': 'Réception: Validée'
        };
        return map[legacyStatus] || 'Non débuté';
    }
}
