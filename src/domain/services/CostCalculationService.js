...
export class CostCalculationService {
    constructor(pricingConfig = null) {
        ...
        this._validatePricing();
    }

    _validatePricing() {
        const requirePositive = (field, value, allowUndefined = false) => {
            if (value === undefined || value === null) {
                if (allowUndefined) return;
                throw new (ValidationErrorLocal || window.ValidationError)(`${field} is required`);
            }
            if (typeof value !== 'number' || Number.isNaN(value) || value <= 0) {
                throw new (ValidationErrorLocal || window.ValidationError)(`${field} must be a positive number`);
            }
        };

        if (!this.pricing) {
            throw new (ValidationErrorLocal || window.ValidationError)('Pricing configuration is missing');
        }

        requirePositive('FUEL_PER_LITER', this.pricing.FUEL_PER_LITER, true);

        Object.entries(this.pricing.DAILY_RATES || {}).forEach(([k, v]) => requirePositive(`DAILY_RATES.${k}`, v, true));
        Object.entries(this.pricing.TASK_RATES || {}).forEach(([k, v]) => requirePositive(`TASK_RATES.${k}`, v, true));
        Object.entries(this.pricing.VEHICLES || {}).forEach(([k, v]) => requirePositive(`VEHICLES.${k}`, v, true));
    }
    ...
    static async calculateDetailedCosts(project, teams) {
        ...
        const getStaffCost = (roleId, count, duration) => {
            ...
            if (!config) return (legacyCosts[roleId] || 100000) * duration * count;
            ...
            if (mode === 'monthly') {
                // Utiliser 22 jours ouvrés par mois
                return Math.round((amount / 22) * safeDuration) * count;
            } else if (mode === 'task') {
                // Le coût par tâche est multiplié par le nombre de ménages au total
                // (Note: au niveau de cette fonction on multiplie par count d'équipes, 
                // mais si c'est 'par tâche', le coût est globalement lié aux ménages)
                // Pour garder la cohérence avec ResourceAllocationService:
                return (totalHouseholds * amount) / (count || 1); // Approximation par équipe si nécessaire
            } else {
                // Daily
                return amount * safeDuration * count;
            }
        };
        ...
        const logisticsCosts = Object.entries(requirements.vehicles?.global || {}).reduce((sum, [type, data]) => {
        ...
        if (requirements.support) {
            Object.entries(requirements.support).forEach(([role, data]) => {
                ...
                // Use getStaffCost wrapper.
                supportCosts += getStaffCost(costKey, count, project.duration || requirements.duration || 180);
            });
        } else {
            // Fallback if support requirements not calculated
            // This fallback is now removed as per instruction to use staffConfig exclusively
        ...
        return {
            ...
            breakdown: {
                ...
                fuel: requirements.fuel?.totalCost || 0,
                vehicles: Object.fromEntries(
                    Object.entries(requirements.vehicles?.global || {}).map(([type, data]) => [
                        type,
                        (data.required || 0) * (project.costs?.vehicleRental?.[type] || 50000) * (project.duration || 180)
                    ])
                )
            },
            requirements
    ...
    calculateLaborCost(teams, duration, params = {}, totalHouses = 0) {
        if (!Array.isArray(teams)) {
            throw new (ValidationErrorLocal || window.ValidationError)('teams must be an array');
        }
        if (typeof duration !== 'number' || duration <= 0) {
            throw new (ValidationErrorLocal || window.ValidationError)('duration must be a positive number');
        }
        ...
        for (const team of teams) {
            ...
            const roleId = `per${team.type.charAt(0).toUpperCase() + team.type.slice(1).toLowerCase()}Team`;
            const config = staffConfig[roleId];

            let specificCost = config ? config.amount : userCosts[team.type];
            let mode = config ? config.mode : (paymentModes[team.type] || 'daily');

            const teamCost = this.calculateTeamCost(team, duration, mode, specificCost, totalHouses);
     ...
     */
    calculateTeamCost(team, duration, paymentMode = 'daily', costOverride = null, totalHouses = 0) {
        // Priority: User override > Default pricing
        const baseRate = costOverride !== null ? costOverride : this.getDailyRateForTeam(team.type);

        if (paymentMode === 'task') {
            // Approximation for single team instance cost based on task mode
            const shareOfTask = totalHouses > 0 ? (totalHouses / 100) : 1; // Arbitrary split for per-team view if needed
            return new (CostLocal || window.Cost)(baseRate * shareOfTask);
        }

        if (paymentMode === 'monthly') {
            const memberCount = team.members.length || 1;
            // Utiliser 22 jours ouvrés
            return new (CostLocal || window.Cost)(Math.round(baseRate / 22) * duration * memberCount, 'XOF');
        }

        // Daily mode
        const memberCount = team.members.length || 1;
        return new (CostLocal || window.Cost)(baseRate * duration * memberCount, 'XOF');
    }
