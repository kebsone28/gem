...
export class Project extends (EntityLocal || window.Entity) {
    constructor(id, name, totalHouses, startDate, zones = []) {
        ...
        if (typeof totalHouses !== 'number') {
            console.warn(`[Project] ID=${id}: Invalid totalHouses (${totalHouses}), defaulting to 0`);
            totalHouses = 0;
        }
        ...
        this._budget = null;
        this._parameters = {};
        this._equipmentRegistry = {};
        this._users = [];
        this._roles = {};
        this._version = 1; // Default to V1 structurally
        this._config = {};
        this._logistics = {};
    }
    ...
    set totalHouses(value) {
        if (typeof value !== 'number' || value < 0) {
            throw new (ValidationErrorLocal || window.ValidationError)('Total houses must be a non-negative number');
        }
        this._totalHouses = value;
        this.touch();
    }
    ...
    get equipmentRegistry() {
        return { ...this._equipmentRegistry };
    }

    set equipmentRegistry(registry) {
        this.setEquipmentRegistry(registry);
    }
    ...
    set version(v) { this._version = v; this.touch(); }
    ...
    /**
     * Définit le registre des équipements
     */
    setEquipmentRegistry(registry) {
        this._equipmentRegistry = { ...registry };
        this.touch();
    ...
    toJSON() {
        return {
            ...
            equipmentRegistry: this._equipmentRegistry,
            config: this._config,
            logistics: this._logistics
        };
    }
    ...
    static fromJSON(data) {
        ...
        // Resilience: Default values for mandatory fields
        const id = data.id || `migrated-${Date.now()}`;
        const name = data.name || 'Projet sans nom';
        const totalHouses = Number(data.totalHouses || data.total || 0);
        ...
        try {
            ...
            project._equipmentRegistry = data.equipmentRegistry || {};
            project._config = data.config || {};
            project._logistics = data.logistics || {};
            // Handle indices if present
            if (data.index) project.index = data.index;
            project._users = data.users || [];
            project._roles = data.roles || {};
            project._version = data.version || 1; // Fallback to 1 if missing

            project._createdAt = data.createdAt ? new Date(data.createdAt) : new Date();
            project._updatedAt = data.updatedAt ? new Date(data.updatedAt) : new Date();

            return project;
        } catch (e) {
            console.error(`❌ [Project.fromJSON] Failed to hydrate project ${id}:`, e.message);
            throw e;
        }
    }
}
