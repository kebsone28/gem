...
// Simple escape function for template strings
...
const CONTRAT_LOT_A = \`${escapeForTemplate(lotA)}\`;

const CONTRAT_LOT_B = \`${escapeForTemplate(lotB)}\`;

const CONTRAT_LOT_C = \`${escapeForTemplate(lotC)}\`;
...
// ─── Génération DOCX ───────────────────────────────────────────────────────────
async function generateContratBlob(params: {
  lot: 'A' | 'B' | 'C';
...
<Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships">
  ...
  const bodyParts = content.split('\\\\n').map(line => {
    ...
    // Check for headers
    if (trimmedLine.startsWith('Article') || trimmedLine.match(/^\\d+\\./)) {
      return wHeading(trimmedLine, 2);
    } else if (trimmedLine.startsWith('Titre')) {
      return wHeading(trimmedLine.replace('Titre ', ''), 1);
    } else if (trimmedLine.match(/^[A-Z]{2,}/) && trimmedLine.length < 50) {
      // Likely a section header (all caps, short)
      return wHeading(trimmedLine, 1);
    } else {
      return wText(trimmedLine);
    }
  }).filter(part => part !== '');
...
export async function generateContratDocxRich(params: {
  ...
  const blob = await generateContratBlob(params);
  const filename = \`Contrat_Lot\${params.lot}_\${params.region}_\${params.grappe}_\${params.entreprise.replace(/\\s+/g, '_')}.docx\`;
  triggerDownload(blob, filename);
}
`;

fs.writeFileSync(path.join(__dirname, 'frontend/src/modules/carto_grappes/engine/docxEngine.ts'), basicContent, 'utf8');

console.log('Rebuilt clean docxEngine.ts with proper formatting');
