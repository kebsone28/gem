...
$resolvedHost = Get-FirstNonEmpty @($ServerHost, $env:WANEKOO_HOST, $env:DEPLOY_HOST, 'proquelec.sn')
...
cd "__DEPLOY_PATH__"
  if [ -n "${GEM_SMOKE_TOKEN:-}" ]; then
    ...
    echo '[DEPLOY] Toolbox smoke skipped: set GEM_SMOKE_TOKEN on the VPS to enable authenticated smoke checks.'
fi
'@
