    ...
    return (
        ...
        >
            <div className="flex items-center gap-3 mb-6">
                ...
                <div>
                    <h4 className="text-[10px] font-black uppercase tracking-[0.2em] opacity-50 italic">Analytique</h4>
            ...
            <div className="space-y-4">
                ...
                <div className="pt-4 mt-2 border-t border-gray-100 dark:border-white/5">
                    <div className="flex justify-between items-end">
                        <span className="text-[9px] font-black uppercase tracking-[0.2em] opacity-40 italic leading-none">Taux de Livraison</span>
                    ...
                    <div className="h-1.5 w-full bg-gray-100 dark:bg-white/10 rounded-full mt-2 overflow-hidden">
                        <motion.div
                            initial={{ width: 0 }}
                            animate={{ width: `${stats.visible > 0 ? (stats.completed / stats.visible) * 100 : 0}%` }}
...
const StatRow = ({ icon, label, value, color }: { icon: any, label: string, value: number, color: string }) => (
    <div className="flex items-center justify-between">
        <div className="flex items-center gap-2.5">
            <span className={`${color} opacity-80`}>{icon}</span>
            <span className="text-[10px] font-bold uppercase tracking-widest opacity-60 italic">{label}</span>
        </div>
