...
interface MapComponentProps {
  ...
  className?: string;
  onRouteFound?: (
    stats: { distance: number; duration: number; instructions?: any[] } | null
  ) => void;
    ...
    <div className="h-full w-full relative bg-slate-100 dark:bg-slate-900 overflow-hidden">
        ...
        <div className="absolute inset-0 z-20 flex items-center justify-center bg-slate-950/40 text-white p-6 text-center pointer-events-none">
          <div className="max-w-md">
            ...
            <div className="text-lg font-bold">
              Aucune donnée géographique valide à afficher dans cette vue
            </div>
            <p className="mt-2 text-sm text-slate-200">
              La carte s’affiche, mais il n’existe aucun ménage avec des coordonnées GPS valides
      ...
      {showDatabaseStats && (
        <MapStatsWidget
          stats={{
            ...
            completed: safeHouseholds.filter(
              (h: Household) =>
                getHouseholdDerivedStatus(h) === 'Contrôle conforme' ||
                getHouseholdDerivedStatus(h) === 'Intérieur terminé'
            ).length,
            problems: safeHouseholds.filter(
              (h: Household) => getHouseholdDerivedStatus(h) === 'Non conforme'
            ).length,
            pending: safeHouseholds.filter(
              (h: Household) => getHouseholdDerivedStatus(h) === 'Non encore commencé'
            ).length,
          }}
        />
      )}
      ...
      {showLegend && (
        <div className="absolute bottom-8 left-4 z-[100] px-5 py-4 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-white/10 shadow-2xl">
          <h4 className="text-xs font-black uppercase tracking-widest mb-3 text-slate-500 dark:text-slate-400">
            Légende / Étapes
          </h4>
