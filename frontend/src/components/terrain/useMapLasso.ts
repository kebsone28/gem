...
export const useMapLasso = (
    ...
    const setupLasso = useCallback((map: maplibregl.Map) => {
        ...
        const onMouseMove = (e: maplibregl.MapMouseEvent) => {
            ...
            const dist = Math.sqrt(Math.pow(e.lngLat.lng - lastPoint[0], 2) + Math.pow(e.lngLat.lat - lastPoint[1], 2));
            
            if (dist > 0.0001) { // Threshold for adding points
            ...
            if (polygon.length > 2) {
                ...
                (householdsRef.current || []).forEach(h => {
                    if (h.location?.coordinates) {
                        const point: [number, number] = [h.location.coordinates[0], h.location.coordinates[1]];
