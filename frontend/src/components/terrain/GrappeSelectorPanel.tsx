...
export function GrappeSelectorPanel({
  ...
  return (
    <motion.div
      ...
      exit={{ opacity: 0, x: -20 }}
      className={`absolute top-[144px] left-3 right-3 md:top-16 md:right-4 md:left-auto z-[40] max-w-[calc(100vw-1.5rem)] md:w-80 rounded-2xl border shadow-2xl backdrop-blur-xl flex flex-col overflow-hidden max-h-[calc(100vh-220px)] md:max-h-[calc(100vh-120px)] ${bg}`}
    >
      <div
      ...
      >
        ...
        <button
          onClick={onClose}
          className={`p-1.5 rounded-lg hover:bg-slate-500/10 ${subText}`}
      ...
      <div className="flex-1 overflow-y-auto p-3 pt-0 space-y-2">
        ...
        >
          <div
            className={`w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 ${isDarkMode ? 'bg-slate-700 text-white' : 'bg-slate-200 text-slate-600'}`}
          >
            <MapPin size={14} />
          </div>
          <div className="flex-1">
            <p
              className={`text-sm font-bold ${activeGrappeId === null ? (isDarkMode ? 'text-indigo-400' : 'text-indigo-700') : text}`}
            >
              🌍 Toutes les zones
            </p>
        ...
        {isLoading && clusters.length === 0 && (
          <div className="space-y-2 mt-4">
            {[1, 2, 3, 4, 5].map((i) => (
              <div
                key={i}
                className={`w-full flex items-center gap-3 p-3 rounded-xl border animate-pulse ${isDarkMode ? 'border-slate-800 bg-slate-800/20' : 'border-slate-100 bg-slate-50'}`}
              >
                <div
                  className={`w-4 h-8 rounded-md flex-shrink-0 ${isDarkMode ? 'bg-slate-700' : 'bg-slate-200'}`}
                />
                <div className="flex-1 space-y-2">
                  <div
                    className={`h-3 rounded w-1/2 ${isDarkMode ? 'bg-slate-700' : 'bg-slate-200'}`}
                  />
                  <div
                    className={`h-2 rounded w-1/4 ${isDarkMode ? 'bg-slate-700' : 'bg-slate-200'}`}
                  />
                </div>
              </div>
            ))}
            ...
            return (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <p
                    className={`text-sm font-bold truncate ${isActive ? (isDarkMode ? 'text-emerald-400' : 'text-emerald-700') : text}`}
                  >
                    {c.name}
                  </p>
