    ...
    useEffect(() => {
        ...
        try {
            ...
            if (!map.getSource('pending-zone')) {
                map.addSource('pending-zone', { type: 'geojson', data: { type: 'FeatureCollection', features: [] } });
            }
            if (!map.getSource('drawn-zones')) {
                map.addSource('drawn-zones', { type: 'geojson', data: { type: 'FeatureCollection', features: [] } });
            }
        } catch (err) {
        ...
        if (source) {
            source.setData({
                ...
                features: pendingPoints.length > 0 ? [{
                    type: 'Feature',
                    geometry: { type: 'Polygon', coordinates: [[...pendingPoints, pendingPoints[0]]] },
                    properties: {}
                } as any] : []
            });
        }
