...
function ensureLayers(map: maplibregl.Map) {
  ...
  if (!map.getLayer('cluster-zone-badge-glow')) {
    map.addLayer({
      ...
      paint: {
        'circle-radius': ['interpolate', ['linear'], ['get', 'point_count'], 8, 18, 180, 28],
  ...
  if (!map.getLayer('cluster-zone-badge-shell')) {
    map.addLayer({
      ...
      layout: {
        ...
        'icon-size': ['interpolate', ['linear'], ['get', 'point_count'], 8, 0.5, 120, 0.68, 220, 0.82],
  ...
  if (!map.getLayer('cluster-zone-name-labels')) {
    map.addLayer({
      ...
      layout: {
        ...
        'text-font': SAFE_FONT,
        'text-size': ['interpolate', ['linear'], ['get', 'point_count'], 8, 10.5, 120, 13.5],
        'text-line-height': 1,
  ...
  if (!map.getLayer('cluster-zone-labels')) {
    map.addLayer({
      ...
      layout: {
        ...
        'text-font': SAFE_FONT,
        'text-size': ['interpolate', ['linear'], ['get', 'point_count'], 8, 9.2, 120, 11.4],
        'text-line-height': 1,
