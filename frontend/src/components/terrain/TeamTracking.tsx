 ...
 * S'appuie sur l'endpoint backend GET /api/teams/positions (optionnel — mode simulé si absent).
 */
...
interface Props {
    ...
    /** Filtre côté API (optionnel) — équipes du projet actif uniquement */
...
export function TeamTrackingPanel({ isDarkMode = true, onSelectPosition, projectId }: Props) {
    ...
    const fetchPositions = useCallback(async () => {
        ...
        try {
            ...
            /** DTO canonique backend: { userId, name, role, lat, lon, updatedAt, accuracy? } */
            const normalized: TeamMemberPosition[] = (Array.isArray(raw) ? raw : []).map((p: any) => {
                if (p?.userId && (p?.lat !== undefined) && (p?.lon !== undefined) && p?.updatedAt) {
                    return {
                        ...
                        role: String(p.role || 'INSTALLATION'),
                        lat: Number(p.lat),
                        lon: Number(p.lon),
                ...
                /** Compat legacy: { id, name, role|type, coordinates: { lat, lng }, lastUpdate } */
                ...
                return {
                    ...
                    name: String(p?.name || 'Équipe'),
                    role: String(p?.role || p?.type || 'INSTALLATION'),
                    lat: Number(lat ?? 0),
                    lon: Number(lon ?? 0),
