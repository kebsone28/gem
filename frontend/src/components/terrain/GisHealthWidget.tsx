...
const GisHealthWidget: React.FC<GisHealthWidgetProps> = ({ result, onFlyTo, isDarkMode }) => {
    ...
    const getSeverityIcon = (severity: string) => {
        switch (severity) {
            case 'CRITICAL': return <AlertTriangle size={12} className="text-rose-500 shrink-0" />;
            case 'WARNING': return <AlertTriangle size={12} className="text-amber-500 shrink-0" />;
            default: return <Info size={12} className="text-blue-500 shrink-0" />;
        }
    };
    ...
    return (
        <div className="fixed bottom-24 right-6 z-[4000] flex flex-col items-end gap-3 pointer-events-none">
                    ...
                    >
                        ...
                        <div className={`p-4 border-b flex items-center justify-between ${isDarkMode ? 'border-slate-800' : 'border-slate-100'}`}>
                            <div className="flex items-center gap-2">
                                ...
                                <span className={`text-xs font-black uppercase tracking-widest ${isDarkMode ? 'text-white' : 'text-slate-900'}`}>Santé GIS (Audit)</span>
                            </div>
                            <button 
                        ...
                        <div className="flex-1 overflow-y-auto p-2 scrollbar-none">
                            {anomalies.length > 0 ? (
                                        ...
                                        >
                                            <div className="flex items-start gap-2">
                                                ...
                                                <div className="flex-1 flex flex-col gap-0.5">
                                                    <span className={`text-xs font-black leading-tight uppercase tracking-tight ${isDarkMode ? 'text-slate-200' : 'text-slate-900'}`}>
                                                        {anno.message}
                                                    </span>
                                                    <span className="text-xs font-mono opacity-50">#{anno.householdId.slice(-8)}</span>
                                                </div>
                                                {anno.lng && anno.lat && (
                                                    <button 
                                                        onClick={(e) => {
                                ...
                                <div className="h-40 flex flex-col items-center justify-center gap-3 text-slate-400">
                                    ...
                                    <span className="text-xs font-black uppercase tracking-widest">Aucune anomalie détectée</span>
                                </div>
                            )}
                        </div>

                        {/* Footer Notification */}
                        <div className={`p-4 text-xs font-black uppercase tracking-tighter italic ${isDarkMode ? 'text-slate-500 bg-white/5' : 'text-slate-400 bg-slate-50'}`}>
            ...
            >
                ...
                <div className={`flex flex-col items-start gap-0.5 pr-2 border-r ${isDarkMode ? 'border-slate-800' : 'border-slate-100'}`}>
                    <span className={`text-[12px] font-black leading-none ${getScoreColor(healthScore)}`}>
                        {healthScore}%
                    </span>
                    <span className="text-xs font-black uppercase tracking-widest text-slate-400">Santé GIS</span>
                </div>
                
                <Activity size={18} className={`transition-all ${stats.critical > 0 ? 'text-rose-500 animate-pulse' : 'text-blue-500'}`} />
