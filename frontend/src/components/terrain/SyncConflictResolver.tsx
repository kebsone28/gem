                ...
                >
                    ...
                    <div className="p-8 border-b border-white/5 flex items-center justify-between">
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                <h2 className="text-xl font-black text-white italic uppercase tracking-tight">Conflits de Données Détectés</h2>
                                <p className="text-slate-400 text-xs font-bold uppercase tracking-widest mt-1">
                                    {conflicts.length} ménage(s) ont des informations différentes dans le nouveau fichier.
                                </p>
                            </div>
                        </div>
                        <div className="flex items-center gap-4">
                            <div className="flex items-center gap-2 mr-4 border-r border-white/10 pr-4">
                                <button onClick={handleSelectAllLocal} className="px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest text-blue-500 bg-blue-500/10 hover:bg-blue-500/20 transition-all border border-blue-500/20">
                                    Tout Local
                                </button>
                                <button onClick={handleSelectAllRemote} className="px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest text-indigo-500 bg-indigo-500/10 hover:bg-indigo-500/20 transition-all border border-indigo-500/20">
                                    Tout Kobo/Import
                                </button>
                            </div>
                            <button onClick={onCancel} className="p-3 rounded-2xl hover:bg-white/5 text-slate-500 transition-all" title="Fermer" aria-label="Fermer">
                                <X size={24} />
                            </button>
                        </div>
                    </div>
                    ...
                    <div className="flex-1 overflow-y-auto p-4 space-y-4">
                        {conflicts.map((conflict, idx) => (
                            <div key={idx} className="grid grid-cols-[1fr,auto,1fr] gap-6 p-6 rounded-3xl bg-white/5 border border-white/5 items-center">
                                {/* Local Column */}
                                <button 
                                ...
                                >
                                    <div className="text-[10px] font-black text-blue-400 uppercase mb-3 italic">Base de Données Locale</div>
                                    <h4 className="text-white font-black uppercase text-sm mb-4 truncate">{conflict.local.owner || 'Sans Nom'}</h4>
                                    ...
                                    <div className="space-y-2">
                                        ...
                                        <div className="flex items-center justify-between text-[10px]">
                                            ...
                                            <span className="text-white font-mono">{conflict.local.location?.coordinates?.[1].toFixed(5)}, {conflict.local.location?.coordinates?.[0].toFixed(5)}</span>
                                        </div>
                                    </div>
                                </button>
                                ...
                                <div className="flex flex-col items-center gap-2">
                                    ...
                                    <span className="text-[8px] font-black text-slate-600 uppercase tracking-tighter">ID: {conflict.local.id}</span>
                                </div>
                                ...
                                >
                                    <div className="text-[10px] font-black text-indigo-400 uppercase mb-3 italic">Donnée Kobo / Import</div>
                                    <h4 className="text-white font-black uppercase text-sm mb-4 truncate">{conflict.remote.owner || 'Sans Nom'}</h4>
                                    
                                    <div className="space-y-2">
                                        {conflict.fields.map(field => (
                                            <div key={field} className="flex items-center justify-between text-[10px] bg-amber-500/10 p-1 px-2 rounded -mx-1">
                                                <span className="text-amber-500 font-black tracking-tighter">{field.toUpperCase()}:</span>
                    ...
                    <div className="p-8 border-t border-white/5 bg-black/20 flex items-center justify-between">
                        <div className="flex items-center gap-2 text-[10px] font-bold text-slate-500 uppercase tracking-widest">
                            <span className="text-blue-500">{Object.values(decisions).filter(v => v === 'local').length} local</span>
                            <span>•</span>
                            <span className="text-indigo-500">{Object.values(decisions).filter(v => v === 'remote').length} importés</span>
                        </div>
                        
                        <div className="flex gap-4">
                            <button onClick={onCancel} className="px-8 py-4 rounded-2xl border border-white/10 text-white font-black uppercase tracking-widest text-[11px] hover:bg-white/5 transition-all">
                                Tout Annuler
                            </button>
                            <button 
                                onClick={handleApply}
