    ...
    const handleFileImport = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            ...
            if (file.name.endsWith('.kml')) {
                ...
                const features = placemarks.map(p => {
                    ...
                    if (points.length === 1) return { type: 'Feature', geometry: { type: 'Point', coordinates: points[0] }, properties: { name } };
                    return { type: 'Feature', geometry: { type: 'LineString', coordinates: points }, properties: { name } };
                }).filter(Boolean);
            ...
            const fallback = {
                ...
                features: [{
                    ...
                    geometry: {
                        type: 'Polygon',
                        coordinates: [[
                            [-17.53, 12.30], [-11.36, 12.30],
                            [-11.36, 16.69], [-17.53, 16.69],
                            [-17.53, 12.30]
                        ]]
                    },
        ...
        >
            ...
            <div className="p-4 border-b border-slate-700/30 flex items-center gap-3">
                ...
                <div>
                    ...
                    <p className={`text-xs ${sub}`}>{layers.length} couche{layers.length !== 1 ? 's' : ''} active{layers.length !== 1 ? 's' : ''}</p>
                </div>
            </div>
