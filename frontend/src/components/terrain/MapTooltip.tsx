                ...
                {isCluster ? (
                    <div className="flex flex-col gap-2">
                        <div className="flex items-center gap-2 mb-1">
                            ...
                            <span className="text-[10px] font-black uppercase tracking-widest text-blue-400">Cluster de ménages</span>
                        </div>
                        <div className="text-xl font-black text-white italic">
                            {data.point_count} <span className="text-[10px] not-italic font-bold uppercase text-slate-400 opacity-60">Points</span>
                        </div>
                    </div>
                ) : (
                    <div className="flex flex-col gap-3">
                        <div className="flex items-center justify-between">
                            <span className={`px-2 py-0.5 rounded-lg text-[8px] font-black uppercase tracking-tighter ${
                                data.status === 'Livraison effectuée' ? 'bg-emerald-500/20 text-emerald-400 border border-emerald-500/20' : 'bg-blue-500/20 text-blue-400 border border-blue-500/20'
                            }`}>
                                {data.status || 'Ménage' }
                            </span>
                            <span className="text-[9px] font-bold text-slate-500 uppercase tracking-widest">#{data.numeroordre || data.id?.slice(0, 8)}</span>
                        </div>
                        ...
                        <div className="flex flex-col gap-1">
                            ...
                            <div className="flex items-center gap-2 text-slate-300">
                                ...
                                <span className="text-[10px] font-bold uppercase tracking-widest truncate max-w-[150px]">{data.village || data.region || 'Localisation...'}</span>
                            </div>
                        </div>
                    </div>
                )}
