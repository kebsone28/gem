...
export default function DataHubSection({ project, onUpdate }: DataHubSectionProps) {
  ...
  const [collectSource, setCollectSource] = useState<'kobo' | 'gedtoolbox'>(
    project?.config?.collectSource || 'kobo'
  );
  ...
  const handleKoboSync = async (force = false) => {
    ...
    try {
      ...
      setKoboStep(0);
      toast.error(
        e?.response?.data?.message || `Erreur ${collectSource === 'kobo' ? 'Kobo' : 'GedToolbox'}`,
        { id: 'ged-os-sync' }
      );
    } finally {
      ...
      if (rawData.length > 0) {
        ...
        const findValue = (obj: any, aliasList: string[]) => {
          ...
          for (const alias of aliasList)
            if (normalizedObjKeys.has(alias)) return obj[normalizedObjKeys.get(alias)!];
          return undefined;
        };

        // Log detected columns for debugging
        const sampleRow = rawData[0];
        const detectedColumns = Object.keys(sampleRow).join(', ');
        console.log(`[DataHub] Detected columns: ${detectedColumns}`);
        console.log(`[DataHub] Total rows in file: ${rawData.length}`);
          ...
          .map((row: any, index: number) => {
            ...
            if (hId === undefined || hId === null || hId === '') {
              if (hOwner || hVillage) {
                ...
                // Last resort: sequential ID
                finalId = `import_seq_${index + 1}`;
                generatedIdCount++;
                console.log(`[DataHub] Generated sequential ID for row ${index}: ${finalId}`);
              }
            } else {
        ...
        console.log(`[DataHub] Skipped ${skippedCount} rows due to missing ID`);
        console.log(`[DataHub] Generated IDs for ${generatedIdCount} rows`);
  ...
  const handleMaintenance = async (action: string) => {
    ...
    if (action.includes('purge_server'))
      confirmMsg = `⚠️ DANGER SÉVÈRE : Voulez-vous VRAIMENT supprimer définitivement ces données du SERVEUR CENTRAL ? Cette action est irréversible.`;

    if (!window.confirm(confirmMsg)) return;

    const password = action.includes('purge_server')
      ? prompt("Veuillez saisir votre mot de passe pour confirmer l'action nucléaire :")
      : null;
          ...
          <div>
            ...
            <div className="flex items-center gap-3 mt-1">
              <div className="flex items-center gap-1.5 px-2 py-0.5 rounded-full bg-emerald-500/10 border border-emerald-500/20 text-[9px] font-black uppercase text-emerald-400">
                <div
                  className={`w-1.5 h-1.5 rounded-full bg-emerald-500 ${connectionStatus === 'connected' ? 'animate-pulse' : 'opacity-50'}`}
                />
              ...
              <div className="space-y-8">
                ...
                <div className="bg-indigo-600/5 border border-indigo-500/20 rounded-3xl p-6 flex flex-col md:flex-row items-center justify-between gap-6">
                  <div className="flex items-center gap-4">
                    <div
                      className={`w-12 h-12 rounded-2xl flex items-center justify-center transition-all ${collectSource === 'kobo' ? 'bg-blue-600/20 text-blue-400' : 'bg-emerald-600/20 text-emerald-400'}`}
                    >
                    ...
                    <div>
                      <h3 className="text-white font-black uppercase tracking-widest text-sm">
                        ...
                        <span
                          className={
                            collectSource === 'kobo' ? 'text-blue-400' : 'text-emerald-400'
                          }
                        >
                      ...
                      <p className="text-[10px] text-slate-500 font-bold uppercase mt-1">
                        {collectSource === 'kobo'
                          ? 'Récupération via API externe KoBo'
                          : 'Plan B : Utilisation des formulaires internes GEM'}
                      </p>
                    </div>
                  </div>
                  ...
                  >
                    <div
                      className={`absolute inset-y-1 transition-all duration-500 rounded-full ${collectSource === 'kobo' ? 'left-1 right-24 bg-blue-600 shadow-[0_0_15px_rgba(59,130,246,0.5)]' : 'left-24 right-1 bg-emerald-600 shadow-[0_0_15px_rgba(16,185,129,0.5)]'}`}
                    />
                    <div className="relative z-10 h-full flex items-center justify-between px-4">
                      <span
                        className={`text-[9px] font-black uppercase tracking-widest transition-colors ${collectSource === 'kobo' ? 'text-white' : 'text-slate-500'}`}
                      >
                        KoBo
                      </span>
                      <span
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                  <div className="bg-slate-900/30 p-8 rounded-[2rem] border border-white/5 space-y-6">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        ...
                        <p className="text-[10px] text-slate-500 font-bold uppercase mt-1">
                          Dernière sync : {lastSync ? lastSync.toLocaleTimeString() : 'Jamais'}
                        </p>
                      </div>
                    </div>
                  ...
                  <div className="bg-slate-900/30 p-8 rounded-[2rem] border border-white/5 space-y-6 relative overflow-hidden">
                    ...
                    <div className="flex items-center gap-4">
                      <div
                        className={`w-12 h-12 rounded-2xl flex items-center justify-center transition-all ${collectSource === 'kobo' ? 'bg-blue-500/10 text-blue-500' : 'bg-emerald-500/10 text-emerald-500'}`}
                      >
                    ...
                    <div className="flex flex-col gap-3">
                      <div className="flex items-center justify-between mb-2">
                        <div className="flex items-center gap-2">
                          <div
                            className={`w-2 h-2 rounded-full animate-pulse ${collectSource === 'kobo' ? 'bg-blue-500' : 'bg-emerald-500'}`}
                          />
                          <span
                            className={`text-[10px] font-black uppercase tracking-widest ${collectSource === 'kobo' ? 'text-blue-400' : 'text-emerald-400'}`}
                          >
                            {collectSource === 'kobo' ? 'Écoute Kobo' : 'Écoute Interne'}
                          </span>
                        </div>
                        <div
                          className={`px-2 py-0.5 border rounded-md text-[8px] font-black uppercase transition-all ${collectSource === 'kobo' ? 'bg-blue-500/10 border-blue-500/20 text-blue-400' : 'bg-emerald-500/10 border-emerald-500/20 text-emerald-400'}`}
                        >
                          Auto-Sync: 5m
                        </div>
                      </div>
                      ...
                      <div className="mt-4 pt-4 border-t border-white/5 flex flex-col gap-2">
                        ...
                        >
                          {collectSource === 'kobo'
                            ...
                            : 'Réindexer toutes les soumissions'}
                        </button>
              ...
              <div className="max-w-3xl mx-auto">
                ...
                >
                  ...
                  <p className="text-sm text-slate-500 font-bold mt-2 uppercase tracking-widest">
                    Excel (xlsx) ou CSV supportés
                  </p>
                  <p className="text-[10px] text-slate-600 font-medium mt-4 uppercase tracking-[0.2em] max-w-md mx-auto leading-relaxed">
                    Importez manuellement vos données. Le système détectera automatiquement les
                    colonnes et les distribuera dans les bons projets.
                  </p>
                  <div className="mt-8 flex justify-center gap-4">
                    <span className="px-4 py-2 rounded-xl bg-black/40 border border-white/5 text-[10px] font-black text-slate-400 uppercase">
                      Detection auto
                    </span>
                    <span className="px-4 py-2 rounded-xl bg-black/40 border border-white/5 text-[10px] font-black text-slate-400 uppercase">
                      Multi-projets
            ...
            {activeTab === 'projects' && (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {projects?.map((p: any) => (
                  <div
                    key={p.id}
                    className={`p-6 rounded-3xl border transition-all ${p.id === activeProjectId ? 'bg-blue-600 border-blue-400 shadow-xl shadow-blue-900/20' : 'bg-slate-900/30 border-white/5 hover:bg-white/5'}`}
                  >
                    <div className="flex items-center justify-between mb-4">
                      <Database
                        size={20}
                        className={p.id === activeProjectId ? 'text-white' : 'text-blue-500'}
                      />
                      {p.id === activeProjectId && <StatusBadge status="success" label="Actif" />}
                    </div>
                    <h4
                      className={`font-black uppercase tracking-tight text-lg ${p.id === activeProjectId ? 'text-white' : 'text-slate-200'}`}
                    >
                      {p.name}
                    </h4>
                    <p
                      className={`text-[10px] font-bold uppercase tracking-widest mt-1 ${p.id === activeProjectId ? 'text-blue-100' : 'text-slate-500'}`}
                    >
                      {p.id.slice(0, 8)}
                    </p>
                ...
                <div className="grid grid-cols-1 gap-3">
                  {backups.length > 0 ? (
                    backups.map((b) => (
                      <div
                        key={b.id}
                        className="bg-slate-900/40 p-5 rounded-2xl border border-white/5 flex items-center justify-between group hover:border-white/10 transition-all"
                      >
                        <div className="flex items-center gap-4">
              ...
              <div className="max-w-4xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="bg-slate-900/30 p-8 rounded-[2rem] border border-white/5 space-y-4">
                  ...
                  <p className="text-xs text-slate-500 leading-relaxed font-bold">
                    Nettoie les doublons et répare les files de synchronisation corrompues dans
                    votre navigateur.
                  </p>
                  <button
                ...
                <div className="bg-slate-900/30 p-8 rounded-[2rem] border border-white/5 space-y-4">
                  ...
                  <p className="text-xs text-slate-500 leading-relaxed font-bold">
                    Supprime toutes les données locales pour forcer un rechargement complet depuis
                    le serveur.
                  </p>
                  <button
                  ...
                  <div className="flex items-center gap-4 text-rose-500">
                    ...
                    <div>
                      <h3 className="font-black uppercase tracking-widest text-lg">
                        Zone de Danger Nucléaire
                      </h3>
                      <p className="text-[10px] font-bold uppercase tracking-widest opacity-60">
                        Actions irréversibles sur le serveur central
                      </p>
                    </div>
                  </div>
