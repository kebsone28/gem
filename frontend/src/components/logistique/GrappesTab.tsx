...
export default function GrappesTab() {
  const { project, computeCompleteness, computeRiskIndex, updateAssignment, grappesConfig } =
    useLogistique();
  ...
  const subGrappes = (grappesConfig as any)?.sous_grappes || [];
  return (
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between xl:justify-end">
          ...
          <div className="flex items-center justify-between gap-4 sm:justify-start">
            ...
            {[1, 2, 3, 4].map((i) => (
              <div
                key={i}
                className="w-10 h-10 rounded-full border-2 border-slate-950 bg-slate-800 flex items-center justify-center text-xs font-bold text-slate-400"
              >
                É{i}
              </div>
            ))}
        ...
        {subGrappes.map((sg: any) => {
          ...
          return (
            <div
              ...
              className={`bg-slate-900 border rounded-3xl p-6 transition-all cursor-pointer group relative overflow-hidden active:scale-95 ${
            ...
            >
              {isBulkMode && (
                <div
                  className={`absolute top-4 right-4 w-6 h-6 rounded-full border-2 flex items-center justify-center transition-colors ${selectedIds.includes(sg.id) ? 'bg-indigo-500 border-indigo-500 text-white' : 'border-slate-600'}`}
                >
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between text-xs font-bold">
                  ...
                  <span className={score === 100 ? 'text-emerald-400' : 'text-blue-400'}>
                    {score}%
                  </span>
                </div>
                  ...
                  {Array.from(new Set(teams.map((t: Team) => t.tradeKey))).map((key: any) => {
                    ...
                    return (
                      <div
                        ...
                        title={
                          isAssigned
                            ? `${assignedTeams.length} équipe(s) affectée(s)`
                      ...
                      >
                        <div className="flex items-center gap-1">
                          <Icon size={14} />
                          {isAssigned && (
                            <span className="text-xs font-bold">{assignedTeams.length}</span>
                          )}
                        </div>
                      </div>
                    );
                ...
                <div key={key} className="flex flex-col gap-2">
                  <div className="relative">
                    <select
                      ...
                      onChange={(e) => {
                        ...
                        if (val === 'UNASSIGN') {
                          ...
                          val &&
                          !currentBulk.includes(val) &&
                          !currentBulk.includes('UNASSIGN')
                        ) {
                    ...
                    >
                      ...
                      {teams
                        ?.filter((t: Team) => t.tradeKey === key)
                        .map((t: Team) => (
                          <optgroup key={t.id} label={t.name}>
                            {(t.children || []).map((st) => (
                              <option key={st.id} value={st.id}>
                                {st.name} {st.leader ? `(${st.leader})` : ''}
                              </option>
                            ))}
                          </optgroup>
                        ))}
                      ...
                      {currentBulk.map((val) => {
                        if (val === 'UNASSIGN') {
                          return (
                            <div
                              key={val}
                              className="flex items-center gap-1 text-xs bg-red-500/20 text-red-300 font-bold px-2 py-1 rounded"
                            >
                              <span>Désassignation</span>
                              <button
                                onClick={() =>
                                  setBulkAssignments({ ...bulkAssignments, [key]: [] })
                                }
                        ...
                        const stName =
                          teams?.flatMap((t) => t.children || []).find((st) => st.id === val)
                            ?.name || val;
                        return (
                          <div
                            key={val}
                            className="flex items-center gap-1 text-xs bg-indigo-500/20 text-indigo-300 font-bold px-2 py-1 rounded"
                          >
                            ...
                            <button
                              onClick={() =>
                                setBulkAssignments({
                                  ...bulkAssignments,
                                  [key]: currentBulk.filter((v) => v !== val),
                                })
                              }
                              className="hover:text-white ml-1"
                            >
            ...
            <div className="space-y-6 max-h-[55vh] overflow-y-auto pr-2">
              {Array.from(new Set(teams.map((t: Team) => t.tradeKey))).map((key: any) => {
                ...
                const currentAssignments =
                  project?.config?.assignments?.[selectedSubGrappe.id]?.[key] || [];

                return (
                  <div
                    key={key}
                    className="bg-slate-950 border border-slate-800 p-6 rounded-3xl group hover:border-blue-500/20 transition-all"
                  >
                    <div className="flex items-center justify-between mb-4">
                      ...
                      <span className="text-xs font-black text-slate-500 uppercase tracking-widest">
                        {currentAssignments.length} affectée(s)
                      </span>
                    </div>
                    ...
                    {currentAssignments.length > 0 && (
                      <div className="flex flex-wrap gap-2 mb-4">
                        {currentAssignments.map((assignedId: string) => {
                          const foundSubTeam = teams
                            ?.flatMap((t: Team) => t.children || [])
                            .find((st) => st.id === assignedId);
                          return (
                            <div
                              key={assignedId}
                              className="flex items-center gap-2 bg-indigo-500/10 text-indigo-400 border border-indigo-500/20 px-3 py-1.5 rounded-lg text-xs font-bold"
                            >
                              <span>{foundSubTeam ? foundSubTeam.name : assignedId}</span>
                              <button
                                onClick={() =>
                                  updateAssignment(
                                    selectedSubGrappe.id,
                                    key,
                                    currentAssignments.filter((id: string) => id !== assignedId)
                                  )
                                }
                      ...
                      <select
                        ...
                        onChange={(e) => {
                          if (e.target.value && !currentAssignments.includes(e.target.value)) {
                            updateAssignment(selectedSubGrappe.id, key, [
                              ...currentAssignments,
                              e.target.value,
                            ]);
                          }
