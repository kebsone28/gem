...
export default function WorkshopTab() {
    ...
    const [selectedWarehouseId, setSelectedWarehouseId] = useState<string | 'global'>(
        'global'
    );
    ...
    const totalHouses = (project as any)?.totalHouses || project?.config?.grappesConfig?.grappes?.reduce(
        (s: number, g: any) => s + (g.nb_menages || 0), 0
    ) || 3500;
    ...
    const handleDeleteWarehouse = async (id: string, name: string) => {
        ...
        if (remainingStock > 0) {
            toast.error(`Impossible de supprimer "${name}" : il reste ${remainingStock} kits en stock. Transférez le stock d'abord.`);
            return;
        }

        if (!confirm(`Souhaitez-vous vraiment supprimer le magasin "${name}" ? Cette action est réversible par un administrateur.`)) return;
    ...
    const handleTransfer = async () => {
        ...
        const sourceStock = Math.max(0, (sourceWh?.kitsLoadedToday || 0) - (sourceWh?.kitsConsumed || 0));
    ...
    const handleAddWarehouse = async () => {
        if (!newWhForm.name || !newWhForm.region) { toast.error('Nom et région requis'); return; }
    ...
    return (
        <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <div className="flex flex-wrap gap-2 items-center">
                <span className="text-slate-400 text-xs font-bold uppercase tracking-widest mr-2">Vue :</span>
                    ...
                    <div className="bg-slate-900 border border-slate-800 rounded-2xl p-8 w-full max-w-md">
                        ...
                        <div className="space-y-4">
                            <div>
                                ...
                                <input value={newWhForm.name} onChange={e => setNewWhForm(p => ({...p, name: e.target.value}))}
                        ...
                        <div className="flex gap-3 mt-6">
                            <button onClick={() => setAddingWarehouse(false)} className="flex-1 py-3 rounded-xl bg-slate-800 text-slate-300 font-bold hover:bg-slate-700 transition-all">Annuler</button>
                            <button onClick={handleAddWarehouse} className="flex-1 py-3 rounded-xl bg-blue-600 text-white font-bold hover:bg-blue-500 transition-all">Créer</button>
                        </div>
                    </div>
                </div>
            )}
                            ...
                            <div>
                                ...
                                <select 
                                    ...
                                    {warehouseStats.map(w => (
                                        <option key={w.id} value={w.id}>{w.name} (Stock: {Math.max(0, w.kitsLoadedToday - w.kitsConsumed)})</option>
                                    ))}
                                </select>
                            </div>
                            <div>
                                <label className="text-xs font-bold text-slate-400 uppercase tracking-widest">Vers (Destination)</label>
                                <select 
                                    title="Magasin destination"
                                    value={transferForm.to} onChange={e => setTransferForm(p => ({...p, to: e.target.value}))}
                                    className="mt-2 w-full bg-slate-950 border border-slate-700 text-white rounded-xl px-4 py-3 focus:outline-none focus:border-indigo-500">
                                    <option value="">Destination...</option>
                                    {warehouses.filter(w => w.id !== transferForm.from).map(w => (
                                        <option key={w.id} value={w.id}>{w.name}</option>
                                    ))}
                                </select>
                            </div>
                            <div>
                                <label className="text-xs font-bold text-slate-400 uppercase tracking-widest">Quantité (Kits)</label>
                                <input type="number" value={transferForm.qty} onChange={e => setTransferForm(p => ({...p, qty: e.target.value}))}
                                    className="mt-2 w-full bg-slate-950 border border-slate-700 text-white rounded-xl px-4 py-3 focus:outline-none focus:border-indigo-500"
                                    placeholder="Nombre de kits complets" />
                            </div>
                        </div>
                        <div className="flex gap-3 mt-6">
                            <button onClick={() => setShowTransferModal(false)} className="flex-1 py-3 rounded-xl bg-slate-800 text-slate-300 font-bold hover:bg-slate-700 transition-all">Annuler</button>
                            <button onClick={handleTransfer} className="flex-1 py-3 rounded-xl bg-indigo-600 text-white font-bold hover:bg-indigo-500 transition-all">Confirmer Transfert</button>
                        </div>
                    </div>
                </div>
            )}
            ...
            <div className="grid grid-cols-1 lg:grid-cols-5 gap-8">
                        ...
                        <div className="space-y-6">
                            <div className="bg-slate-900 border border-slate-800 rounded-[2rem] p-8">
                                <div className="flex items-center gap-3 mb-8">
                                    ...
                                    <h3 className="text-2xl font-black text-white uppercase tracking-tight">Tableau de Bord National</h3>
                                </div>
                                ...
                                <div className="space-y-4">
                                    <h4 className="text-xs font-black text-slate-400 uppercase tracking-widest">Répartition par Zone</h4>
                                    <div className="space-y-3">
                                        {warehouseStats.map(ws => (
                                            <div key={ws.id} className="bg-slate-950/40 border border-slate-800 px-6 py-4 rounded-xl flex items-center justify-between">
                        ...
                        <>
                            ...
                            <div className="bg-slate-900 border border-slate-800 rounded-[2rem] p-8 relative overflow-hidden">
                                ...
                                <div className="flex items-center justify-between mb-6">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <h3 className="text-xl font-black text-white">Stock Local : {activeWh.name}</h3>
                                    </div>
                                    <div className={`flex items-center gap-2 px-3 py-1.5 rounded-xl bg-${healthBadge.color}-500/10 border border-${healthBadge.color}-500/30`}>
                                        <healthBadge.icon size={14} className={`text-${healthBadge.color}-400`} />
                                        <span className={`text-${healthBadge.color}-400 font-bold text-xs`}>{healthBadge.text}</span>
                                    </div>
                                </div>
                                ...
                                <div className="grid grid-cols-3 gap-4 mb-6">
                                    {[
                                        ...
                                        { label: 'Disponible', val: Math.max(0, kitsLoadedToday - kitsConsumed), color: activeWh?.hasAlert ? 'rose' : 'slate' },
                                    ].map(card => (
                                        <div key={card.label} className={`bg-slate-950/60 border border-${card.color}-500/20 rounded-2xl p-4 text-center`}>
                                            <p className="text-3xl font-black text-white">{card.val}</p>
                                            <p className={`text-xs font-bold text-${card.color}-400 uppercase tracking-widest mt-1`}>{card.label}</p>
                                        </div>
                                    ))}
                                </div>
                                <div className="mt-4 pt-4 border-t border-slate-800">
                                    <button 
                                        onClick={() => { setReceivingWh(activeWh.id); setShowReceiveModal(true); }}
                                ...
                                {activeWh?.hasAlert && (
                                    <div className="flex items-center gap-3 bg-red-500/10 border border-red-500/30 rounded-xl p-3">
                                        ...
                                        <p className="text-red-300 text-xs font-semibold">Risque de rupture locale. Stock insuffisant pour la cadence actuelle.</p>
                                    </div>
                                )}
                            </div>
                                    ...
                                    <div className="space-y-3">
                                        <div className="grid grid-cols-2 gap-3">
                                            <div>
                                                ...
                                                <input value={coordForm.lat} onChange={e => setCoordForm(p => ({...p, lat: e.target.value}))}
                                            ...
                                            <div>
                                                ...
                                                <input value={coordForm.lng} onChange={e => setCoordForm(p => ({...p, lng: e.target.value}))}
                                                    className="mt-1 w-full bg-slate-950 border border-slate-700 text-white rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:border-orange-500"
                                                    placeholder="Ex: -15.5560" />
                                            </div>
                                        </div>
                                        <div>
                                            <label className="text-xs font-bold text-slate-400 uppercase tracking-widest">Adresse Physique</label>
                                            <input value={coordForm.address} onChange={e => setCoordForm(p => ({...p, address: e.target.value}))}
                                                className="mt-1 w-full bg-slate-950 border border-slate-700 text-white rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:border-orange-500"
                                                placeholder="Ex: Route nationale, Kaffrine" />
                                        </div>
                                        <div className="flex gap-2 pt-1">
                                            <button onClick={() => setEditingCoords(false)} className="flex-1 flex items-center justify-center gap-2 py-2 rounded-lg bg-slate-800 text-slate-300 text-sm font-bold hover:bg-slate-700 transition-all"><X size={14}/> Annuler</button>
                                            <button onClick={handleSaveCoords} className="flex-1 flex items-center justify-center gap-2 py-2 rounded-lg bg-orange-600 text-white text-sm font-bold hover:bg-orange-500 transition-all"><Save size={14}/> Enregistrer</button>
                                        </div>
                                    </div>
                                ) : (
                                        ...
                                        {activeWh?.latitude && activeWh?.longitude ? (
                                            <>
                                                <div className="flex items-center gap-3 text-sm">
                                                    <span className="font-mono text-slate-300 bg-slate-950 border border-slate-800 rounded-lg px-3 py-1.5 text-xs">{activeWh.latitude.toFixed(6)}, {activeWh.longitude.toFixed(6)}</span>
                                                </div>
                                ...
                                {filteredTeams.length > 0 ? (
                                    <div className="divide-y divide-slate-800">
                                        {filteredTeams.map(team => {
                                            ...
                                            return (
                                                <div key={team.id} className="flex items-center gap-4 px-6 py-4 hover:bg-slate-800/20 transition-all">
                                                    ...
                                                    <div className="flex items-center gap-2">
                                                        ...
                                                        <div className="flex items-center gap-1 bg-slate-950/60 p-1 rounded-xl border border-slate-800">
                                                            ...
                                                            <input type="number" min="0"
                                                                ...
                                                                onChange={e => setLoadingInputs(p => ({...p, [team.id]: e.target.value}))}
                                                                placeholder="0"
                            ...
                            <div className="bg-slate-900 border border-slate-800 rounded-2xl overflow-hidden shadow-xl">
                                <div className="flex items-center gap-3 px-6 py-4 border-b border-slate-800 bg-slate-950">
                                    ...
                                    <h4 className="font-black text-white uppercase tracking-tight text-sm">Journal de Bord : {activeWh.name}</h4>
                                </div>
                                <div className="max-h-[300px] overflow-y-auto divide-y divide-slate-800/50">
                                    {movementHistory.filter((m: any) => m.warehouseId === activeWh.id || m.fromId === activeWh.id || m.toId === activeWh.id).length > 0 ? (
                                        movementHistory.filter((m: any) => m.warehouseId === activeWh.id || m.fromId === activeWh.id || m.toId === activeWh.id).map((move: any) => (
                                            <div key={move.id} className="px-6 py-3 hover:bg-slate-800/20 transition-all flex items-center justify-between gap-4">
                                                <div className="flex items-center gap-3">
                ...
                {showReceiveModal && (
                    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-sm">
                        <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="bg-slate-900 border border-slate-800 rounded-[2rem] p-8 w-full max-w-md shadow-2xl">
                            <h3 className="text-xl font-black text-white mb-2 uppercase tracking-tight">Réception de Matériel</h3>
                            <p className="text-slate-400 text-sm mb-6">Enregistrez l'arrivée de nouveaux kits dans ce magasin.</p>
                            ...
                            <div className="space-y-4">
                                ...
                                <div className="flex gap-3 pt-4">
                                    <button onClick={() => setShowReceiveModal(false)} className="flex-1 py-3 rounded-xl bg-slate-800 text-slate-300 font-bold hover:bg-slate-700 transition-all">Annuler</button>
                                    <button onClick={handleReceiveStock} className="flex-1 py-3 rounded-xl bg-emerald-600 text-white font-bold hover:bg-emerald-500 shadow-lg shadow-emerald-900/40 transition-all">Confirmer</button>
                                </div>
                            </div>
                        </motion.div>
                    </div>
                ...
                <div className="lg:col-span-2">
                    <div className="bg-gradient-to-br from-indigo-900/40 via-slate-900 to-slate-900 border border-indigo-500/20 p-8 rounded-[2rem] shadow-2xl relative overflow-hidden group sticky top-6">
                        ...
                        <div className="flex items-center gap-3 mb-6">
                            ...
                            <h3 className="text-xl font-black text-white tracking-tight uppercase">Intelligence Réseau</h3>
                        </div>
