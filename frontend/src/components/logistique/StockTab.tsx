...
export default function StockTab({ searchQuery = '' }: StockTabProps) {
    ...
    const categoriesMap = KIT_CATEGORIES.map(cat => ({
        name: cat,
        items: stockData.filter((i: any) => i.category === cat && i.label.toLowerCase().includes(searchQuery.toLowerCase()))
        ...
        <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            {warehouseStats && warehouseStats.length > 0 && (
                <div className="flex flex-wrap gap-2 items-center">
                    <span className="text-slate-400 text-xs font-bold uppercase tracking-widest mr-2">Filtre Magasin :</span>
            ...
            {activeWh?.hasAlert && (
                <div className="flex items-center gap-4 bg-red-500/10 border border-red-500/30 rounded-2xl p-4">
                    ...
                    <div>
                        <h4 className="font-black text-red-500 uppercase tracking-tighter">Alerte Rupture Imminente</h4>
                        <p className="text-red-400/80 text-sm font-medium leading-tight mt-0.5">Le magasin <b>{activeWh.name}</b> montre un risque de rupture dans moins de 3 jours sur {activeWh.alerts?.length} article(s).</p>
                    </div>
                </div>
            )}
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                {[
                    ...
                    },
                    {
                        ...
                        color: activeWh?.daysBeforeBreakout < 3 ? 'rose' : activeWh?.daysBeforeBreakout < 7 ? 'amber' : 'emerald',
                    ...
                    const colorClasses: Record<string, { bg: string; icon: string; badge: string }> = {
                        ...
                        indigo: { bg: 'bg-indigo-500/10 border-indigo-500/20', icon: 'text-indigo-500', badge: 'bg-indigo-500/10 text-indigo-500' },
                        rose: { bg: 'bg-rose-500/10 border-rose-500/20', icon: 'text-rose-500', badge: 'bg-rose-500/10 text-rose-500' },
                        slate: { bg: 'bg-slate-500/10 border-slate-500/20', icon: 'text-slate-500', badge: 'bg-slate-500/10 text-slate-500' }
                    };
                                    ...
                                    <table className={`w-full text-sm ${isDarkMode ? 'divide-slate-800' : 'divide-slate-200'}`}>
                                        <thead className={isDarkMode ? 'bg-slate-950' : 'bg-slate-50'}>
                                            <tr>
                                                ...
                                                <th className={`px-6 py-3 text-center text-xs font-semibold uppercase tracking-wider ${isDarkMode ? 'text-slate-400' : 'text-slate-600'}`}>Consommé</th>
                                                <th className={`px-6 py-3 text-right text-xs font-semibold uppercase tracking-wider ${isDarkMode ? 'text-slate-400' : 'text-slate-600'}`}>Stock Réel</th>
                                            </tr>
                                        </thead>
                                        <tbody className={isDarkMode ? 'divide-y divide-slate-800' : 'divide-y divide-slate-200'}>
                                            {cat.items.map((item: any) => {
                                                ...
                                                return (
                                                    <tr key={item.id} className={`${isDarkMode ? 'hover:bg-slate-800/30' : 'hover:bg-slate-50'} ${isCritical ? 'bg-red-500/5' : ''}`}>
                                                        ...
                                                        <td className={`px-6 py-4 text-right`}>
                                                            <div className="flex flex-col items-end">
                                                                <span className={`text-lg font-black ${isCritical ? 'text-red-500' : isWarning ? 'text-amber-500' : isDarkMode ? 'text-indigo-400' : 'text-indigo-600'}`}>
