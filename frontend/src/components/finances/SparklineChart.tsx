...
const toX = (i: number, total: number) => PAD.left + (i / (total - 1)) * (W - PAD.left - PAD.right);
...
export default function SparklineChart() {
  ...
  return (
    <div
      className={`border rounded-[2.5rem] p-8 shadow-2xl transition-all ${isDarkMode ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-100'}`}
    >
      {/* Header */}
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3
              className={`font-black text-base tracking-tight uppercase italic ${isDarkMode ? 'text-white' : 'text-slate-900'}`}
            >
        ...
        <div className="flex items-center gap-4">
          ...
          {/* Delta badge */}
          <div
            className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full font-black text-xs ${
              deltaSign === 'positive'
      ...
      {/* Footer summary */}
      <div
        className={`flex items-center justify-between mt-4 pt-4 border-t ${isDarkMode ? 'border-slate-800' : 'border-slate-100'}`}
      >
        ...
        <div className="flex items-center gap-4">
          <div>
            <span
              className={`text-xs font-black uppercase tracking-widest ${isDarkMode ? 'text-slate-600' : 'text-slate-400'}`}
            >
              Total Prévu
            </span>
          ...
          <div>
            ...
            <p
              className={`text-sm font-black ${deltaSign === 'positive' ? 'text-emerald-500' : 'text-rose-500'}`}
            >
              {lastActual}M FCFA
            </p>
          </div>
