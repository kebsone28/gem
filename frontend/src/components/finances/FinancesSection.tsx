...
/** Ligne autonome avec calcul en temps réel */
function DevisRow({
  item,
  onUpdatePlanned,
  onUpdateReal,
  ...
  item: any;
  onUpdatePlanned: (id: string, field: 'qty' | 'unit', val: number) => void;
  onUpdateReal: (id: string, field: 'qty' | 'unit', val: number) => void;
  onDelete: (id: string) => void;
}) {
  const [pQty, setPQty] = useState<number>(item.qty ?? 0);
  ...
  const margin = totalPrevu - totalReel;
  ...
  return (
    <tr className="group hover:bg-white/5 transition-all">
      ...
      <td className="px-4 py-3 text-sm font-bold text-white border-r border-white/5">
        {item.label}
      </td>
      <td className="px-4 py-3 text-xs font-black text-indigo-400 uppercase text-center border-r border-white/10">
        {item.region}
      </td>
      ...
      <td className="px-2 py-1 text-right">
        <input
          type="number"
          value={pQty}
          onChange={(e) => setPQty(Number(e.target.value) || 0)}
          onBlur={(e) => onUpdatePlanned(item.id, 'qty', Number(e.target.value) || 0)}
      ...
      <td className="px-2 py-1 text-right">
        <input
          type="number"
          value={pUnit}
          onChange={(e) => setPUnit(Number(e.target.value) || 0)}
          onBlur={(e) => onUpdatePlanned(item.id, 'unit', Number(e.target.value) || 0)}
      ...
      <td className="px-4 py-3 text-right text-sm font-bold text-blue-400 border-r border-white/10 whitespace-nowrap">
      ...
      <td className="px-2 py-1 text-right">
        <input
          type="number"
          value={rQty}
          onChange={(e) => setRQty(Number(e.target.value) || 0)}
          onBlur={(e) => onUpdateReal(item.id, 'qty', Number(e.target.value) || 0)}
      ...
      <td className="px-2 py-1 text-right">
        <input
          type="number"
          value={rUnit}
          onChange={(e) => setRUnit(Number(e.target.value) || 0)}
          onBlur={(e) => onUpdateReal(item.id, 'unit', Number(e.target.value) || 0)}
      ...
      <td className="px-4 py-3 text-right text-sm font-bold text-amber-300 border-r border-white/10 whitespace-nowrap">
        {totalReel.toLocaleString('fr-FR')}
      </td>

      {/* MARGE */}
      <td
        className={`px-4 py-3 text-right text-sm font-black whitespace-nowrap ${margin >= 0 ? 'text-emerald-400' : 'text-rose-400'}`}
      >
...
export function FinancesSection({ project, onUpdate }: Props) {
  ...
  const handleExcel = async () => {
    ...
    utils.book_append_sheet(
      wb,
      utils.json_to_sheet(data.length ? data : [{ info: 'Aucun poste' }]),
      'Devis vs Reel'
    );
    ...
    <div className="space-y-10">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        ...
        <div className="flex items-center gap-3 flex-wrap">
          <button
            onClick={handleExcel}
            className="flex items-center gap-2 px-5 py-3 bg-emerald-600 hover:bg-emerald-700 text-white font-black text-xs rounded-xl transition-all active:scale-95"
          >
          ...
          <button
            onClick={async () => {
              if (
                confirm(
                  "Voulez-vous vraiment réinitialiser aux valeurs d'exemple de base ? Vos données importées seront effacées."
                )
              ) {
          ...
          {previousConfig && (
            <button
              onClick={handleUndo}
              className="flex items-center gap-2 px-5 py-3 bg-rose-600 hover:bg-rose-700 text-white font-black text-xs rounded-xl transition-all active:scale-95"
            >
              ANNULER L'IMPORT
            </button>
          )}
      ...
      {importErrors.length > 0 && (
        <div className="p-4 bg-rose-900/20 border border-rose-800 text-rose-300 rounded-2xl">
          <div className="flex items-center gap-2 font-black text-xs uppercase mb-2">
            <AlertTriangle size={14} /> Erreurs d'import
          </div>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              ...
              <tr className="bg-slate-950/70 border-b border-white/5">
                <th
                  colSpan={3}
                  className="px-4 py-2 text-xs font-black text-slate-400 uppercase tracking-widest text-center border-r border-white/10"
                >
                  DONNÉE FINANCIÈRE
                </th>
                <th
                  colSpan={3}
                  className="px-4 py-2 text-xs font-black text-blue-400 uppercase tracking-widest text-center border-r border-white/10"
                >
                  COÛT PROJET (PRÉVISION)
                </th>
                <th
                  colSpan={3}
                  className="px-4 py-2 text-xs font-black text-amber-400 uppercase tracking-widest text-center border-r border-white/10"
                >
                  CHARGE RÉELLE
                </th>
              ...
              <tr className="bg-slate-950/50 border-b border-white/5">
                <th className="px-4 py-3 text-xs font-black text-slate-500 uppercase tracking-widest text-left border-r border-white/5">
                  ID
                </th>
                <th className="px-4 py-3 text-xs font-black text-slate-500 uppercase tracking-widest text-left border-r border-white/5">
                  Poste de Dépense
                </th>
                <th className="px-4 py-3 text-xs font-black text-slate-500 uppercase tracking-widest text-center border-r border-white/10">
                  Région
                </th>
                <th className="px-4 py-3 text-xs font-black text-blue-300 uppercase tracking-widest text-right">
                  Prév. Qté
                </th>
                <th className="px-4 py-3 text-xs font-black text-blue-300 uppercase tracking-widest text-right">
                  Prév. PU (FCFA)
                </th>
                <th className="px-4 py-3 text-xs font-black text-blue-300 uppercase tracking-widest text-right border-r border-white/10">
                  Total Prévu
                </th>
                <th className="px-4 py-3 text-xs font-black text-amber-300 uppercase tracking-widest text-right">
                  Réel Qté
                </th>
                <th className="px-4 py-3 text-xs font-black text-amber-300 uppercase tracking-widest text-right">
                  Réel PU (FCFA)
                </th>
                <th className="px-4 py-3 text-xs font-black text-amber-300 uppercase tracking-widest text-right border-r border-white/10">
                  Total Réel
                </th>
                <th className="px-4 py-3 text-xs font-black text-emerald-400 uppercase tracking-widest text-right">
                  Écart (FCFA)
                </th>
                <th></th>
              </tr>
            ...
            <tbody className="divide-y divide-white/5">
              {devisItems.length === 0 ? (
                <tr>
                  <td colSpan={11} className="py-16 text-center text-slate-600 font-bold text-sm">
                    Aucun poste. Ajoutez-en ci-dessous ou importez un Excel.
                  </td>
                </tr>
              ) : (
                devisItems.map((item: any) => (
                  <DevisRow
                    key={item.id}
                    item={item}
                    onUpdatePlanned={updatePlannedCost}
                    onUpdateReal={updateRealCost}
                    onDelete={deleteDevisItem}
                  />
                ))
              )}
            </tbody>
            {devisItems.length > 0 && (
              <tfoot className="bg-slate-950/60 border-t-2 border-white/10">
                <tr>
                  <td
                    colSpan={3}
                    className="px-4 py-4 text-xs font-black text-slate-400 uppercase tracking-widest border-r border-white/10"
                  >
                    TOTAL — {devisItems.length} postes
                  </td>
                  <td colSpan={2} className="px-4 py-4" />
                  <td className="px-4 py-4 text-right text-base font-black text-blue-400 border-r border-white/10">
                    {devisItems
                      .reduce((s: number, i: any) => s + (i.planned || 0), 0)
                      .toLocaleString('fr-FR')}
                  </td>
                  <td colSpan={2} className="px-4 py-4" />
                  <td className="px-4 py-4 text-right text-base font-black text-amber-300 border-r border-white/10">
                    {devisItems
                      .reduce((s: number, i: any) => s + (i.realTotal || 0), 0)
                      .toLocaleString('fr-FR')}
                  </td>
                  <td
                    className={`px-4 py-4 text-right text-base font-black ${devis.globalMargin >= 0 ? 'text-emerald-400' : 'text-rose-400'}`}
                  >
                    {devis.globalMargin.toLocaleString('fr-FR')}
                  </td>
                  <td />
                </tr>
              </tfoot>
      ...
      <div className="bg-white/5 border border-white/10 rounded-2xl p-5 flex flex-col sm:flex-row items-stretch gap-4">
        <input
          id="fin_label"
          placeholder="Libelle du poste (ex: Transport materiel)"
          title="Libelle"
          className="flex-1 bg-slate-950 border border-white/10 rounded-xl px-4 py-3 text-sm text-white focus:ring-2 focus:ring-indigo-500 outline-none"
        />
        <select
          id="fin_region"
          title="Region"
          className="bg-slate-950 border border-white/10 rounded-xl px-4 py-3 text-sm text-white focus:ring-2 focus:ring-indigo-500 outline-none"
        >
        ...
        <input
          ...
          title="Quantite"
          defaultValue={1}
          className="w-24 bg-slate-950 border border-white/10 rounded-xl px-3 py-3 text-sm text-white focus:ring-2 focus:ring-indigo-500 outline-none"
        />
        <input
          id="fin_unit"
          type="number"
          ...
          defaultValue={0}
          className="w-32 bg-slate-950 border border-white/10 rounded-xl px-3 py-3 text-sm text-white focus:ring-2 focus:ring-indigo-500 outline-none"
        />
        <button
          onClick={() => {
            ...
            const qty =
              Number((document.getElementById('fin_qty') as HTMLInputElement)?.value) || 1;
            const unit =
              Number((document.getElementById('fin_unit') as HTMLInputElement)?.value) || 0;
            if (lbl) {
