...
export default function MaterialDatabase() {
  ...
  const handleExcelImport = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = async (evt) => {
      try {
        ...
        const wb = await XLSX.read(String(bstr || ''), { type: 'binary' });
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div
        className={`p-8 rounded-[2.5rem] border-2 transition-all flex flex-col md:flex-row items-center justify-between gap-6 ${
          includeSupply
      ...
      >
        <div className="flex items-center gap-6">
          <div
            className={`p-4 rounded-2xl ${includeSupply ? 'bg-indigo-600 text-white' : 'bg-slate-800 text-slate-500'}`}
          >
            <Database size={32} />
          </div>
          <div>
            <h3
              className={`text-xl font-black uppercase tracking-tighter italic ${isDarkMode ? 'text-white' : 'text-slate-900'}`}
            >
              Suivi de l'Approvisionnement Matériel
            </h3>
            <p className="text-xs font-medium text-slate-500 max-w-md">
              Activez cette option pour inclure le coût d'achat des matériaux dans le bilan
              financier global. Sinon, seuls les coûts de main d'œuvre et logistique seront
              comptabilisés.
            </p>
          </div>
        </div>
      ...
      {/* Inventory Table */}
      <div
        className={`border rounded-[2.5rem] overflow-hidden shadow-2xl transition-all ${isDarkMode ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-100'}`}
      >
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500 font-medium">
                Gérez vos références et prix unitaires HT
              </p>
            </div>
          </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left">
            <thead>
              <tr
                className={`border-b ${isDarkMode ? 'bg-slate-950/30 border-slate-800/50' : 'bg-slate-50 border-slate-100'}`}
              >
                  ...
                  >
                    ...
                    <td className="px-6 py-4 text-right">
                      <input
                        ...
                        value={newItem.stock}
                        onChange={(e) =>
                          setNewItem({ ...newItem, stock: parseInt(e.target.value) || 0 })
                        }
                      />
                    </td>
                    <td className="px-6 py-4 text-right">
                      <input
                        ...
                        onChange={(e) =>
                          setNewItem({ ...newItem, unitPrice: parseInt(e.target.value) || 0 })
                        }
                      />
                    </td>
                    ...
                    >
                      <td className="px-8 py-5">
                        <input
                          ...
                          onChange={() =>
                            updateInventoryItem(item.id, { isActive: item.isActive === false })
                          }
                      ...
                      <td className="px-6 py-5">
                        <div className="relative">
                          <select
                            ...
                            value={item.category}
                            onChange={(e) =>
                              updateInventoryItem(item.id, { category: e.target.value })
                            }
                          >
                          ...
                          <div
                            className={`absolute -left-3 top-1/2 -translate-y-1/2 w-1.5 h-1.5 rounded-full transition-shadow group-hover:shadow-[0_0_8px] 
                                                        ${
                                                          categoryColor === 'indigo'
                      ...
                      <td className="px-6 py-5 text-right font-mono text-xs text-slate-400">
                        <input
                          ...
                          value={item.stock}
                          onChange={(e) =>
                            updateInventoryItem(item.id, { stock: parseInt(e.target.value) || 0 })
                          }
                        />
                      </td>
                      <td className="px-6 py-5 text-right font-mono text-xs text-slate-400">
            ...
            <tfoot>
              <tr className="bg-slate-950/30">
                <td
                  colSpan={5}
                  className="px-8 py-6 text-xs font-black uppercase tracking-[0.3em] text-slate-500"
                >
                  VALORISATION TOTALE DU STOCK (APPROVISIONNEMENT)
                </td>
      ...
      {/* Note Section */}
      <div
        className={`p-6 rounded-3xl border flex gap-4 ${isDarkMode ? 'bg-amber-500/5 border-amber-500/20' : 'bg-amber-50 border-amber-100'}`}
      >
        ...
        <div className="space-y-1">
          ...
          <p className="text-xs text-slate-500 leading-relaxed font-medium">
            Les prix saisis ici sont utilisés pour calculer la part "Achat/Approvisionnement" du
            projet. Si le maître d'ouvrage fournit lui-même ces équipements, désactivez le calcul en
            haut de page pour que votre bilan financier ne porte que sur vos prestations de services
            (Installation, Transport, Gestion).
          </p>
        </div>
      </div>
    </div>
