...
const MODE_COLORS: Record<AIEngineMode, string> = {
  ...
  HYBRID_RULES_FIRST:
    'border-emerald-500/50 bg-emerald-900/30 shadow-[0_0_20px_rgba(16,185,129,0.15)]',
...
export default function AIEngineAdminPanel({ user, onClose, standalone = false }: Props) {
  const isAuthorized = hasPermission(user, PERMISSIONS.IA_CONFIG);
  ...
  const checkAllVps = useCallback(async () => {
    ...
    try {
      ...
      setVpsStatus({
        ollama: {
          status: data.ollama?.status === 'ok' ? 'ok' : 'error',
          hint: data.ollama?.hint,
        ...
        codeserver: {
          ...
          hint: data.codeserver?.hint,
          command: data.codeserver?.command,
          url: data.codeserver?.url,
        },
      });
  ...
  if (!isAuthorized) {
    return (
      <div
        className={`${standalone ? '' : 'fixed inset-0 z-50'} flex items-center justify-center bg-slate-950/80 backdrop-blur-xl p-4`}
      >
  ...
  const content = (
    <div
      className={`bg-[#020617] border border-slate-800/50 rounded-[2.5rem] shadow-[0_0_100px_rgba(0,0,0,0.5)] w-full ${standalone ? '' : 'max-w-2xl max-h-[95vh]'} overflow-hidden flex flex-col scale-in-center`}
    >
      {/* Header Premium */}
      ...
      <div className="p-8 space-y-8 overflow-y-auto custom-scrollbar flex-1 bg-gradient-to-b from-[#020617] to-slate-950">
          ...
          {activeTab === 'engines' && (
            ...
            >
              <section className="rounded-2xl border border-cyan-500/20 bg-cyan-950/20 p-5 flex items-start gap-4 backdrop-blur-sm group hover:border-cyan-500/40 transition-all">
                ...
                <div>
                  ...
                  <p className="mt-1.5 text-xs leading-relaxed text-slate-300/90 font-medium">
                    Orchestration centralisée du cerveau GEM. Les appels sont sécurisés via votre
                    VPS privé PROQUELEC.
                  </p>
                </div>
              </section>
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center gap-4 mb-3 relative z-10">
                        <div
                          className={`w-12 h-12 rounded-2xl flex items-center justify-center text-3xl bg-slate-950/50 border border-white/5 transition-transform group-hover:scale-110 ${isActive ? 'shadow-[0_0_15px_rgba(255,255,255,0.1)]' : ''}`}
                        >
                          {MODE_ICONS[mode]}
                        </div>
                        <p
                          className={`font-black text-xs uppercase tracking-tight ${isActive ? 'text-white' : 'text-slate-400'}`}
                        >
                          {getModeLabelFR(mode)}
                        </p>
                      </div>
                      <p
                        className={`text-[10px] leading-relaxed relative z-10 font-bold ${isActive ? 'text-white/80' : 'text-slate-500'}`}
                      >
              ...
              <div className="grid grid-cols-2 gap-4">
                <div
                  className={`rounded-2xl p-5 border transition-all duration-500 ${rulesActive ? 'bg-blue-600/10 border-blue-500/40 shadow-[0_0_20px_rgba(59,130,246,0.1)]' : 'bg-slate-900/50 border-slate-800 opacity-40 grayscale'}`}
                >
                  ...
                  <div className="flex items-center gap-2">
                    <div
                      className={`w-2 h-2 rounded-full ${rulesActive ? 'bg-blue-400 animate-pulse shadow-[0_0_8px_rgba(96,165,250,0.8)]' : 'bg-slate-600'}`}
                    ></div>
                ...
                <div
                  className={`rounded-2xl p-5 border transition-all duration-500 ${aiActive ? 'bg-purple-600/10 border-purple-500/40 shadow-[0_0_20px_rgba(168,85,247,0.1)]' : 'bg-slate-900/50 border-slate-800 opacity-40 grayscale'}`}
                >
                  ...
                  <div className="flex items-center gap-2">
                    <div
                      className={`w-2 h-2 rounded-full ${aiActive ? 'bg-purple-400 animate-pulse shadow-[0_0_8px_rgba(192,132,252,0.8)]' : 'bg-slate-600'}`}
                    ></div>
                    <p className="text-xs font-black text-white">
            ...
            >
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {[
                  ...
                  return (
                    ...
                    >
                      <div
                        className={`p-3 rounded-2xl bg-gradient-to-br ${isEnabled ? item.color : 'from-slate-800 to-slate-900 opacity-40'} text-white shadow-lg`}
                      >
                        <Icon size={20} />
                      </div>
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center justify-between mb-2">
                          ...
                          >
                            <div
                              className={`absolute top-1 w-4 h-4 rounded-full bg-white transition-all ${isEnabled ? 'left-6' : 'left-1'} shadow-sm`}
                            />
                          </div>
                        </div>
              ...
              <div className="p-6 bg-gradient-to-r from-emerald-600/10 to-teal-600/5 border border-emerald-500/20 rounded-3xl flex items-center gap-5 hover:border-emerald-500/40 transition-all group">
                ...
                <div className="flex-1">
                  <p className="text-[10px] text-emerald-400/60 font-black uppercase tracking-widest mb-1">
                    Cœur de Connaissances
                  </p>
                  <p className="text-sm text-white font-black">
                    Bible Technique NS 01-001 & NF C18-510
                  </p>
                </div>
                <div className="px-4 py-1.5 bg-emerald-500 text-[#020617] rounded-xl text-[10px] font-black shadow-[0_0_15px_rgba(16,185,129,0.3)]">
                  ACTIF
                </div>
              </div>
            </motion.div>
            ...
            >
              <div className="bg-gradient-to-br from-slate-900 to-slate-950 border border-slate-800 rounded-3xl p-6 shadow-xl">
                <div className="flex items-center justify-between mb-6">
                  ...
                  <button
                    onClick={checkAllVps}
                    className="w-10 h-10 flex items-center justify-center bg-slate-800 hover:bg-slate-700 rounded-2xl transition-all text-blue-400 border border-slate-700 active:rotate-180 duration-500"
                  >
                    <RefreshCw
                      size={20}
                      className={vpsStatus.ollama?.status === 'checking' ? 'animate-spin' : ''}
                    />
                  </button>
                </div>
                ...
                <div className="grid grid-cols-1 gap-3">
                  {[
                    ...
                    },
                    {
                      ...
                      docHint:
                        'VS Code accessible depuis le navigateur pour éditer le code sur le VPS directement.',
                    },
                  ].map((service) => {
                      ...
                      >
                        ...
                        <div className="flex items-center gap-4 mb-3">
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <p className="text-[10px] text-slate-500 font-bold mt-0.5">
                              Port {service.port} • {service.role}
                            </p>
                          </div>
                        </div>
                        ...
                        {isErr && info.command && (
                          <div className="mt-2 flex items-center gap-2 bg-slate-950/80 border border-slate-700/50 rounded-xl px-3 py-2">
                            <span className="text-slate-500 text-[9px] font-black uppercase tracking-widest flex-shrink-0">
                              CMD ›
                            </span>
                            <code className="text-amber-400 text-[10px] font-mono font-bold flex-1 truncate">
                              {info.command}
                            </code>
                          </div>
                        )}
                        ...
                        {info.status === 'idle' && (
                          <p className="text-[10px] text-slate-600 mt-2 italic font-bold">
                            {service.docHint}
                          </p>
                        )}
                      </div>
                ...
                <div className="bg-slate-900/40 border border-slate-800 rounded-3xl p-6">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {vpsModels.map((m) => (
                      <span
                        key={m}
                        className="px-3 py-1.5 bg-blue-600/10 border border-blue-500/20 text-blue-400 rounded-xl text-[10px] font-mono font-bold hover:bg-blue-600/20 transition-colors cursor-default"
                      >
                        {m}
                      </span>
                    ))}
              ...
              <div className="space-y-6">
                  ...
                  <div className="flex justify-between items-end mb-5">
                    <div>
                      ...
                      <p className="text-[10px] text-slate-500 font-bold mt-1">
                        Niveau d'exigence minimal pour les réponses IA.
                      </p>
                    </div>
                ...
                <div className="p-6 bg-slate-900/60 border border-slate-800 rounded-3xl shadow-inner group">
                  <div className="flex justify-between items-end mb-5">
                    <div>
                      ...
                      <p className="text-[10px] text-slate-500 font-bold mt-1">
                        Contexte historique (nombre d'échanges conservés).
                      </p>
                    </div>
                ...
                <label className="flex items-center justify-between p-6 bg-slate-900/60 border border-slate-800 rounded-3xl cursor-pointer hover:border-slate-700 transition-all group">
                  <div className="flex items-center gap-4">
                    <div
                      className={`p-3 rounded-xl transition-all ${config.enableConversationMemory ? 'bg-purple-600/20 text-purple-400' : 'bg-slate-800 text-slate-600'}`}
                    >
                      <RefreshCw
                        size={20}
                        className={config.enableConversationMemory ? 'animate-spin-slow' : ''}
                      />
                    </div>
                    <div>
                      ...
                      <p className="text-[10px] text-slate-500 font-bold">
                        Active la mémoire sémantique multi-tours.
                      </p>
                    </div>
                  </div>
                  <div
                    ...
                    onClick={() =>
                      update({ enableConversationMemory: !config.enableConversationMemory })
                    }
                  >
                    <div
                      className={`absolute top-1 w-4 h-4 rounded-full bg-white transition-all ${config.enableConversationMemory ? 'left-7' : 'left-1'} shadow-md`}
                    />
                  </div>
                </label>
              </div>
        ...
        {config.lastUpdatedBy && (
          <div className="p-4 bg-slate-900/60 rounded-2xl text-center border border-slate-800/60 backdrop-blur-sm shadow-inner">
            <p className="text-[10px] text-slate-500 uppercase font-black tracking-[0.2em] flex items-center justify-center gap-2">
              ...
              <span className="text-blue-400 bg-blue-400/10 px-2 py-0.5 rounded-md border border-blue-400/20">
                {config.lastUpdatedBy}
              </span>
        ...
        {/* High-End Actions */}
        <div
          className={`flex gap-5 pt-4 sticky bottom-0 bg-gradient-to-t from-slate-950 via-slate-950/90 to-transparent pb-2 ${standalone ? 'mt-6' : ''}`}
        >
          <button
            onClick={handleReset}
