...
import { Send, Bot, Sparkles, X, Info, ShieldAlert, Heart, Mic, Camera, Volume2, VolumeX } from 'lucide-react';
...
export const MissionMentor: React.FC<MissionMentorProps> = ({ stats, auditLogs, households }) => {
    ...
    useEffect(() => {
        if (isOpen && history.length === 0 && user && (stats || auditLogs.length > 0)) {
            const checkProactive = async () => {
                setIsThinking(true);
                try {
                    const msg = await missionSageService.getProactiveMessage(user, { stats, auditLogs, households });
    ...
    const handleSend = async () => {
        ...
        setTimeout(async () => {
            try {
                const resp = await missionSageService.processQuery(originalQuery, user, { stats, auditLogs, households });
                
                // J.A.R.V.I.S : Génération dynamique de Smart Replies contextuelles
                if (!resp.smartReplies) {
                     if (resp.message.toLowerCase().includes("tech") || resp.message.toLowerCase().includes("norme")) {
                         resp.smartReplies = ["Voir les Anomalies MFR", "Comment dénuder un câble ?", "Aide Vision AI"];
                ...
                if (!isMuted && 'speechSynthesis' in window) {
                    ...
                    const pureText = resp.message.replace(/[\u{1F600}-\u{1F6FF}|[\u{2600}-\u{26FF}]|\*/gu, '');
                ...
                setHistory(prev => [...prev, { 
                    message: "Désolé, une erreur technique a empêché la réponse. Vérifiez votre connexion ou la configuration de l'IA.", 
                    type: 'error' 
                } as AIResponse]);
            } finally {
                setIsThinking(false);
    ...
    const handleCameraUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
        if (e.target.files && e.target.files[0]) {
            ...
            reader.onload = async (event) => {
                ...
                try {
                    const resp = await missionSageService.processQuery("Analyse cette photo pour audit technique.", user!, { stats, auditLogs, households }, base64);
                    setHistory(prev => [...prev, resp]);

                    if (!isMuted && 'speechSynthesis' in window) {
                        window.speechSynthesis.cancel();
                        const utterance = new SpeechSynthesisUtterance("Analyse terminée. J'ai détecté des points d'attention sur votre installation.");
                        utterance.lang = 'fr-FR';
                        window.speechSynthesis.speak(utterance);
                    }
                } catch (err) {
    ...
    const toggleListening = () => {
        if (!('webkitSpeechRecognition' in window) && !('SpeechRecognition' in window)) {
            alert("La reconnaissance vocale n'est pas supportée sur ce navigateur. Veuillez utiliser Chrome ou Edge.");
            return;
        }
                    ...
                    >
                        ...
                        <div className="bg-slate-900 border-b border-white/5 p-6 flex items-center justify-between">
                            <div className="flex items-center gap-4">
                                ...
                                <div>
                                    <h2 className="text-[11px] font-black tracking-[0.3em] text-white uppercase italic leading-none">GEM-MINT</h2>
                                    <p className="text-[8px] font-black text-blue-500/60 uppercase tracking-widest mt-1">Mentor Sage PROQUELEC</p>
                                </div>
                            </div>
                            ...
                            {history.length === 0 && (
                                <div className="text-center py-20 opacity-30 italic px-10">
                                    ...
                                    <p className="text-xs font-black uppercase tracking-widest leading-loose">
                                        "Posez-moi une question sur vos missions, le terrain ou l'audit, et je vous guiderai vers l'excellence."
                                    </p>
                                </div>
                            )}
                            ...
                            {history.map((resp, i) => (
                                <motion.div 
                                    ...
                                    className={`p-5 rounded-[2rem] border relative overflow-hidden backdrop-blur-xl ${
                                        resp.type === 'user' ? 'bg-slate-800/80 border-slate-700/50 text-slate-200 ml-12 rounded-tr-none' :
                                        resp.type === 'warning' ? 'bg-amber-500/10 border-amber-500/20 text-amber-100 mr-12 rounded-tl-none' :
                                        resp.type === 'error' ? 'bg-rose-500/10 border-rose-500/20 text-rose-100 mr-12 rounded-tl-none' :
                                    ...
                                    <div className="flex gap-4 items-start">
                                        {resp.type !== 'user' && (
                                            <div className={`p-2 rounded-xl border ${
                                                resp.type === 'warning' ? 'bg-amber-500/20 border-amber-500/30 text-amber-400' :
                                                resp.type === 'error' ? 'bg-rose-500/20 border-rose-500/30 text-rose-400' :
                                                'bg-blue-600/20 border-blue-600/30 text-blue-400'
                                            }`}>
                                                {resp.type === 'warning' ? <ShieldAlert size={16}/> : 
                                                 resp.type === 'error' ? <X size={16}/> : <Info size={16}/>}
                                            </div>
                                        )}
                                        <div className="flex-1 space-y-4">
                                            <p className={`text-[13px] font-bold leading-relaxed whitespace-pre-wrap ${resp.type === 'user' ? 'italic' : ''}`}>{resp.message}</p>
                                            
                                            {resp.images && resp.images.length > 0 && (
                                                <div className="grid gap-4 mt-2">
                                                    {resp.images.map((img, idx) => (
                                                        <div key={idx} className="rounded-xl overflow-hidden border border-white/10 bg-black/50 p-1">
                                                            <img src={img.url} alt={img.caption} className="w-full object-contain rounded-lg max-h-64" />
                                                            <p className="text-[10px] text-center text-slate-400 mt-2 italic px-2">{img.caption}</p>
                                                        </div>
                                                    ))}
                                                </div>
                                            )}
                                            ...
                                            {resp.actionType === 'download_report' ? (
                                                <button 
                                                    onClick={async () => {
                                                        ...
                                                        await wordReportService.generateStrategicReport(stats, insights, households);
                                                    }}
