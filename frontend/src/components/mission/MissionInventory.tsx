...
export function MissionInventory({ inventory = [], onChange }: MissionInventoryProps) {
    ...
    const handleUpdateQuantity = (id: string, field: 'quantityTaken' | 'quantityReturned', value: number) => {
        const newInventory = activeInventory.map(item => 
                        ...
                        <div key={item.id} className={`grid grid-cols-12 gap-4 items-center p-4 rounded-2xl border transition-all ${
                            ...
                            <div className="col-span-6 flex items-center gap-3">
                                ...
                                <input 
                                    ...
                                    value={item.label}
                                    onChange={e => {
                                        const newInv = activeInventory.map(i => i.id === item.id ? { ...i, label: e.target.value } : i);
                                        onChange(newInv);
                                    }}
                            ...
                            <div className="col-span-2 flex justify-center">
                                <button 
                                    ...
                                    className={`w-6 h-6 rounded-lg flex items-center justify-center transition-all ${
                                        item.isChecked ? 'bg-indigo-500 text-white shadow-lg shadow-indigo-500/20' : 'bg-slate-200 dark:bg-slate-700 text-transparent'
                                    }`}
                                >
