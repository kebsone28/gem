            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 pb-6 border-b border-white/5">
                <div className="flex items-center gap-5">
                    ...
                    <div>
                        ...
                        <p className="text-[10px] font-bold text-slate-500 uppercase tracking-[0.3em]">Isolation totale par espace de travail</p>
                    </div>
                </div>
                ...
                <div className="lg:col-span-2 space-y-8">
                    <div className="bg-slate-900/40 backdrop-blur-xl p-8 rounded-[2.5rem] border border-white/5 relative overflow-hidden group">
                        ...
                        <div className="relative z-10 space-y-8">
                            <div className="flex items-center gap-4 mb-2">
                                ...
                                <h3 className="text-xs font-black text-indigo-300 uppercase tracking-widest">Configuration des Identifiants (API Token)</h3>
                            </div>

                            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                <div className="space-y-4">
                                    <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-1">Kobo API Token</label>
                                    ...
                                    <p className="text-[9px] text-slate-600 font-medium italic">Ce token permet d'accéder à vos formulaires KoBoToolbox de manière sécurisée.</p>
                                </div>

                                <div className="space-y-4">
                                    <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-1">Kobo Asset UID (Form ID)</label>
                                    <input
                                        type="text"
                                        ...
                                        className="w-full bg-slate-950/50 border border-white/10 rounded-2xl px-6 py-4 text-sm text-indigo-100 placeholder:text-slate-700 focus:border-indigo-500/50 focus:ring-4 focus:ring-indigo-500/10 transition-all outline-none"
                                    />
                                    <p className="text-[9px] text-slate-600 font-medium italic">L'identifiant unique du formulaire source pour ce projet.</p>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="bg-indigo-500/5 border border-indigo-500/10 rounded-[2.5rem] p-8 flex items-start gap-6">
                        ...
                        <div className="space-y-2">
                            <h4 className="text-xs font-black text-white uppercase tracking-widest">Validation de connexion</h4>
                            ...
                            <button 
                                onClick={async () => {
                                    if (!config.token || !config.assetUid) return toast.error("Veuillez remplir les deux champs.");
                                    toast.loading("Test de connexion en cours...", { id: 'kobo-test' });
                                    try {
                                        const res = await apiClient.post('kobo/test-connection', config);
                                        if (res.data.success) {
                                            toast.success(`Connexion réussie ! Formulaire: ${res.data.formName}`, { id: 'kobo-test' });
                                        } else {
                ...
                <div className="space-y-6">
                    <div className="bg-slate-900/40 p-8 rounded-[2.5rem] border border-white/5 space-y-6">
                        <div className="flex items-center gap-3">
                            ...
                            <h3 className="text-xs font-black text-white uppercase tracking-widest">Guide d isolation</h3>
                        </div>
                        <p className="text-[11px] text-slate-400 font-bold leading-relaxed uppercase">
                            Chaque projet (Tenant) possède sa propre clé API. 
                            Cela garantit que les données d'un projet ne sont jamais mélangées avec celles d'un autre.
                        </p>
                        <div className="pt-4 space-y-4">
                            <div className="flex items-start gap-3 text-[10px] font-bold text-slate-500 uppercase italic">
                                ...
                                Ces identifiants sont spécifiques au projet <span className="text-indigo-400">"{project?.name}"</span>.
                            </div>
                            <div className="flex items-start gap-3 text-[10px] font-bold text-slate-500 uppercase italic">
                                <ChevronRight size={14} className="text-indigo-600 shrink-0" />
