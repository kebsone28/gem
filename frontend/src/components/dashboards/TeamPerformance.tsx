...
export const TeamPerformance: React.FC<TeamPerformanceProps> = ({ teamStats, productionRates = {} }) => {
    if (!teamStats || teamStats.length === 0) {
        return (
            <div className="bg-white p-8 rounded-[20px] border border-gray-100 shadow-sm text-center">
                <Users size={32} className="mx-auto mb-3 text-gray-200" />
                <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Aucune donnée de performance</p>
            </div>
        );
    }
        ...
        <div className="bg-white p-8 rounded-[20px] border border-gray-100 shadow-sm space-y-8">
            ...
            <div className="grid grid-cols-1 gap-6">
                {teamStats.map((team, idx) => {
                    ...
                    const efficiency = target > 0 ? Math.min(100, Math.round((team.yield / target) * 100)) : 0;
                        ...
                        <div key={idx} className="group relative">
                            <div className="flex justify-between items-end mb-2">
                                ...
                                <div className="text-right">
                                    <span className={`text-lg font-black tracking-tighter ${efficiency >= 100 ? 'text-emerald-500' : efficiency >= 70 ? 'text-blue-500' : 'text-rose-500'}`}>
                                        {efficiency}%
                                    </span>
                                    <p className="text-[9px] font-bold text-gray-400 uppercase">Efficacité</p>
                                </div>
                            </div>
                            ...
                            <div className="h-3 w-full bg-gray-50 rounded-full overflow-hidden p-0.5 border border-gray-100">
                                <motion.div
                                    ...
                                    className={`h-full rounded-full ${
                                        efficiency >= 100 ? 'bg-emerald-500 shadow-[0_0_10px_rgba(16,185,129,0.3)]' : 
                                        efficiency >= 70 ? 'bg-blue-600 shadow-[0_0_10px_rgba(37,99,235,0.3)]' : 
            ...
            <div className="pt-4 grid grid-cols-2 gap-4">
                <div className="bg-gray-50/50 p-4 rounded-xl border border-gray-100 text-center">
                    <p className="text-2xl font-black text-gray-900">{fmtNum(teamStats.reduce((acc, t) => acc + t.done, 0))}</p>
                    <p className="text-[9px] font-bold text-gray-400 uppercase tracking-widest">Total Réalisé</p>
                </div>
                <div className="bg-gray-50/50 p-4 rounded-xl border border-gray-100 text-center">
                    <p className="text-2xl font-black text-gray-900">
                        {Math.round(teamStats.reduce((acc, t) => acc + t.yield, 0) / teamStats.length * 10) / 10}
                    </p>
                    <p className="text-[9px] font-bold text-gray-400 uppercase tracking-widest">Moyenne Rendement</p>
                </div>
            </div>
        </div>
    );
