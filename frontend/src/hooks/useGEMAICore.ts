/**
 * 🪝 useGEMAICore - Hook React pour le cerveau IA centralisé
 * Fournit une interface facile pour utiliser GEMAICore dans les composants React
 */
...
import {
  getGEMAICore,
  type GEMAICoreConfig,
  type GEMAIRequest,
  type GEMAIResponse,
  type GEMAIState,
  type TrainingSuggestion,
  type LearningMetric,
} from '../services/ai/GEMAICore';

export function useGEMAICore(config?: Partial<GEMAICoreConfig>) {
  const { user } = useAuth();
  const coreRef = useRef(getGEMAICore(config));
  const [state, setState] = useState<GEMAIState>(coreRef.current.getState());
  ...
  const processRequest = useCallback(
    async (query: string, context?: AIState, options?: GEMAIRequest['options']): Promise<GEMAIResponse> => {
      const request: GEMAIRequest = {
        query,
        user: user || null,
        context,
        options,
  ...
  const updateConfig = useCallback((newConfig: Partial<GEMAICoreConfig>): void => {
...
export function useGEMAIChat(context?: AIState) {
  const {
    processRequest,
    recordFeedback,
    isThinking,
  ...
  const sendMessage = useCallback(
    async (query: string, options?: GEMAIRequest['options']) => {
...
export function useGEMAITraining() {
  const {
    learningStatus,
    trainingSuggestions,
    generateTrainingSuggestions,
