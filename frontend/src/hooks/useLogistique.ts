...
export function useLogistique() {
    ...
    useEffect(() => {
        ...
        const fetchDbTeams = async () => {
            try {
                ...
                // Ensure teams have a region slug for matching with warehouses
                const mappedTeams = (response.data.teams || [])
                    .filter((t: any) => !t.status || t.status === 'active') // Status Filter
                    .map((t: any) => ({
                        ...t,
                        // If team has a region object from DB, use its name as slug
    ...
    const warehouses: any[] = useMemo(() => {
        ...
        const regions = Array.from(new Set(allGrappes.map((g: any) => g.region).filter(Boolean))) as string[];
        return regions.map(region => {
            const regionGrappes = allGrappes.filter((g: any) => g.region === region);
            const avgLat = regionGrappes.reduce((s: number, g: any) => s + (g.centroide_lat || 0), 0) / (regionGrappes.length || 1);
            const avgLng = regionGrappes.reduce((s: number, g: any) => s + (g.centroide_lon || 0), 0) / (regionGrappes.length || 1);
            return {
    ...
    const warehouseStats = useMemo(() => {
        ...
        return warehouses.map(wh => {
            ...
            const regionHouseholds = households?.filter(h => {
            ...
            const alerts = stockRealtime.filter((item: any) =>
                teamVelocity > 0 && item.remaining < item.qty * teamVelocity * 3
            );

            const kitsLoadedAllTime = (wh.preparatorTeams || []).reduce((sum: number, team: any) =>
                sum + (team.loadings || []).reduce((s: number, l: any) => s + (l.kitsLoaded || 0), 0), 0);
            ...
            return {
                ...
                kitsLoadedAllTime,
                daysBeforeBreakout: teamVelocity > 0 ? Math.round(kitsLoadedToday / teamVelocity) : Infinity,
            };
        });
    ...
    const logMovement = async (type: 'ENTRY' | 'EXIT' | 'TRANSFER', details: any) => {
        ...
        try { await apiClient.patch(`/projects/${project.id}`, { config: newConfig }); } catch(e) {}
    };
    ...
    const computeCompleteness = (assignments?: Record<string, string[]>) => {
        ...
        const trades = Array.from(new Set(teams.map(t => (t as any).type || t.tradeKey).filter(Boolean)));
    ...
    const addWarehouse = async (name: string, region: string) => {
        const newWh = {
            ...
            preparatorTeams: [], stockOverrides: {},
        };
        await _saveWarehouseConfig([...warehouses, newWh]);
    };

    const addPreparatorLoading = async (warehouseId: string, teamId: string, teamName: string, kitsLoaded: number, variantId: string = 'standard') => {
        const whStatus = warehouseStats.find(w => w.id === warehouseId);
        if (whStatus && whStatus.kitsLoadedToday + kitsLoaded > whStatus.kitsLoadedToday + 500) { // arbitrary logical cap or real stock check
    ...
    const receiveStock = async (warehouseId: string, kitsCount: number, source: string) => {
        ...
        const updated = warehouses.map(wh => {
            ...
            if (sysIdx >= 0) {
                teams[sysIdx] = { ...teams[sysIdx], loadings: [...(teams[sysIdx].loadings || []), loading] };
            } else {
    ...
    const updateWarehouseCoords = async (warehouseId: string, latitude: number, longitude: number, address: string) => {
        const updated = warehouses.map(wh =>
        ...
        const updated = configured.map(wh => {
            if (wh.id === fromId) {
                ...
                if (teamIdx >= 0) {
                    ...
                    teams.push({ teamId: 'transfer_system', teamName: 'Système (Transfert)', loadings: [loading] });
                }
                return { ...wh, preparatorTeams: teams };
            }
            if (wh.id === toId) {
