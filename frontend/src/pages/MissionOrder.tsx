...
export default function MissionOrder() {
    ...
    } = useMissionSync(state, missionState, activeProjectId);
    ...
    const healthScore = useMemo(() => selectMissionHealthScore(state, projectBudget), [state, projectBudget]);
    const healthStatus = useMemo(() => selectHealthStatus(state, projectBudget), [state, projectBudget]);
    const budgetVariance = useMemo(() => selectBudgetVariance(state, projectBudget), [state, projectBudget]);
    ...
    const handleDuplicate = async () => {
        ...
        const { 
            id, projectId, status, overallStatus, workflow, 
            createdAt, updatedAt, isSubmitted, isCertified, 
            ...cleanFormData 
    ...
    const handleMemberUpdate = useCallback((i: number, field: string | number | symbol, value: any) => {
    ...
    const handleDeleteMission = async (id: string, orderNumber: string) => {
        if (!window.confirm(`Êtes-vous sûr de vouloir supprimer la mission ${orderNumber} ? Cette action est irréversible.`)) return;
    ...
    const handleToggleFeature = (feature: string) => {
        const currentFeatures = state.formData.features || { map: true, expenses: false, inventory: false, ai: false };
    ...
    const handleExportExcel = () => {
        ...
        const infoRows = [
            ...
            ['Itinéraire Aller', (data.itineraryAller || '').replace(/&/g, '').replace(/!'/g, '->').replace(/→/g, '->')],
            ['Itinéraire Retour', (data.itineraryRetour || '').replace(/&/g, '').replace(/!'/g, '->').replace(/→/g, '->')],
            ['Date de départ', data.startDate || ''],
            ['Date de retour prévue', data.endDate || ''],
            [],
            ['COMPOSITION DE L\'ÉQUIPE'],
            ['N°', 'Nom & Prénom', 'Fonction / Rôle', 'Unité', 'Ind. Journalière (FCFA)', 'Nb Jours', 'Total Indemnité (FCFA)'],
            ...state.members.map((m, i) => [
                i + 1, m.name, m.role, m.unit || '',
                m.dailyIndemnity, m.days, m.dailyIndemnity * m.days
            ]),
            [],
            ['', '', '', '', '', 'TOTAL',
        ...
        // Largeurs colonnes
        ws1['!cols'] = [{ wch: 28 }, { wch: 40 }, { wch: 22 }, { wch: 12 }, { wch: 24 }, { wch: 10 }, { wch: 24 }];
    ...
    const handleMissionSubmit = async () => {
        ...
        try {
            ...
            if (syncResult?.serverSuccess === true || navigator.onLine === false) {
                ...
                missionState.addAuditEntry("Mission soumise pour approbation (Workflow activé)", user?.name || "Utilisateur");
    ...
    const handleMissionCertify = () => {
        ...
        if (workflow && typeof workflow.currentStep === 'number' && workflow.currentStep < 3) {
            toast.error("Toutes les étapes d'approbation précédentes doivent être validées avant la certification DG.");
            return;
        }
                    ...
                    <div className="lg:col-span-9">
                        <div className="grid grid-cols-1 xl:grid-cols-12 gap-8 items-start">
                            ...
                            <div className="xl:col-span-8 space-y-8">
                                ...
                                {state.formData.features?.map && (
                                    <section className="glass-card !p-0 !rounded-[2.5rem] overflow-hidden border-2 border-indigo-500/10 shadow-2xl shadow-indigo-500/5">
                                        <div className="p-4 bg-slate-900 border-b border-white/5 flex justify-between items-center">
                                            <h2 className="!text-[9px] font-black uppercase tracking-widest text-indigo-400">Preview SIG Intelligente</h2>
                                ...
                                <MissionItineraryEditor
                                    ...
                                    onRemoveStep={(i: number) => {
                                        const newPlanning = (state.formData.planning || []).filter((_, idx) => idx !== i);
                                        missionState.updateFormField('planning', newPlanning);
                                    }}
                                />
                            ...
                            <div className="xl:col-span-4 space-y-8">
                                <MissionBudgetPanel totalFrais={totalFrais} projectBudget={projectBudget} members={state.members} />
                                <MissionStatusWidget 
                                    data={state.formData}
                                    members={state.members}
                                    isCertified={state.isCertified}
                ...
                <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm transition-opacity">
                    <div className="bg-white dark:bg-slate-900 rounded-3xl shadow-2xl p-8 max-w-md w-full animate-in zoom-in-95 duration-200 border border-slate-200 dark:border-slate-700">
                        <div className="flex flex-col items-center text-center">
                            ...
                            <h3 className="text-2xl font-black text-slate-900 dark:text-white mb-2">Signature Officielle</h3>
                            <p className="text-sm font-medium text-slate-500 mb-8 max-w-[280px]">
                                Veuillez saisir votre code PIN de Directeur Général pour certifier et générer cet ordre de mission.
                            </p>
