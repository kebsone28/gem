            ...
            <div className="max-w-7xl mx-auto space-y-8">
                ...
                <header className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <h1 className="text-4xl font-black text-white tracking-tight italic uppercase">Moteur d'Optimisation</h1>
                            <p className="text-indigo-300/70 font-medium">Réduisez les coûts de revient et maximisez la marge nette à l'aide de l'IA.</p>
                        </div>
                    </div>

                    <button
                        onClick={handleOptimize}
                        className="flex items-center gap-3 px-8 py-4 bg-gradient-to-r from-emerald-500 to-emerald-700 hover:from-emerald-400 hover:to-emerald-600 text-white font-black rounded-2xl transition-all shadow-xl shadow-emerald-600/20 active:scale-95 group"
                    >
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                    ...
                    <aside className="lg:col-span-4 space-y-6">
                        <div className="bg-slate-900/80 rounded-[2.5rem] border border-slate-800 p-8 space-y-8 backdrop-blur-xl shadow-2xl">
                            <h3 className="text-sm font-black text-slate-500 uppercase tracking-[0.2em] flex items-center justify-between">
                                Paramètres Actuels
                                {isOptimized && (
                            ...
                            <div className="space-y-6">
                                ...
                                <div className="space-y-4">
                                    <label className="text-xs font-black text-white uppercase tracking-widest block">Effectifs & Rémunérations</label>
                                    {Object.entries(activeConfigs).map(([role, config]) => {
                                        ...
                                        return (
                                            <div key={role} className={`flex flex-col gap-3 p-4 rounded-2xl border transition-all ${isOptimized ? 'bg-indigo-950/40 border-indigo-500/30' : 'bg-slate-950/50 border-slate-800/50'}`}>

                                                <div className="flex items-center justify-between">
                                                    <div className="flex flex-col">
                                                        <span className="text-slate-200 font-bold">{ROLE_LABELS[role as keyof typeof ROLE_LABELS]}</span>
                                                        <span className="text-[10px] text-slate-500 font-black tracking-widest uppercase">{ROLE_CAPACITY[role as keyof typeof ROLE_CAPACITY]}/j</span>
                                                    </div>
                                                    <div className="flex items-center gap-3">
                                                        {!isOptimized && (
                                                            <button title="Retirer" onClick={() => updateTeamConfig(role, 'count', Math.max(1, config.count - 1))} className="w-8 h-8 rounded-xl bg-slate-900 border border-slate-800 flex items-center justify-center text-white hover:bg-slate-800 transition-colors">-</button>
                                                        )}
                                                        <span className={`text-xl font-black w-6 text-center ${isOptimized ? 'text-emerald-400' : 'text-white'}`}>{config.count}</span>
                                                        {!isOptimized && (
                                                            <button title="Ajouter" onClick={() => updateTeamConfig(role, 'count', config.count + 1)} className="w-8 h-8 rounded-xl bg-slate-900 border border-slate-800 flex items-center justify-center text-white hover:bg-slate-800 transition-colors">+</button>
                                                        )}
                                                        {isOptimized && diff !== 0 && (
                                                ...
                                                <div className="grid grid-cols-2 gap-2 mt-2 pt-3 border-t border-slate-800/50">
                                                    <div className="flex flex-col gap-1">
                                                        <label className="text-[9px] text-slate-500 uppercase tracking-widest font-black">Paiement</label>
                                                        <select
                                                            title="Mode de paiement"
                                                            disabled={isOptimized}
                                                            value={config.paymentMode}
                                                            onChange={(e) => updateTeamConfig(role, 'paymentMode', e.target.value)}
                                                            className="bg-slate-900 border border-slate-800 rounded-lg p-1.5 text-xs text-white focus:outline-none focus:border-indigo-500"
                                                        >
                                                    ...
                                                    <div className="flex flex-col gap-1">
                                                        <label className="text-[9px] text-slate-500 uppercase tracking-widest font-black">Tarif ({config.paymentMode === 'task' ? '/Foyer' : '/Jour'})</label>
                                                        <input
                                                            title="Tarif"
                                                            disabled={isOptimized}
                                                            type="number"
                                                    ...
                                                    <div className="flex flex-col gap-1 col-span-2">
                                                        <label className="text-[9px] text-slate-500 uppercase tracking-widest font-black flex items-center justify-between">
                                ...
                                <div className="space-y-4 pt-6 border-t border-slate-800/50">
                                    ...
                                    {/* Weather Toggle */}
                                    <div className="flex flex-col gap-3 p-4 rounded-xl bg-slate-900/50 border border-slate-800">
                                        <div className="flex items-center justify-between">
                                            <div className="flex flex-col">
                                                <span className="text-white text-sm font-bold">Saison des Pluies (Hivernage)</span>
                                                <span className="text-[10px] text-slate-500">Baisse des cadences Maçons/Réseau</span>
                                            </div>
                                            <button
                                        ...
                                        <AnimatePresence>
                                            {isHivernage && (
                                                ...
                                                >
                                                    <div className="space-y-4 pt-3 mt-1 border-t border-slate-800/50">
                                                        <div className="space-y-2">
                                                            <div className="flex justify-between text-xs">
                                                                <span className="text-slate-400">Pénalité Maçons</span>
                                                        ...
                                                        <div className="space-y-2">
                                                            <div className="flex justify-between text-xs">
                                                                <span className="text-slate-400">Pénalité Réseau</span>
                                    ...
                                    <div className="space-y-2 p-4 rounded-xl bg-slate-900/50 border border-slate-800">
                                        <div className="flex justify-between text-sm">
                                            <label htmlFor="reject-rate" className="text-slate-400">Taux de Rejet & Reprises</label>
                                            <span className="text-white font-bold">{rejectRate}%</span>
                                        </div>
                                        <input
                                            id="reject-rate"
                                            title="Taux de rejet"
                                            ...
                                            className="w-full accent-rose-500"
                                        />
                                        <div className="text-[10px] text-slate-500 text-center">Affecte le volume de travail Réseau/Intérieur</div>
                                    </div>
                                </div>
                                ...
                                <div className="space-y-4 pt-6 border-t border-slate-800/50">
                                    ...
                                    <div className="space-y-2 p-4 rounded-xl bg-slate-900/50 border border-slate-800">
                                        <div className="flex justify-between text-sm">
                                            <label htmlFor="acompte-rate" className="text-slate-400">Acompte à la commande</label>
                                            <span className="text-white font-bold">{acompteRate}%</span>
                                        </div>
                                        <input
                                            id="acompte-rate"
                                        ...
                                        />
                                        <div className="text-[10px] text-slate-500 text-center">Fonds disponibles avant acompte suivant</div>
                                    </div>
                                </div>
                                ...
                                <div className="space-y-4 pt-4 border-t border-slate-800">
                                    <div className="flex justify-between items-center">
                                        <label className="text-xs font-black text-white uppercase tracking-widest block" title="Véhicules mutualisés (Base logistique)">Logistique (Base)</label>
                                ...
                                <div className="space-y-4 pt-4 border-t border-slate-800">
                                    <div className="flex justify-between items-center">
                                        <label className="text-xs font-black text-white uppercase tracking-widest block">Taux d'aléas estimé</label>
                                ...
                                <div className="space-y-4 pt-6 border-t border-slate-800/50">
                                    ...
                                    <div className="grid grid-cols-2 gap-4">
                                        <div className="flex flex-col gap-2 p-3 rounded-xl bg-slate-900/50 border border-slate-800">
                                            <label htmlFor="work-days" className="text-[10px] text-slate-400 uppercase tracking-widest font-bold">Jours/Semaine</label>
                                            <select
                                                id="work-days"
                                                title="Jours travaillés par semaine"
                                                value={workDaysPerWeek}
                                                onChange={(e) => setWorkDaysPerWeek(parseInt(e.target.value))}
                                                className="bg-slate-950 border border-slate-800 rounded-lg p-2 text-sm text-white focus:outline-none focus:border-indigo-500"
                                            >
                                        ...
                                        <div className="flex flex-col gap-2 p-3 rounded-xl bg-slate-900/50 border border-slate-800">
                                            <label htmlFor="holidays" className="text-[10px] text-slate-400 uppercase tracking-widest font-bold">Jours Fériés</label>
                                            <input
                                                id="holidays"
                                                ...
                                                className="bg-slate-950 border border-slate-800 rounded-lg p-2 text-sm text-white font-mono focus:outline-none focus:border-indigo-500"
                                            />
                                        </div>
                                    </div>
                                    <div className="text-[10px] text-slate-500 text-center">Affecte la durée calendaire et le coût logistique</div>
                                </div>
                            </div>
                        </div>
                    </aside>
                    ...
                    <main className="lg:col-span-8 space-y-8">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                            <AnimatePresence mode="popLayout">
                                <motion.div
                                    ...
                                    className={`p-8 rounded-[2rem] border-t-4 shadow-xl ${isOptimized ? 'bg-emerald-950/20 border-emerald-500 border-x border-b border-x-emerald-500/20 border-b-emerald-500/20' : 'bg-slate-900/50 border-blue-500 border-x border-b border-slate-800'}`}
                                >
                                    <div className="flex items-center gap-4 mb-4">
                                        ...
                                        <span className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Durée Globale</span>
                                    </div>
                                    <div className="flex flex-col gap-1">
                                        <div className="flex items-end gap-3">
                                            <p className="text-4xl font-black text-white">{activeScenario.calendarDuration} <span className="text-lg text-slate-500 font-bold">Jours</span></p>
                                ...
                                <motion.div
                                    ...
                                    className={`p-8 rounded-[2rem] border-t-4 shadow-xl ${isOptimized ? 'bg-emerald-950/20 border-emerald-500 border-x border-b border-x-emerald-500/20 border-b-emerald-500/20' : 'bg-slate-900/50 border-indigo-500 border-x border-b border-slate-800'}`}
                                >
                                    <div className="flex items-center gap-4 mb-4">
                                        ...
                                        <span className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Coût de Revient</span>
                                    </div>
                                    <p className="text-3xl font-black text-white truncate">{formatFCFA(activeScenario.cost)}</p>
                                    <p className="text-[11px] font-bold text-slate-400 mt-2">Vs Devis: {formatFCFA(devis.totalPlanned)}</p>
                                </motion.div>
                                ...
                                <motion.div
                                    ...
                                    className={`p-8 rounded-[2rem] border-t-4 shadow-xl ${activeScenario.margin >= 0 ? 'bg-slate-900/50 border-emerald-500 border-x border-b border-slate-800' : 'bg-rose-950/20 border-rose-500 border-x border-b border-rose-500/20'}`}
                                >
                                    <div className="flex items-center gap-4 mb-4">
                                        ...
                                        <span className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Marge Simulée</span>
                                    </div>
                                    <p className={`text-3xl font-black truncate ${activeScenario.margin >= 0 ? 'text-emerald-400' : 'text-rose-400'}`}>
                                        {formatFCFA(activeScenario.margin)}
                                    </p>
                                    {isOptimized && activeScenario.margin > currentScenario.margin && (
                                        <p className="text-[11px] font-black text-emerald-500 mt-2 flex items-center gap-1">
                                            <TrendingUp size={12} />
                                            +{formatFCFA(activeScenario.margin - currentScenario.margin)} optimisés !
                                        </p>
                                    )}
                                </motion.div>
                            </AnimatePresence>
                        ...
                        {/* Cashflow Display */}
                        <div className="bg-slate-900/80 rounded-[3rem] p-10 border border-slate-800 shadow-2xl space-y-8">
                            <div>
                                <h3 className="text-2xl font-black text-white italic uppercase tracking-tighter">Profil de Trésorerie</h3>
                                <p className="text-sm font-medium text-slate-400 mt-1">Risque de rupture de liquidité avant la fin du projet.</p>
                            </div>

                            <div className="space-y-4">
                                <div className="flex justify-between items-center bg-slate-950/50 p-4 rounded-2xl border border-slate-800">
                                    <span className="text-slate-400">Acompte perçu ({acompteRate}%)</span>
                                    <span className="text-white font-mono font-bold">{formatFCFA(activeScenario.initialCash)}</span>
                                </div>
                                <div className="flex justify-between items-center bg-slate-950/50 p-4 rounded-2xl border border-slate-800">
                                    <span className="text-slate-400">Besoin FdR (Salaires + Logistique)</span>
                                    <span className="text-rose-400 font-mono font-bold">{formatFCFA(activeScenario.maxOutflow)}</span>
                                </div>
                                ...
                                {activeScenario.hasCashflowRisk ? (
                                    <div className="bg-rose-500/10 border border-rose-500/20 p-4 rounded-xl flex items-start gap-4">
                                        ...
                                        <div>
                                            ...
                                            <p className="text-rose-400/80 text-xs mt-1">Le paiement des équipes et des locations de véhicules dépassera l'acompte perçu de {formatFCFA(activeScenario.maxOutflow - activeScenario.initialCash)}. Prévoyez un fond de roulement.</p>
                                        </div>
                                    </div>
                                ) : (
                        ...
                        <div className="bg-slate-900/80 rounded-[3rem] p-10 border border-slate-800 shadow-2xl space-y-8">
                            <div>
                                <h3 className="text-2xl font-black text-white italic uppercase tracking-tighter">Planning & Flux</h3>
                                <p className="text-sm font-medium text-slate-400 mt-1">L'IA décale les équipes plus rapides pour éviter l'attente et le surcoût.</p>
                            </div>
                                    ...
                                    return (
                                        <div key={role} className="space-y-2">
                                            <div className="flex justify-between items-end">
                                                <div className="flex items-center gap-3">
                                                    <span className="text-white font-bold">{ROLE_LABELS[role as keyof typeof ROLE_LABELS]}</span>
                                                    {isBottleneck && (
                                                ...
                                                <div className="flex flex-col items-end">
                                                    <span className="text-xs font-black text-slate-500">{config.count * ROLE_CAPACITY[role as keyof typeof ROLE_CAPACITY]} Foyers/j</span>
                                                    <span className="text-[10px] text-indigo-400 font-bold">Jour {sched.start} à {sched.end} ({sched.duration}j)</span>
                                                </div>
                                            </div>
                                            <div className="h-6 w-full bg-slate-900 border border-slate-800 rounded-full overflow-hidden relative">
                                                <motion.div
                                                ...
                                                >
                                                    {sched.duration > 15 && <span className="text-[9px] font-black text-white/80">{sched.duration}j</span>}
                                                </motion.div>
                                            </div>
                                        </div>
                                    );
                            ...
                            {activeScenario.bottleneck && (
                                <div className="p-6 bg-indigo-950/30 border border-indigo-500/20 rounded-2xl flex items-start gap-4">
