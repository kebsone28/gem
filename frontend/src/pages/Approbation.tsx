...
export default function Approbation() {
    ...
    const isDirector = roleStr.includes('DG') || roleStr.includes('DIR') || nameStr.includes('DIRECTEUR');
    const isAccountant = roleStr.includes('COMPTABLE') || roleStr.includes('FINANCE');
    const isProjectManager = roleStr.includes('CHEF') || roleStr.includes('PROJECT') || roleStr.includes('CP');
    ...
    const handleDelete = async (e: React.MouseEvent, id: string, title: string) => {
        e.stopPropagation();
        if (!window.confirm(`Êtes-vous sûr de vouloir supprimer définitivement la soumission "${title}" ?`)) return;
    ...
    const handleDecision = async (signatureData?: string) => {
        ...
        try {
            if (decisionType === 'approve') {
                await missionApprovalService.approveMissionStep(selectedMission.id, workflowRole, comment, signatureData);
                ...
                {[
                    ...
                    },
                    {
                        ...
                        value: pendingMissions.filter(m => {
                            const days = m.startDate ? Math.ceil((new Date(m.startDate).getTime() - Date.now()) / 86400000) : 99;
                            return days <= 3;
                        }).length,
                    ...
                    {
                        label: 'Coût Moyen',
                        value: pendingMissions.length > 0
                            ? fmtFCFA(Math.round(pendingMissions.reduce((s, m) => s + (m.budget || 0), 0) / pendingMissions.length))
                            : '—',
                        sub: 'par mission',
                    ...
                    <div key={i} className={`glass-card !p-6 !rounded-3xl border-white/5 flex items-center gap-4`}>
                        ...
                        <div className="min-w-0">
                            <div className="text-[9px] font-black text-slate-500 uppercase tracking-[0.2em] mb-0.5">{kpi.label}</div>
                            <div className="text-lg font-black text-white tracking-tight truncate">{kpi.value}</div>
                            <div className="text-[9px] text-slate-600 font-bold uppercase tracking-widest">{kpi.sub}</div>
                        </div>
                    </div>
                ))}
            </div>
                    ...
                    <div className="flex items-center justify-between px-2">
                        <div className="flex flex-col gap-1">
                            <h3 className="text-xs font-black text-slate-500 uppercase tracking-[0.2em]">
                                {isArchiveMode ? 'Archives Certifiées' : 'Soumissions Reçues'} ({pendingMissions.length})
                            </h3>
                            <button 
                        ...
                        <div className="flex items-center gap-2">
                            {canDelete && !isArchiveMode && (
                                <button 
                                    ...
                                    title={pendingMissions.length > 0 ? "Tout supprimer définitivement" : "La liste est déjà vide"}
                            ...
                            <button title="Actualiser la liste" onClick={fetchPending} className="p-2 text-blue-500 hover:bg-blue-500/10 rounded-xl transition-all">
                    ...
                    <div className="space-y-4 max-h-[70vh] overflow-y-auto pr-2 custom-scrollbar">
                        {pendingMissions.length === 0 ? (
                            <div className="py-20 text-center border-2 border-dashed border-white/5 rounded-[2.5rem] bg-white/2">
                                ...
                                <p className="text-slate-500 font-bold text-xs uppercase italic">Aucune mission {isArchiveMode ? 'archivée' : 'en attente'}</p>
                            </div>
                        ) : (
                                ...
                                <button
                                    ...
                                    className={`w-full text-left p-6 rounded-[2rem] border transition-all duration-300 group
                                        ${selectedMission?.id === m.id 
                                            ? (isArchiveMode ? 'bg-emerald-600 border-emerald-500 shadow-emerald-500/20' : 'bg-blue-600 border-blue-500 shadow-blue-500/20') + ' shadow-xl text-white' 
                                            : 'bg-white/5 border-white/5 text-slate-400 hover:bg-white/10 hover:border-white/10'}`}
                                >
                                    <div className="flex justify-between items-start mb-4">
                                        <div className={`p-2.5 rounded-xl ${selectedMission?.id === m.id ? 'bg-white/20' : (isArchiveMode ? 'bg-emerald-500/10 text-emerald-500' : 'bg-blue-500/10 text-blue-500')}`}>
                                            <FileText size={18} />
                                        </div>
                                        <div className="flex items-center gap-2">
                                            {(() => {
                                                const daysLeft = m.startDate ? Math.ceil((new Date(m.startDate).getTime() - Date.now()) / 86400000) : 99;
                                                return !isArchiveMode && daysLeft <= 3 ? (
                                                    <span className="text-[8px] font-black uppercase tracking-widest px-2 py-0.5 rounded-lg bg-rose-500/20 text-rose-400 border border-rose-500/20">Urgent</span>
                                                ) : null;
                                            })()}
                                            <div className={`text-[10px] font-black uppercase tracking-widest px-2 py-1 rounded-lg ${selectedMission?.id === m.id ? 'bg-white/20 text-white' : (isArchiveMode ? 'bg-emerald-500/10 text-emerald-500' : 'bg-amber-500/10 text-amber-500')}`}>
                                                {isArchiveMode ? (m.orderNumber || 'CERTIFIÉE') : (m.approvalWorkflow?.currentStep ? `Étape ${m.approvalWorkflow.currentStep}` : 'Soumis')}
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <p className={`text-[10px] font-bold mb-4 opacity-60`}>Par {m.user?.name || 'Inconnu'}</p>
                    ...
                    <AnimatePresence mode="wait">
                        {selectedMission ? (
                                ...
                                <div className="glass-card !p-10 !rounded-[3rem] border-white/5 bg-gradient-to-br from-slate-900 to-slate-950 shadow-2xl relative overflow-hidden">
                                    ...
                                    <div className="flex flex-col md:flex-row justify-between items-start gap-8 mb-10 relative z-10">
                                        <div className="flex-1">
                                            <div className="flex items-center gap-3 mb-4">
                                                ...
                                                <span className="text-[10px] font-black text-emerald-500 uppercase tracking-[0.3em]">Stratégie & Objectifs</span>
                                            </div>
                                        ...
                                        <div className="w-full md:w-auto space-y-4">
                                            <div className="p-6 rounded-3xl bg-emerald-500/5 border border-emerald-500/10 text-right min-w-[240px]">
                                                <span className="text-[10px] font-black text-emerald-500/60 uppercase tracking-widest block mb-2">Budget Prévisionnel</span>
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-10 relative z-10">
                                        <div className="space-y-6">
                                            <div className="flex items-center gap-3 mb-2">
                                                ...
                                                <h4 className="text-xs font-black text-white uppercase tracking-widest">Composition de l'équipe</h4>
                                            </div>
                                                    ...
                                                    return (
                                                        <div key={i} className="flex items-center gap-3 p-4 bg-white/2 rounded-2xl border border-white/5">
                                                            ...
                                                            <div className="flex-1 min-w-0">
                                                                <div className="text-[11px] font-black text-white truncate">{m.name}</div>
                                                                <div className="text-[9px] font-bold text-slate-500 uppercase tracking-widest">{m.role} · {m.days || 1}j</div>
                                                            </div>
                                                            <div className="text-right flex-shrink-0">
                                                                <div className="text-[10px] font-black text-emerald-400">{fmtFCFA(memberTotal)}</div>
                                                                <div className="text-[8px] text-slate-600">{fmtFCFA(m.dailyIndemnity || 0)}/j</div>
                                                            </div>
                                                        </div>
                                                    );
                                                })}
                                                {(selectedMission.data?.members && selectedMission.data.members.length > 0) && (
                                                    <div className="flex items-center justify-between p-3 bg-emerald-500/5 border border-emerald-500/10 rounded-2xl mt-2">
                                                        <span className="text-[9px] font-black text-emerald-500/70 uppercase tracking-widest">Total Indemnités</span>
                                                        <span className="text-sm font-black text-emerald-400">
                                                            {fmtFCFA((selectedMission.data.members as any[]).reduce((s: number, m: any) => s + ((m.dailyIndemnity || 0) * (m.days || 1)), 0))}
                                                        </span>
                                                    </div>
                                                )}
                                                {(!selectedMission.data?.members || selectedMission.data.members.length === 0) && (
                                                    <p className="text-[10px] text-slate-600 italic">Aucun missionnaire listé</p>
                                                )}
                                            </div>
                                        </div>
                                        ...
                                        <div className="space-y-6">
                                            <div className="flex items-center gap-3 mb-2">
                                                ...
                                                <h4 className="text-xs font-black text-white uppercase tracking-widest">Calendrier & Itinéraire</h4>
                                            </div>
                                            <div className="p-6 rounded-3xl bg-white/2 border border-white/5 space-y-4">
                                                <div className="flex items-center justify-between">
                                                    <span className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Départ</span>
                                                    <span className="text-xs font-black text-white">{selectedMission.startDate ? new Date(selectedMission.startDate).toLocaleDateString() : 'N/A'}</span>
                                                </div>
                                                <div className="flex items-center justify-between">
                                                    <span className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Retour Prévu</span>
                                                    <span className="text-xs font-black text-white">{selectedMission.endDate ? new Date(selectedMission.endDate).toLocaleDateString() : 'N/A'}</span>
                                                </div>
                                                <div className="pt-2 border-t border-white/5">
                                                    <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-1">Trajet</p>
                                                    <p className="text-xs font-bold text-white italic">{selectedMission.data?.itineraryAller || 'Non spécifié'}</p>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-10 relative z-10">
                                        <div className="space-y-4">
                                            <div className="flex items-center gap-3">
                                                ...
                                                <h4 className="text-[10px] font-black text-slate-500 uppercase tracking-widest leading-none">Objet de la Mission</h4>
                                            </div>
                                        ...
                                        <div className="lg:col-span-2 space-y-4">
                                            <div className="flex items-center gap-3">
                                                ...
                                                <h4 className="text-[10px] font-black text-slate-500 uppercase tracking-widest leading-none">Observations de Terrain</h4>
                                            </div>
                                            <div className="p-6 rounded-3xl bg-white/2 border border-white/5 min-h-[100px]">
                                                <p className="text-xs font-medium text-slate-400 leading-relaxed italic">
                                                    {selectedMission.data?.reportObservations || "Aucune observation particulière rattachée."}
                                                </p>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    {isArchiveMode ? (
                                        <div className="p-10 rounded-[2.5rem] bg-emerald-500/10 border border-emerald-500/20 relative z-10 flex flex-col items-center justify-center text-center">
                                            ...
                                            <h4 className="text-xl font-black text-white uppercase italic tracking-tighter mb-2">Mission Certifiée & Clôturée</h4>
                                            ...
                                            <div className="px-6 py-3 bg-white/5 rounded-2xl border border-white/5 text-[10px] font-medium text-slate-400 italic">
                                                Cette mission a été validée le {selectedMission.updatedAt ? new Date(selectedMission.updatedAt).toLocaleDateString() : 'N/A'}. 
                                        ...
                                        (() => {
                                            ...
                                            const currentStep = workflow?.approvalSteps?.find((s: any) => s.sequence === workflow.currentStep);
                                            ...
                                            if (!isMyTurn) {
                                                return (
                                                    <div className="p-10 rounded-[2.5rem] bg-amber-500/5 border border-amber-500/20 relative z-10 flex flex-col items-center justify-center text-center">
                                                        ...
                                                        <h4 className="text-sm font-black text-white uppercase italic mb-2">En attente d'une autre validation</h4>
                                                        <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest max-w-xs">
                                                            Cette mission est actuellement à l'étape {workflow?.currentStep} ({currentStep?.label || currentStep?.role}).
                                                            Vous pourrez intervenir une fois cette étape validée.
                                                        </p>
                                                    </div>
                                                );
                                            }
                                            ...
                                            return (
                                                <div className="p-10 rounded-[2.5rem] bg-emerald-500/5 border border-emerald-500/20 relative z-10">
                                                    <div className="flex items-center gap-4 mb-8">
                                                        ...
                                                        <div>
                                                            <h4 className="text-sm font-black text-white uppercase tracking-tight italic">Panel de Décision ({currentStep?.label || workflowRole})</h4>
                                                            <p className="text-[10px] font-bold text-emerald-500/60 uppercase tracking-widest">Votre validation apposera votre signature électronique</p>
                                                        </div>
                                                    </div>
                            ...
                            <div className="h-full flex flex-col items-center justify-center text-center p-20 glass-card">
                                ...
                                <h3 className="text-xl font-black text-slate-600 uppercase tracking-tighter italic mb-2">Sélectionnez une mission</h3>
                                <p className="text-xs text-slate-500 font-bold max-w-xs leading-relaxed uppercase tracking-widest opacity-60">
                                    Utilisez la liste de gauche pour examiner les demandes en attente de votre expertise décisionnelle.
                                </p>
                            </div>
                        )}
                    </AnimatePresence>
