...
const DEFAULT_TASK_LIBRARY = {
  Préparateur: {
    ...
    materials: [
      'Unités de sertissage hydraulique étalonnées',
      "Stations d'assemblage certifiées",
      'Dispositifs de marquage industriel',
    ],
    hse: [
  ...
  Livreur: {
    ...
    introduction:
      'ARTICLE 1 - OBJET ET OBLIGATION DE RÉSULTAT : Le présent lot régit la chaîne logistique de transport et de distribution des kits. Le Titulaire est tenu par une obligation de résultat quant à la remise physique des matériels aux bénéficiaires finaux dans les délais impartis.',
    missions: [
    ...
    materials: [
      'Vecteurs de transport habilités (Assurance marchandises incluse)',
      'Unités mobiles de saisie numérique synchronisées',
      'Bordereaux de livraison à sécurisation renforcée',
    ],
    hse: [
  ...
  Maçonnerie: {
    ...
    introduction:
      "ARTICLE 1 - OBJET ET NORMES DE CONSTRUCTION : Le Titulaire s'engage à la réalisation des socles et ancrages supports. Les travaux doivent garantir une pérennité décennale et une résistance mécanique certifiée sur chaque point de descente réseau.",
    missions: [
    ...
    materials: [
      'Agrégats de carrière certifiés',
      'Liants hydrauliques de haute performance',
      'Instruments de mesure topographique de haute précision',
    ],
    hse: [
  ...
  Réseau: {
    ...
    introduction:
      'ARTICLE 1 - OBJET ET CONTINUITÉ DE SERVICE : Le présent lot régit le raccordement au réseau de distribution Basse Tension. Le Titulaire garantit la sécurité des biens lors des raccordements sous tension et la conformité aux normes NFC 11-201.',
    missions: [
    ...
    materials: [
      'Conducteurs torsadés aluminium conformes aux normes NFC',
      'Pièces de jonction et connectique homologuées Senelec',
      'Équipements de travaux sous tension (TST) et travaux en hauteur certifiés',
    ],
    hse: [
  ...
  'Installation Intérieure': {
    ...
    introduction:
      "ARTICLE 1 - OBJET ET SÉCURITÉ DES USAGERS : Le présent lot concerne l'intégralité des installations électriques intérieures. Le Titulaire devient garant de la sécurité électrique du ménage, avec une obligation de conformité stricte à la norme NFC 15-100.",
    missions: [
    ...
    materials: [
      'Composants certifiés NF / CE',
      'Appareillage modulaire de marque agréée',
      'Conducteurs cuivre de section normalisée',
    ],
    hse: [
  ...
  'Contrôle & Validation': {
    ...
    introduction:
      "ARTICLE 1 - OBJET ET CERTIFICATION FINALE : Le Titulaire assure la mission de contrôle indépendant avant mise en service. Son intervention conditionne l'admission définitive du projet et l'obtention des certificats de conformité Cossuel.",
    missions: [
    ...
    materials: [
      'Appareillage de mesure haute précision étalonné par un laboratoire agréé',
      'Système expert de validation analytique GEM-SAAS',
    ],
    hse: [
...
export default function Cahier() {
  ...
  const [customLibrary, setCustomLibrary] = useState(() => {
    try {
      ...
      if (saved) {
        ...
        if (
          !parsed['Installation Intérieure'] ||
          !parsed['Préparateur'] ||
          !parsed['Préparateur'].image
        ) {
  ...
  const currentTask =
    ...
    DEFAULT_TASK_LIBRARY['Installation Intérieure'];
  ...
  const [editSubcontracting, setEditSubcontracting] = useState(
    currentTask.subcontracting?.join('\n') || ''
  );
  const [editFinances, setEditFinances] = useState(currentTask.finances?.join('\n') || '');

  // Pricing states
  const [editPricingDailyRate, setEditPricingDailyRate] = useState(
    currentTask.pricing?.dailyRate || 0
  );
  const [editPricingPersonnel, setEditPricingPersonnel] = useState(
    currentTask.pricing?.personnelCount || 0
  );
  const [editPricingDuration, setEditPricingDuration] = useState(
    currentTask.pricing?.durationDays || 0
  );
  const [editPricingPenalties, setEditPricingPenalties] = useState(
  ...
  const handleRoleChange = (role: string) => {
    ...
    setIsEditing(false);
    const task =
      customLibrary[role as keyof typeof customLibrary] ||
      DEFAULT_TASK_LIBRARY['Installation Intérieure'];
  ...
  const filteredRoles = Object.entries(customLibrary).filter(([name]) => {
    ...
    return (
      userSearch.includes(name.toLowerCase()) ||
        ...
        actions={
          <ActionBar>
            <div className="flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-orange-100 dark:bg-orange-900/50 border border-orange-600 dark:border-orange-600">
              ...
              <span className="text-xs font-black text-orange-900 dark:text-orange-100 uppercase tracking-widest leading-none">
                NS 01-001
              </span>
            </div>
            {isAdmin && (
        ...
        <div className="max-w-7xl mx-auto grid grid-cols-1 xl:grid-cols-4 gap-8 p-8">
              ...
              return (
                ...
                >
                  ...
                  >
                    <Icon
                      size={22}
                      className={isSelected ? 'text-white' : `text-${data.color}-500`}
                    />
                  </div>
                  <div className="ml-4 text-left">
                    <span
                      className={`block font-bold leading-tight ${isSelected ? 'text-white' : 'text-slate-200 group-hover:text-white'}`}
                    >
                      {name}
                    </span>
                    <span
                      className={`text-xs uppercase font-bold tracking-widest ${isSelected ? 'text-orange-100' : 'text-slate-500 opacity-60'}`}
                    >
                      Cahier Lot n°24
                    </span>
                  </div>
          ...
          <main className="xl:col-span-3 space-y-8">
            <div className="bg-slate-900 border border-slate-800 rounded-3xl overflow-hidden shadow-2xl">
              <div
                className={`p-8 bg-gradient-to-br from-${currentTask.color}-500/10 to-transparent border-b border-slate-800`}
              >
                <div className="flex items-center justify-between">
                  <div className="flex items-center space-x-4">
                    <div
                      className={`w-14 h-14 bg-${currentTask.color}-500/20 rounded-2xl flex items-center justify-center border border-${currentTask.color}-500/30`}
                    >
                    ...
                    <div>
                      ...
                      <p className="text-slate-500">
                        Détails des missions techniques et logistiques
                      </p>
                    </div>
                  </div>
              ...
              <div className="p-8 grid grid-cols-1 md:grid-cols-2 gap-10">
                    ...
                    <div className="relative aspect-[4/5] bg-slate-950 rounded-2xl overflow-hidden border border-slate-800 shadow-2xl">
                      ...
                      <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-slate-950 via-slate-950/40 to-transparent p-6">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <span className="block text-xs font-black text-orange-400 uppercase tracking-widest">
                              Guide Officiel
                            </span>
                    ...
                    <ul className="space-y-4">
                      {currentTask.missions.map((m: string, i: number) => (
                        <li key={i} className="flex items-start space-x-3 group">
                          ...
                          />
                          <span className="text-slate-300 text-sm leading-relaxed font-bold">
                            {m.split(' : ')[0]}
                          </span>
                ...
                <section>
                    ...
                    {isEditing ? (
                      ...
                      <div className="flex flex-wrap gap-2">
                        {currentTask.materials.map((m: string, i: number) => (
                          <span
                            key={i}
                            className="flex items-center space-x-2 bg-slate-800 text-slate-300 px-3 py-1.5 rounded-lg border border-slate-700 text-xs font-bold"
                          >
                  ...
                  <div className="flex items-center space-x-2 mb-6 mt-8">
                    ...
                    <h4 className="font-bold text-white uppercase tracking-wider text-sm">
                      Dispositions Financières, Cautions & Garanties
                    </h4>
                  </div>
                  ...
                  <div className="flex items-center space-x-2 mb-6 mt-8">
                    ...
                    <h4 className="font-bold text-white uppercase tracking-wider text-sm">
                      Configurations & Barèmes Contractuels
                    </h4>
                  </div>
                          ...
                          <div>
                            ...
                            <div className="relative">
                              <input
                                ...
                                onChange={(e) =>
                                  setEditPricingDailyRate(e.target.valueAsNumber || 0)
                                }
                        ...
                        <div className="pt-2 flex justify-end">
                          <div className="bg-emerald-500/20 px-4 py-2 rounded-lg border border-emerald-500/30">
                            ...
                            <span className="text-emerald-300 font-black">
                              {(
                                editPricingDailyRate *
                                editPricingPersonnel *
                                editPricingDuration
                              ).toLocaleString()}{' '}
                              FCFA
                            </span>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 opacity-50 grayscale pointer-events-none">
              ...
              <div className="bg-slate-900 border border-slate-800 p-8 rounded-3xl border-dashed">
                <div className="h-20 flex flex-col items-center justify-center">
                  <span className="text-xs text-slate-600 dark:text-slate-400 font-bold uppercase tracking-widest text-center">
