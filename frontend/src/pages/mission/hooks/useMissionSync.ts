...
export const useMissionSync = (
  ...
  actions: {
    ...
    loadMission: (id: string | null, data: any, members: any[], version?: number, updatedAt?: string, auditTrail?: AuditEntry[]) => void;
  ...
  const { formData, members, currentMissionId, isCertified, isSubmitted, version, auditTrail } = state;

  /**
   * SAUVEGARDE & SYNC (Push)
   */
  const handleSaveMission = useCallback(async (overrideFlags?: { isSubmitted?: boolean; isCertified?: boolean }) => {
  ...
  }, [formData, members, currentMissionId, isCertified, version, auditTrail, activeProjectId, actions, state.version]);
    ...
    try {
      ...
      for (const m of missions) {
        ...
        if (!localMission) {
          ...
          logger.warn(`Conflit détecté (Mission ${m.id}): Local v${localMission.version} > Server v${serverVersion}. Push prioritaire.`);
          await syncQueue.enqueue(localMission.id, { type: 'SET_FORM_DATA', payload: localMission });
          conflictCount++;
        }
      }
      actions.setStatus('success');
      if (mergedCount > 0 || conflictCount > 0) {
        actions.addAuditEntry(`Import: ${mergedCount} MAJ, ${conflictCount} conflits résolus`, "System");
      }
    } catch (err) {
      actions.setStatus('error');
