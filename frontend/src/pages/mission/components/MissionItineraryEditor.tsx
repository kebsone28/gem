    ...
    <section className="glass-card !p-8 !rounded-[2.5rem] space-y-6 relative overflow-hidden group">
      ...
      <div className="flex items-center justify-between pb-4 border-b border-white/5 relative z-10">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h2 className="!text-[11px] font-black text-white uppercase tracking-[0.2em]">Itinéraire Détaillé</h2>
            <p className="text-[9px] font-bold text-slate-500 uppercase mt-1">Planification des étapes quotidiennes</p>
          </div>
        </div>
      ...
      <div className="space-y-4 relative z-10">
        {planning.map((step, i) => (
          <div 
            key={i} 
            className="flex gap-6 group/item relative pl-10"
          >
            {/* Timeline Line */}
            ...
            <div className="absolute left-0 top-0 w-10 h-10 rounded-xl bg-slate-900 border-2 border-indigo-500/30 flex flex-col items-center justify-center z-10 shadow-lg group-hover/item:border-indigo-400 transition-colors">
              <span className="text-[8px] font-black text-indigo-400 uppercase leading-none">J</span>
            ...
            <div className="flex-1 space-y-2">
              <div className="relative group/field">
                ...
                <div className="mt-3 flex gap-4 text-[8px] font-black text-slate-500 uppercase tracking-widest pl-1">
                   <div className="flex items-center gap-1.5"><MapPin size={10} className="text-emerald-500/50" /> Trace GPS Pro</div>
                   <div className="flex items-center gap-1.5"><Calendar size={10} className="text-indigo-500/50" /> Sync Automatique</div>
                </div>
              </div>
            </div>
          </div>
        ...
        {planning.length === 0 && (
          <div className="text-center py-12 bg-slate-900/30 rounded-3xl border-2 border-dashed border-white/5">
             ...
             <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Aucune étape planifiée</p>
             <p className="text-[8px] text-slate-600 mt-1 uppercase">Cliquez sur "Ajouter une étape" pour commencer</p>
          </div>
        )}
      </div>
    </section>
