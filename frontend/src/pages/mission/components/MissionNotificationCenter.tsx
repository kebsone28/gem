...
export const MissionNotificationCenter: React.FC<MissionNotificationCenterProps> = ({ onClose, projectId }) => {
  ...
  const filteredNotifications = notifications.filter((n: MissionNotification) => 
      ...
      <div className="relative w-full max-w-md bg-white dark:bg-slate-900 shadow-2xl h-full flex flex-col animate-in slide-in-from-right duration-300">
        <div className="p-6 border-b border-slate-200 dark:border-white/10 flex items-center justify-between bg-slate-50 dark:bg-slate-800/50">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-sm font-black text-slate-800 dark:text-white uppercase tracking-widest">Centre d'Alertes</h3>
              <p className="text-[10px] text-slate-500 font-bold uppercase tracking-tighter">{notifications.length} Messages</p>
            </div>
          </div>
          <button onClick={onClose} title="Fermer" className="p-2 hover:bg-slate-200 dark:hover:bg-white/10 rounded-xl transition-colors">
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-3">
          {filteredNotifications.length === 0 ? (
            <div className="h-full flex flex-col items-center justify-center text-center p-8 opacity-40">
              ...
              <p className="text-xs font-black uppercase tracking-widest text-slate-400">Aucun message archivé</p>
            </div>
          ) : (
            filteredNotifications.map((n: MissionNotification) => (
              ...
              >
                <div className="flex gap-4">
                  <div className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 ${n.type === 'approval' ? 'bg-emerald-500/10' : n.type === 'rejection' ? 'bg-rose-500/10' : 'bg-slate-500/10'}`}>
                    {n.type === 'approval' ? <CheckCircle2 className="text-emerald-500" size={18} /> : 
                     n.type === 'rejection' ? <XCircle className="text-rose-500" size={18} /> : 
                     <Clock className="text-slate-500" size={18} />}
                  </div>
                  <div className="flex-1">
                    <div className="flex items-center justify-between mb-1">
                      <h4 className="text-[11px] font-black text-slate-800 dark:text-white uppercase tracking-wider">{n.title}</h4>
                      <span className="text-[9px] font-bold text-slate-400">{new Date(n.createdAt).toLocaleDateString('fr-FR', { hour: '2-digit', minute: '2-digit' })}</span>
                    </div>
                    <p className="text-[11px] text-slate-600 dark:text-slate-400 leading-relaxed font-semibold">{n.message}</p>
                    <div className="mt-2 flex items-center gap-2">
                       <span className="text-[8px] font-black bg-slate-100 dark:bg-white/10 text-slate-500 px-2 py-0.5 rounded uppercase tracking-tighter">De: {n.sender}</span>
                    </div>
                  </div>
                </div>
