...
export default function AdminModules() {
  ...
  const toggleModule = async (moduleId: string, enabled: boolean) => {
    ...
    try {
      ...
      toast.success(`Module ${modules.find(m => m.id === moduleId)?.name} ${enabled ? 'activé' : 'désactivé'} globalement`);
    } catch (error: any) {
            ...
            >
              ...
              <div className="p-5 rounded-[2rem] bg-slate-800/50 border border-slate-700 mb-8">
                <p className="text-slate-300 text-sm font-medium mb-3">
                  Vous êtes sur le point de {confirmModal.enabled ? 'activer' : 'désactiver'} le module <strong>{confirmModal.moduleName}</strong> pour tous les utilisateurs.
                </p>
        ...
        <div className="max-w-7xl mx-auto space-y-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <div className="p-6 bg-gradient-to-br from-purple-500/10 to-purple-600/10 border border-purple-500/20 rounded-2xl">
              ...
              <p className="text-2xl font-black text-white">
                {Object.values(modulesByCategory).filter(m => m[0]?.category === 'advanced').filter(m => m.enabled).length}/{Object.values(modulesByCategory).filter(m => m[0]?.category === 'advanced').length}
              </p>
            ...
            <div className="p-6 bg-gradient-to-br from-orange-500/10 to-orange-600/10 border border-orange-500/20 rounded-2xl">
              ...
              <p className="text-2xl font-black text-white">
                {Object.values(modulesByCategory).filter(m => m[0]?.category === 'experimental').filter(m => m.enabled).length}/{Object.values(modulesByCategory).filter(m => m[0]?.category === 'experimental').length}
              </p>
            ...
            <div className="p-6 bg-gradient-to-br from-red-500/10 to-red-600/10 border border-red-500/20 rounded-2xl">
              ...
              <p className="text-2xl font-black text-white">
                {Object.values(modulesByCategory).filter(m => m[0]?.category === 'admin').filter(m => m.enabled).length}/{Object.values(modulesByCategory).filter(m => m[0]?.category === 'admin').length}
              </p>
          ...
          {Object.entries(modulesByCategory).map(([category, categoryModules]) => (
            <div key={category} className="space-y-4">
              <div className="flex items-center gap-3 mb-4">
                <div className={`w-8 h-8 rounded-xl flex items-center justify-center ${getCategoryColor(category)}`}>
                  {getModuleIcon(categoryModules[0]?.icon)}
                </div>
                <h3 className="text-lg font-black text-white capitalize">
                  {category === 'core' ? 'Modules Core' : 
                   category === 'advanced' ? 'Modules Avancés' :
                   category === 'experimental' ? 'Modules Expérimentaux' :
                   'Modules Admin'}
                </h3>
                <span className="text-xs text-slate-500 uppercase tracking-widest">
                  {categoryModules.filter(m => m.enabled).length}/{categoryModules.length} activés
                </span>
              </div>
                  ...
                  .map(module => {
                    ...
                    return (
                      <motion.div
                        ...
                        onClick={() => canToggleGlobally && openConfirmModal(module.id, !isGloballyEnabled)}
                      >
                        <div className="flex items-start justify-between mb-4">
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              <h4 className="text-white font-black text-base mb-1">{module.name}</h4>
                          ...
                          <div className="mt-4 pt-4 border-t border-slate-700/50">
                            <div className="flex items-center justify-between text-xs">
                              <span className="text-slate-400">
                                {moduleStats.userCount} utilisateur{moduleStats.userCount > 1 ? 's' : ''}
                              </span>
                              <div className="flex items-center gap-2">
                                {isGloballyEnabled ? (
                                  <>
                        ...
                        <div className="mt-3">
                          ...
                          <div className="flex flex-wrap gap-1">
                            {module.permissions.map(perm => (
                              <span key={perm} className="px-2 py-1 bg-slate-800 rounded text-xs text-slate-300">
                                {perm}
                              </span>
                            ))}
                          </div>
