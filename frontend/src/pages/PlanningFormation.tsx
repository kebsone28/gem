...
export default function PlanningFormation() {
  ...
  const applyPreviewSessionUpdate = async (
    sessionId: string,
    updates: Partial<
    ...
    try {
      const { session: updatedSession } = await formationApi.validatePreviewSession({
        sessionId,
        updates,
        plan: previewPlan,
        plannerConfig,
      ...
      return {
        ok: false as const,
        reason: error instanceof Error ? error.message : 'Validation impossible',
      };
    }
  };

  const movePreviewSessionToDate = async (sessionId: string, nextStartDate: string) => {
    if (!previewPlan) return;
    const targetSession = previewPlan.sessions.find((session) => session.id === sessionId);
    if (!targetSession) return;

    const result = await applyPreviewSessionUpdate(sessionId, { startDate: nextStartDate });
    if (result.ok) {
  ...
  const savePreviewSessionEdit = async () => {
    ...
    setSubmitting(true);
    try {
      const result = await applyPreviewSessionUpdate(editingPreviewSession.id, {
