...
import { 
    Database, Link2, Search, Save, AlertCircle, CheckCircle2, 
    RefreshCcw, ArrowRight, Table, Settings2, Info, Lock, Unlock
} from 'lucide-react';
                    ...
                    <div className="flex flex-col md:flex-row items-center justify-between gap-8">
                        <div className="space-y-4">
                            <div className="flex items-center gap-3">
                                ...
                                <h2 className="text-3xl font-black text-white tracking-tight uppercase italic">Architecture des Données</h2>
                            </div>
                            <p className="text-slate-500 text-sm font-medium leading-relaxed max-w-xl">
                                Gérez la correspondance entre vos formulaires KoboToolbox et la base de données centrale PROQUELEC. 
                                Les changements ici affectent instantanément les futures synchronisations terrain.
                            </p>
                        </div>
                        <div className="flex flex-col items-end gap-3">
                            ...
                            <span className="text-[10px] font-black text-slate-600 uppercase tracking-widest flex items-center gap-2">
                                <CheckCircle2 size={12} className="text-emerald-500" /> Dernière mise à jour: {lastSync || 'Jamais'}
                            </span>
                        </div>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
                    ...
                    <div className="lg:col-span-8 space-y-6">
                        <div className="flex items-center justify-between px-2 mb-4">
                            <h3 className="text-xs font-black text-slate-500 uppercase tracking-[0.3em] italic">Champs de Destination PROQUELEC</h3>
                        ...
                        <div className="space-y-4">
                            {TARGET_FIELDS.filter(f => f.label.toLowerCase().includes(searchField.toLowerCase())).map((field) => (
                                <motion.div 
                                    key={field.id}
                                ...
                                >
                                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6">
                                        <div className="flex items-center gap-4">
                                            ...
                                            <div>
                                                <h4 className="text-sm font-black text-white uppercase italic tracking-tight">{field.label}</h4>
                                                <div className="flex items-center gap-2 mt-1">
                                                    <span className="text-[9px] font-black text-slate-600 uppercase tracking-widest">ID: {field.id}</span>
                                                    {field.required && <span className="text-[9px] font-black text-rose-500/80 uppercase tracking-widest bg-rose-500/5 px-2 py-0.5 rounded-full border border-rose-500/10">Requis</span>}
                                                </div>
                                            </div>
                                        </div>
                    ...
                    <div className="lg:col-span-4 space-y-8">
                        <div className="p-8 bg-indigo-500/5 border border-indigo-500/10 rounded-[2.5rem] space-y-6">
                            ...
                            <div className="space-y-4">
                                <div className="flex gap-4">
                                    <div className="w-8 h-8 rounded-full bg-indigo-500/10 border border-indigo-500/20 flex items-center justify-center text-indigo-400 font-black text-[10px] shrink-0">01</div>
                                    <p className="text-xs text-slate-400 leading-relaxed">Référez-vous au <strong>Name</strong> de la question dans votre fichier XLSForm ou sur KoboToolbox.</p>
                                </div>
                                <div className="flex gap-4">
                                    <div className="w-8 h-8 rounded-full bg-indigo-500/10 border border-indigo-500/20 flex items-center justify-center text-indigo-400 font-black text-[10px] shrink-0">02</div>
                                    <p className="text-xs text-slate-400 leading-relaxed">Pour les champs imbriqués, utilisez le format <strong>groupe/variable</strong>.</p>
                                </div>
                                <div className="flex gap-4">
                                    <div className="w-8 h-8 rounded-full bg-indigo-500/10 border border-indigo-500/20 flex items-center justify-center text-indigo-400 font-black text-[10px] shrink-0">03</div>
                                    <p className="text-xs text-slate-400 leading-relaxed">Sauvegardez pour que le moteur de synchronisation prenne en compte les nouveaux chemins.</p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="p-10 rounded-[3rem] bg-rose-500/5 border border-rose-500/10 h-fit">
                            <div className="flex items-center gap-4 mb-6">
                                ...
                                <h3 className="text-xs font-black text-rose-500 uppercase tracking-widest italic">Contrôle Accès Master</h3>
                            </div>
                            <p className="text-[11px] text-slate-500 font-bold leading-relaxed mb-6">
                                Le mapping des champs est une opération à cœur ouvert. Les erreurs ici peuvent casser la traçabilité des ménages. 
                                <br/><br/>
                            ...
                            <div className="p-4 rounded-2xl bg-slate-950/50 border border-rose-500/10">
                                <p className="text-[9px] font-black text-rose-500 uppercase tracking-[0.2em] mb-1">Status Sécurité</p>
                                <p className="text-[10px] text-white font-bold italic">Session Administrateur Protégée</p>
                            </div>
                        </div>
                    </div>
                </div>
