...
import {
    ...
    TrendingUp, Package, RefreshCw, ShieldCheck
} from 'lucide-react';
...
export default function Reports() {
    ...
    const stats = useMemo(() => {
        const allStats = [
            ...
            { label: `${getLabel('household.plural')} Servis`, value: households.length.toLocaleString('fr-FR') || '0', icon: Calendar, color: 'text-blue-500', lseVisible: true },
            { label: `${getLabel('zone.plural')} Actives`, value: zones.length || 0, icon: MapPin, color: 'text-amber-500', lseVisible: true },
                ...
                actions={
                    <div className="flex items-center gap-3 flex-1 md:max-w-md">
                        <div className="relative flex-1 group">
                            <Search className="absolute left-3.5 top-1/2 -translate-y-1/2 text-blue-300/50 group-focus-within:text-blue-500 transition-colors" size={16} />
                            <input
                                type="text"
                    ...
                    {stats.map((stat, i) => (
                        <div key={i} className={`${COMMON_CLASSES.card} ${COMMON_CLASSES.cardHover} p-6 group`}>
                            <div className="flex items-center justify-between mb-4">
                                ...
                                <span className="text-xs font-black text-blue-300/40 uppercase tracking-widest">{stat.label}</span>
                            </div>
                    ...
                    <Section title="Modèles Disponibles" subtitle={`${visible.length} RAPPORTS TROUVÉS`}>
                        <CardGrid columns={2}>
                            {visible.map((report) => {
                                ...
                                return (
                                    <div key={report.id} className={`${COMMON_CLASSES.card} ${COMMON_CLASSES.cardHover} p-8 group relative`}>
                                        <div className="flex items-start justify-between mb-6">
                                            <div className={`w-14 h-14 rounded-2xl bg-white/5 flex items-center justify-center ${report.textColor} group-hover:scale-110 transition-transform shadow-inner`}>
                                                <Icon size={28} />
                                            </div>
                                            <span className={`text-xs font-black px-3 py-1.5 rounded-lg ${typeColor(report.type)} uppercase tracking-widest`}>
                                                {report.type}
                                            </span>
                                        </div>

                                        <h4 className={`${COMMON_CLASSES.heading3} mb-2 text-white group-hover:text-blue-400 transition-colors uppercase tracking-tight`}>{report.title}</h4>
                                        ...
                                        <div className="space-y-2 mb-8 bg-white/5 p-4 rounded-2xl border border-white/5">
                                            <p className="text-xs font-black text-blue-300/40 uppercase tracking-widest mb-2">Inclus dans l'export :</p>
                                            {report.preview.map((p, i) => (
                                                <div key={i} className="flex items-center gap-2 text-xs font-bold text-blue-100/80">
                        ...
                        <ContentArea className="bg-[#0D1E35] border-white/5">
                            <div className="space-y-6">
                                <div className="space-y-3">
                                    <label className={COMMON_CLASSES.label} htmlFor="export-format">Format du fichier</label>
                            ...
                            <div className={`flex items-center gap-4 p-5 rounded-3xl border text-xs font-bold leading-relaxed mt-8
                                ${isAdmin ? 'bg-blue-500/10 border-blue-500/20 text-blue-400' :
                                    isLSE ? 'bg-amber-500/10 border-amber-500/20 text-amber-400' :
                                        'bg-emerald-500/10 border-emerald-500/20 text-emerald-400'}`}>
                                <ShieldCheck size={20} className="shrink-0" />
                                <div>
                                    <span className="uppercase text-xs font-black opacity-60">Permissions :</span><br />
