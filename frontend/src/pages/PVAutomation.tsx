...
function usePVAutomation() {
  ...
  // 🔄 Correction: Différencier le chargement (undefined) du résultat vide ([])
  ...
  const handleCreatePV = useCallback(async (type: PVType, submission: any) => {
    ...
    try {
      ...
      await dispatchPVAlerts({
        pvId, householdId: submission.id, projectId: submission.projectId || 'N/A',
      ...
      await alertsAPI.createAlert({
        projectId: submission.projectId || 'N/A', householdId: submission.id, pvId,
  ...
  return {
    ...
    archivedPVs, filteredSubmissions, getRecommendedType, handleCreatePV, isLoadingDB
  };
}
  ...
  return (
    <PageContainer>
      ...
      <ContentArea className="space-y-8">
        ...
        {logic.archivedPVs.length > 0 && <PVArchivePanel archivedPVs={logic.archivedPVs} />}
      </ContentArea>
      <SignatureModal 
      ...
      <div className="p-8 border-b border-white/5 flex flex-col md:flex-row justify-between items-center gap-4">
        ...
        <div className="flex flex-wrap gap-2">
          {(Object.keys(PV_TEMPLATES) as PVType[]).map(type => {
            ...
            const colors = COLOR_MAP[tmpl.color];
            const isSelected = logic.selectedSubmission.activePVType === type;
            return (
              <button 
                key={type} 
                onClick={() => logic.handleCreatePV(type, logic.selectedSubmission)} 
                disabled={logic.isGenerating}
                title={tmpl.title}
                aria-label={`Générer ${tmpl.title}`}
      ...
      <div className="bg-white/5 border border-white/5 rounded-[2rem] p-10 font-mono text-xs text-slate-300 relative overflow-hidden">
         ...
         <div className="space-y-6">
            ...
            <div className="grid grid-cols-2 gap-8 pt-6">
               <div><p className="text-[10px] font-bold text-slate-500 uppercase mb-2">Signature Direction</p><div className="h-16 flex items-center justify-center bg-slate-900/50 rounded-xl border border-white/5"><ShieldCheck className="text-emerald-500/20" size={32} /></div></div>
       ...
       <div className="flex gap-4">
         ...
         <button 
           ...
           className="bg-emerald-600 hover:bg-emerald-500 text-white font-black px-6 rounded-2xl flex items-center justify-center transition-all active:scale-95 shadow-xl shadow-emerald-900/20"
         >
...
function PVStatsBoard({ archivedPVs, isLoadingDB }: { archivedPVs: any[], isLoadingDB?: boolean }) {
  const stats = useMemo(() => {
    ...
    const ncCount = archivedPVs.filter(pv => pv.type === 'PVNC').length;
    return { 
      total, 
      conformity: total > 0 ? Math.round(((total - ncCount) / total) * 100) : 100,
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mt-8">
      <StatWidget label="Conformité" value={stats.conformity} suffix="%" color="emerald" icon={ShieldCheck} />
...
function StatWidget({ label, value, color, icon: Icon, suffix = "" }: any) {
  const styles = COLOR_MAP[color as keyof typeof COLOR_MAP];
  return (
    <div className={`p-6 bg-slate-900/40 border ${styles.border} rounded-[2rem] relative overflow-hidden shadow-lg`}>
      ...
      <div className="text-3xl font-black text-white"><AnimatedCounter value={value} suffix={suffix} /></div>
    </div>
  );
}
  ...
  return (
    <div className="mt-12 bg-slate-900/40 border border-white/5 rounded-[2.5rem] overflow-hidden shadow-2xl">
      <div className="p-8 border-b border-white/5 flex justify-between items-center bg-gradient-to-b from-white/5 to-transparent">
        <div><h3 className="text-lg font-black text-white uppercase tracking-widest">Registre de Traçabilité</h3><p className="text-[10px] text-slate-500 font-bold flex items-center gap-2 mt-1"><ShieldCheck size={12} className="text-emerald-500" /> Documents persistés GEM</p></div>
      ...
      <div className="p-2 overflow-x-auto">
        <table className="w-full text-left text-xs border-separate border-spacing-y-2">
          <thead className="text-slate-600 font-black uppercase tracking-[0.2em] text-[9px]"><tr><th className="px-6 py-4">Status / Type</th><th className="px-6 py-4">Lot ID</th><th className="px-6 py-4">Horodatage</th><th className="px-6 py-4 text-right">Actions</th></tr></thead>
          <tbody>
            {archivedPVs.map(pv => (
              <tr key={pv.id} className="bg-white/[0.02] hover:bg-white/[0.05] transition-all">
                <td className="px-6 py-4 border-y border-l border-white/5">
                  <span className="inline-flex items-center gap-2 px-3 py-1 rounded-lg font-black text-[9px] border bg-white/5">{pv.type}</span>
                </td>
                <td className="px-6 py-4 border-y border-white/5 font-black text-white">{pv.metadata?.numeroordre || '—'}</td>
                <td className="px-6 py-4 border-y border-white/5 text-slate-500">{format(new Date(pv.createdAt), 'dd/MM/yyyy')}</td>
                <td className="px-6 py-4 border-y border-r border-white/5 text-right">
                  <button 
                    aria-label="Voir le PV"
                    title="Voir le PV"
                    className="p-2.5 bg-slate-800 rounded-xl hover:bg-blue-600 text-white transition-all"
                  >
                    <Eye size={14}/>
                  </button>
                </td>
