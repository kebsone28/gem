...
export const ControlPanel: React.FC<ControlPanelProps> = ({ metrics, feedActivities, missions }) => {
  return (
    <div className="lg:col-span-4 space-y-10">
          ...
          <AnimatePresence>
            {metrics.problemHouseholds > 0 && (
              ...
              >
                ...
                <div>
                  <p className="text-xs font-black text-rose-500 uppercase tracking-[0.1em] italic">INCIDENTS CRITIQUES</p>
            ...
            {metrics.syncHealth !== 'healthy' && (
              ...
              >
                ...
                <div>
                  <p className="text-xs font-black text-amber-500 uppercase tracking-[0.1em] italic">ALERTE SYNCHRO</p>
                  <p className="text-[11px] text-slate-400 mt-2 leading-relaxed font-bold">LATENCE DÉTECTÉE DANS LA LIAISON CLOUD.</p>
                </div>
              </motion.div>
            )}
            {metrics.problemHouseholds === 0 && metrics.syncHealth === 'healthy' && (
              <div className="py-14 text-center bg-white/[0.02] rounded-[2.5rem] border border-dashed border-white/10">
                ...
                <p className="text-[10px] font-black text-slate-600 uppercase tracking-[0.3em] italic">SYSTÈMES OPÉRATIONNELS</p>
              </div>
            )}
          </AnimatePresence>
        </AlertPanel>
      ...
      <div className="p-10 rounded-[3.5rem] bg-slate-900/40 border border-white/5 backdrop-blur-3xl shadow-2xl h-[500px] flex flex-col overflow-hidden">
        ...
        <div className="space-y-5 relative z-10">
          {missions.length > 0 ? (
            ...
            <div className="py-14 text-center opacity-20 flex flex-col items-center border border-dashed border-white/10 rounded-2xl">
              ...
              <p className="text-[10px] font-black uppercase tracking-[0.2em] text-white">AUCUNE MISSION À VENIR</p>
            </div>
          )}
        </div>
      </div>
