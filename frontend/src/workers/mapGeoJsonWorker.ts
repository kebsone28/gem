...
function getHouseholdDerivedStatus(h: Household): string {
    ...
    if (cData) {
        if (cData.audit?.installation_conforme === 'conforme' || cData.audit?.branchement_conforme === 'conforme' || h.status === 'TERMINE') return 'Contrôle conforme';
        if (cData.audit?.installation_conforme === 'non_conforme' || cData.audit?.branchement_conforme === 'non-conforme' || h.status === 'BLOQUE') return 'Non conforme';
        if (cData.interiorStatus === 'COMPLETE' || h.status === 'Intérieur terminé') return 'Intérieur terminé';
        if (cData.networkStatus === 'COMPLETE' || h.status === 'Réseau terminé') return 'Réseau terminé';
        if (cData.wallType === 'STANDARD' || cData.wallType === 'CHIMNEY' || h.status === 'Murs terminés') return 'Murs terminés';
        if (cData.kit?.status === 'COMPLETE' || h.status === 'Livraison effectuée') return 'Livraison effectuée';
    }
    ...
    if (h.status && h.status !== 'Non débuté' && h.status !== 'Pending' && h.status !== 'NON_DEMARRE') return h.status;
