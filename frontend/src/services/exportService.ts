...
const addFooters = (pdf: jsPDF) => {
    ...
    for (let p = 1; p <= totalPages; p++) {
        ...
        pdf.text('Document genere automatiquement par la plateforme GEM SaaS PROQUELEC.', 10, pageHeight - 9);
...
export const exportFinancialPDF = async (devisReport: any[], devis: any, stats: any, projectName?: string): Promise<true | string> => {
    try {
        const pdf = new jsPDF({ orientation: 'l', unit: 'mm', format: 'a4' });

        addHeader(pdf, 'Bilan Financier Prévisionnel', `Projet : ${projectName || 'N/A'} — Plafond devis : ${fmtFCFA(devis.ceiling)}`);
        ...
        autoTable(pdf, {
            ...
            head: [kpiData[0]],
            body: [kpiData[1]],
            headStyles: { fillColor: [30, 41, 59], textColor: [148, 163, 184], fontSize: 8, fontStyle: 'bold', halign: 'center' },
        ...
        autoTable(pdf, {
            startY: afterKPI + 4,
            head: [['Poste de Depense', 'Region', 'Qte Prevu', 'P.U Prevu', 'Total Prevu', 'Qte Reel', 'P.U Reel', 'Total Reel', 'Ecart / Marge']],
            ...
            foot: [['TOTAUX CONSOLIDES', '', '', '', fmtFCFA(devis.totalPlanned), '', '', fmtFCFA(devis.totalReal), fmtFCFA(devis.globalMargin)]],
...
export const exportToPDF = async (_elementId: string, _options: ExportOptions): Promise<true | string> => {
