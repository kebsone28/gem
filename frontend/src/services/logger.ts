...
export type LogCategory = 'SYNC' | 'OFFLINE' | 'AUTH' | 'CONFLICT' | 'QUEUE' | 'RETRY' | 'UI' | 'PERF' | 'GENERAL';
...
function createEntry(level: LogLevel, category: LogCategory, message: string, data?: unknown): LogEntry {
    return {
        level,
        category,
        message,
...
function emit(level: LogLevel, category: LogCategory, message: string, data?: unknown) {
    ...
    switch (level) {
        case 'debug': console.debug(formatted, ...(data !== undefined ? [data] : [])); break;
        case 'info':  console.log(formatted, ...(data !== undefined ? [data] : [])); break;
