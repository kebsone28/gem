 ...
 */
export function calcBackoffDelay(attempt: number, config: RetryConfig = DEFAULT_RETRY_CONFIG): number {
    ...
    for (let attempt = 0; attempt < config.maxAttempts; attempt++) {
        try {
            ...
            if (attempt < config.maxAttempts - 1) {
                ...
                logger.warn('RETRY', `${label} attempt ${attempt + 1}/${config.maxAttempts} failed — retrying in ${delay}ms`, err);
                await sleep(delay);
            }
        }
    }
