...
export function generateRapportAvancement(data: {
  ...
  y = drawKpiRow(
    ...
    [
      ...
      { label: 'En attente', value: num(pending), color: GRAY },
      {
        ...
        color: completionPct >= 50 ? GREEN : INDIGO,
      },
    ],
    y
  );
  ...
  pipeline.forEach((p) => {
    y = drawProgressBar(doc, p.label, p.prog, y, p.color);
  });
  y += 4;

  // Recent actions table
  y = drawSectionTitle(doc, 'Dernières Validations', y);
  const tableRows = households
    .slice(0, 50)
    .map((h, i) => [
      (i + 1).toString(),
...
export function generateRapportKobo(data: { households: any[] }) {
  ...
  const teamStats = {
    livraison:
      totalForms > 0
        ? Math.round(
            (households.filter((h) => !!h.koboSync?.livreurDate).length / totalForms) * 100
          )
        : 0,
    macon:
      totalForms > 0
        ? Math.round((households.filter((h) => !!h.koboSync?.maconOk).length / totalForms) * 100)
        : 0,
    reseau:
      totalForms > 0
        ? Math.round((households.filter((h) => !!h.koboSync?.reseauOk).length / totalForms) * 100)
        : 0,
    installation:
      totalForms > 0
        ? Math.round(
            (households.filter((h) => !!h.koboSync?.interieurOk).length / totalForms) * 100
          )
        : 0,
    controle:
      totalForms > 0
        ? Math.round((households.filter((h) => !!h.koboSync?.controleOk).length / totalForms) * 100)
        : 0,
  };
  ...
  forms.forEach((f) => {
    y = drawProgressBar(doc, f.name, f.pct, y, f.pct >= 70 ? GREEN : f.pct >= 50 ? AMBER : RED);
  });
  y += 6;
...
export function generateRapportLogistique(data: { households: any[]; zones?: any[] }) {
  ...
  const installed = households.filter(
    (h) => h.status === 'Terminé' || h.status === 'Conforme'
  ).length;
