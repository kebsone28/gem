...
export const cahierChargesGenerator = {
  ...
  async generateContract(data: MissionOrderData) {
    const qrResponse = await fetch(`https://api.qrserver.com/v1/create-qr-code/?size=100x100&data=${encodeURIComponent(`https://gem.proquelec.com/contract/${data.orderNumber}`)}`);
      ...
      sections: [{
        ...
        children: [
          ...
          new Paragraph({
            ...
            children: [
              qrData ? new ImageRun({ data: qrData, transformation: { width: 50, height: 50 }, type: 'png' } as any) : new TextRun({ text: "[SÉCURISÉ]" })
            ]
          }),
          new Paragraph({
          ...
          // ARTICLE 1 : OBJET
          new Paragraph({ text: "ARTICLE 1 : OBJET DU CONTRAT", heading: HeadingLevel.HEADING_2 }),
          new Paragraph({
            children: [
              new TextRun({ text: "Le présent contrat définit les conditions d'exécution de la mission de : ", size: 20 }),
              new TextRun({ text: data.purpose, bold: true, size: 20 }),
              new TextRun({ text: ` qui se déroulera à ${data.region} du ${data.startDate} au ${data.endDate}.`, size: 20 })
            ],
            spacing: { after: 300 }
          }),

          // ARTICLE 2 : SYSTÈME DE GARANTIES (ÉQUILIBRE)
          new Paragraph({ text: "ARTICLE 2 : GARANTIES ET CAUTIONS (MODE ÉQUILIBRE)", heading: HeadingLevel.HEADING_2 }),
          ...
          new Paragraph({ text: "ARTICLE 3 : CONTRÔLE QUALITÉ PAR VISION IA", heading: HeadingLevel.HEADING_2 }),
          new Paragraph({
          ...
          new Paragraph({ text: "ARTICLE 4 : RÉFÉRENTIELS TECHNIQUES", heading: HeadingLevel.HEADING_2 }),
          new Paragraph({
          ...
          new Table({
            ...
            rows: [
              new TableRow({
                children: [
                  new TableCell({ children: [new Paragraph({ text: "L'ENTREPRENEUR (Bon pour accord)", bold: true, alignment: AlignmentType.CENTER })] }),
                  new TableCell({ children: [new Paragraph({ text: "LA DIRECTION GÉNÉRALE", bold: true, alignment: AlignmentType.CENTER })] }),
                ]
              }),
              new TableRow({
                children: [
