...
export type GedOsAiCoreConfig = AIEngineSettings;

export interface GedOsAiState {
  isThinking: boolean;
  lastQuery: string;
...
export interface GedOsAiRequest {
  query: string;
  user: User | null;
  context?: AIState;
  options?: {
...
export interface GedOsAiResponse {
  response: AIResponse;
  enriched: boolean;
  confidence: number;
...
export class GedOsAiCore {
  private state: GedOsAiState;
  ...
  async processRequest(request: GedOsAiRequest): Promise<GedOsAiResponse> {
  ...
  getState(): GedOsAiState {
    return { ...this.state };
  }

  getConfig(): GedOsAiCoreConfig {
    return getAIEngineConfig();
  }

  updateConfig(newConfig: Partial<GedOsAiCoreConfig>): void {
    // On utilise l'email système ou admin pour la trace
    saveAIEngineConfig(newConfig, 'system@ged-os.com');
  }
...
let gedOsAiCoreInstance: GedOsAiCore | null = null;

export function getGedOsAiCore(config?: Partial<GedOsAiCoreConfig>): GedOsAiCore {
  if (!gedOsAiCoreInstance) {
    gedOsAiCoreInstance = new GedOsAiCore(config);
  }
  
  return gedOsAiCoreInstance;
}

export function resetGedOsAiCore(): void {
  if (gedOsAiCoreInstance) {
    gedOsAiCoreInstance.reset();
    gedOsAiCoreInstance = null;
  }
}
