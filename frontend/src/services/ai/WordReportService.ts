...
export const wordReportService = {
  ...
  async generateStrategicReport(
    ...
    const qrResponse = await fetch(`https://api.qrserver.com/v1/create-qr-code/?size=80x80&data=${encodeURIComponent(`https://gem.proquelec.com/report/strategic-${Date.now()}`)}`);
        ...
        children: [
          new Paragraph({
            ...
            children: [
              qrData ? new ImageRun({
                ...
                type: 'png'
              } as any) : new TextRun({ text: "[CERTIFIÉ]" })
            ]
          }),
          new Paragraph({
          ...
          new Paragraph({
            children: [
              ...
              new TextRun(`${new Intl.NumberFormat('fr-FR').format(stats?.totalIndemnities || 0)}`),
            ],
          }),
          ...
          new Table({
            ...
            rows: [
              new TableRow({
                children: [
                  new TableCell({ children: [new Paragraph({ children: [new TextRun({ text: "Priorité", bold: true })] })] }),
                  new TableCell({ children: [new Paragraph({ children: [new TextRun({ text: "Analyse / Recommandation", bold: true })] })] }),
                ],
              }),
              ...insights.map(insight => new TableRow({
                children: [
                  new TableCell({ 
