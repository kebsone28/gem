...
export const generateGrappePDF = (grappe: GrappeData, projectName: string = 'Projet GEM') => {
    ...
    const percent = grappe.householdCount > 0 ? Math.round((grappe.electrified / grappe.householdCount) * 100) : 0;
    doc.text(`Progression: ${percent}%`, 160, currentY);
    currentY += 8;

    // TEAMS
    const teamList = grappe.teams?.length > 0 
    ...
    const tableData = (grappe.households || []).map((h, index) => {
        ...
        return [
            ...
            h.status === 'completed' || h.status === 'Terminé' ? 'Terminé' : (h.status === 'in_progress' ? 'En Cours' : 'Planifié')
        ];
    });

    autoTable(doc, {
        startY: currentY,
        head: [['#', 'ID B.', 'Nom du Ménage / Propriétaire', 'Village', 'Lat.', 'Long.', 'Téléphone', 'Statut']],
        body: tableData,
        theme: 'grid',
        headStyles: { 
            fillColor: INDIGO, 
...
export const generateGrappeExcel = (grappe: GrappeData) => {
    ...
    const tableData = (grappe.households || []).map((h, index) => {
        ...
        return {
            ...
            "Grappe": grappe.name,
            "Statut": h.status === 'completed' || h.status === 'Terminé' ? 'Terminé' : (h.status === 'in_progress' ? 'En Cours' : 'Planifié')
        };
    });
    ...
    const kpiData = [
        ...
        { Paramètre: "Progression", Valeur: `${grappe.householdCount > 0 ? Math.round((grappe.electrified / grappe.householdCount) * 100) : 0}%` },
        { Paramètre: "", Valeur: "" },
