...
const ToggleRow = ({ icon: Icon, label, color, active, onToggle }: any) => (
  ...
  >
    <div className="flex items-center gap-2.5 pointer-events-none">
      <Icon
        size={13}
        className={`${color} ${active ? 'opacity-100' : 'opacity-30'} transition-opacity`}
      />
      <span
        className={`text-[11px] font-semibold ${active ? 'text-white' : 'text-slate-500'} transition-colors`}
      >
        {label}
      </span>
    </div>
    <div
      className={`w-8 h-4.5 h-[18px] rounded-full relative transition-all duration-250 pointer-events-none ${active ? 'bg-indigo-500 shadow-md shadow-indigo-500/30' : 'bg-slate-800'}`}
    >
      <div
        className={`absolute top-[3px] w-3 h-3 rounded-full bg-white shadow transition-all duration-250 ${active ? 'left-[18px]' : 'left-[3px]'}`}
      />
    </div>
  </div>
);
...
export const MissionOrderActionBar = (props: any) => {
  ...
  const statusBadge = isCertified
    ...
    : isSubmitted
      ...
      : {
          label: 'BROUILLON',
          bg: 'bg-slate-700/60 border-white/8 text-slate-400',
          dot: isDirty ? 'bg-amber-400' : 'bg-slate-500',
          pulse: false,
        };

  /* ── Save button config ── */
  const saveState = (() => {
    if (isSubmitted || isCertified)
      return {
        label: 'ARCHIVÉ',
        cls: 'bg-slate-800/60 text-slate-600 cursor-default',
        icon: CheckCheck,
        disabled: true,
      };
    if (isSyncing || isSyncingServer)
      return {
        label: 'ENVOI…',
        cls: 'bg-indigo-600/50 text-indigo-300',
        icon: Loader2,
        disabled: true,
      };
    if (isDirty)
      return {
        label: 'ENREGISTRER',
        cls: 'bg-gradient-to-r from-orange-600 to-amber-500 text-white shadow-lg shadow-amber-500/25',
        icon: Save,
        disabled: false,
      };
    if (syncStatus === 'failed')
      return {
        label: 'ERREUR SYNC',
        cls: 'bg-gradient-to-r from-rose-700 to-rose-600 text-white shadow-lg shadow-rose-500/25',
        icon: Save,
        disabled: false,
      };
    return {
      label: 'À JOUR',
      cls: 'bg-gradient-to-r from-blue-700 to-indigo-600 text-white shadow-lg shadow-indigo-500/25',
      icon: Save,
      disabled: false,
    };
  })();
  ...
  return (
    <ActionBar className="no-print !bg-[#080b12]/90 backdrop-blur-3xl border-b border-white/[0.06] px-4 py-2.5 shadow-[0_4px_24px_rgba(0,0,0,0.4)]">
      <div className="flex flex-col w-full gap-1 min-w-0">
        ...
        <div className="flex items-center gap-2 min-w-0">
          ...
          <div className="flex items-center gap-2 min-w-0 flex-1">
            {!isCertified && !isSubmitted ? (
              <input
                ...
                placeholder="OBJET…"
                className="bg-transparent border-none outline-none text-[11px] font-black text-white/90 placeholder:text-slate-600 w-full max-w-[200px] focus:ring-0 p-0 tracking-tight truncate"
              />
            ) : (
              <span className="text-[11px] font-black text-white/90 truncate tracking-tight max-w-[200px]">
                {formData.purpose || 'Mission Sans Titre'}
              </span>
            )}
            <div
              className={`inline-flex items-center gap-1 px-1.5 py-0.5 rounded-full text-[8px] font-black border shrink-0 ${statusBadge.bg}`}
            >
              <motion.div
                animate={statusBadge.pulse ? { scale: [1, 1.5, 1], opacity: [1, 0.5, 1] } : {}}
                transition={{ duration: 2, repeat: Infinity }}
                className={`w-1 h-1 rounded-full ${statusBadge.dot}`}
              />
              {statusBadge.label}
            </div>
          </div>
          ...
          <motion.button
            ...
            className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-[9px] font-black tracking-widest transition-all ${saveState.cls} disabled:cursor-default shrink-0`}
          >
            {isSyncing || isSyncingServer ? (
              <Loader2 size={12} className="animate-spin" />
            ) : (
              <saveState.icon size={12} />
            )}
          ...
          <motion.button
            ...
            className="group relative flex items-center gap-1 px-2.5 py-1.5 rounded-lg bg-gradient-to-r from-indigo-500/20 to-purple-500/20 border border-indigo-400/30 text-indigo-300 hover:from-indigo-500/30 hover:to-purple-500/30 hover:border-indigo-400/50 transition-all shadow-lg shadow-indigo-500/10 shrink-0"
          >
          ...
          <motion.button
            ...
            title="Dupliquer la mission active"
            className="flex items-center gap-1 px-2 py-1.5 rounded-lg text-slate-500 hover:text-emerald-300 hover:bg-emerald-500/10 disabled:opacity-15 disabled:cursor-not-allowed transition-all shrink-0"
          >
            <Copy size={12} />
            <span className="hidden sm:inline text-[8px] font-bold uppercase tracking-widest">
              Dupliquer
            </span>
          </motion.button>
          ...
          {!isSubmitted && !isCertified && (
            <motion.button
              ...
              className="flex items-center gap-1 px-3 py-1.5 bg-gradient-to-r from-violet-600 to-indigo-600 text-white rounded-lg text-[9px] font-black tracking-widest shadow-md shadow-indigo-600/30 transition-all border-t border-white/15 shrink-0"
            >
              <ListChecks size={12} />
          ...
          {isValidator && isSubmitted && !isCertified && (
            <motion.button
              ...
              whileTap={{ scale: 0.97 }}
              onClick={onValidate}
              className="flex items-center gap-1 px-3 py-1.5 bg-gradient-to-r from-emerald-600 to-teal-500 text-white rounded-lg text-[9px] font-black tracking-widest shadow-md shadow-emerald-500/30 transition-all border-t border-white/15 shrink-0"
            >
              <Fingerprint size={12} />
          ...
          {!projectId && (
            <motion.button
              ...
              className="flex items-center gap-1 px-3 py-1.5 bg-gradient-to-r from-cyan-600 to-blue-600 text-white rounded-lg text-[9px] font-black tracking-widest shadow-md shadow-blue-500/30 transition-all border-t border-white/15 disabled:opacity-50 shrink-0"
            >
              {isAssigning ? <Loader2 size={12} className="animate-spin" /> : <Link2 size={12} />}
        ...
        <div className="flex items-center gap-1 min-w-0">
          <div className="flex items-center gap-0.5" ref={containerRef}>
            ...
            >
              <p className="px-3 py-2 text-[9px] font-black text-slate-600 uppercase tracking-widest border-b border-white/5 mb-1">
                Modèles disponibles
              </p>
            ...
            >
              ...
              <ToggleRow
                ...
                active={!!formData.features?.map}
                onToggle={() => onToggleFeature('map')}
              />
              <ToggleRow
                icon={DollarSign}
                label="Frais & Indemnités"
                color="text-emerald-400"
                active={!!formData.features?.expenses}
                onToggle={() => onToggleFeature('expenses')}
              />
              <ToggleRow
                icon={ListChecks}
                ...
                onToggle={() => onToggleFeature('inventory')}
              />
              <div className="my-1.5 border-t border-white/5" />
              <ToggleRow
                icon={Smartphone}
                label="Mode Terrain (Simplifié)"
                color="text-blue-400"
                active={!!isSimplifiedMode}
                onToggle={() => onToggleSimplifiedMode(!isSimplifiedMode)}
              />
            </Dropdown>
            ...
            <div className="flex items-center gap-0.5 pl-2 border-l border-white/[0.06] ml-1">
              ...
              >
                <RefreshCw
                  size={15}
                  className={isSyncingServer ? 'animate-spin text-indigo-400' : ''}
                />
              </button>
