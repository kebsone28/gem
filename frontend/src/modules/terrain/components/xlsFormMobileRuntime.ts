...
const normalizeRuntimeLookupKey = (value: unknown) =>
  String(value ?? '')
    .trim()
    .normalize('NFD')
...
const getRuntimeLeafKey = (value: unknown) => {
  const parts = String(value ?? '').split('/').filter(Boolean);
  return parts.length ? parts[parts.length - 1] : String(value ?? '');
};

const getAliasedRuntimeValue = (source: unknown, fieldName: string) => {
  if (!isRecord(source)) return undefined;
  if (Object.prototype.hasOwnProperty.call(source, fieldName)) return source[fieldName];

  const normalizedFieldName = normalizeRuntimeLookupKey(fieldName);
  const normalizedLeafName = normalizeRuntimeLookupKey(getRuntimeLeafKey(fieldName));

  const match = Object.entries(source).find(([key]) => {
    const normalizedKey = normalizeRuntimeLookupKey(key);
    const normalizedKeyLeaf = normalizeRuntimeLookupKey(getRuntimeLeafKey(key));
    return (
      normalizedKey === normalizedFieldName ||
      normalizedKey === normalizedLeafName ||
      normalizedKeyLeaf === normalizedFieldName ||
      normalizedKeyLeaf === normalizedLeafName
    );
  });
...
const getExpressionValue = (
  ...
  const value = getAliasedRuntimeValue(values, fieldName);
  if (value !== undefined) return value;
  const repeatValue = getAliasedRuntimeValue(context.repeatValues, fieldName);
  if (repeatValue !== undefined) return repeatValue;
  const choiceValue = getAliasedRuntimeValue(context.choice, fieldName);
...
const normalizePulldataKey = (value: unknown) => normalizeRuntimeLookupKey(value);
...
export const isXlsFormRuntimeFieldVisible = (
  ...
  const relevantExpressions = [...(field.parentRelevant || []), field.relevant].filter(Boolean);
  
  // If no relevant expressions, field is always visible
  if (relevantExpressions.length === 0) return true;
  
  // If role is not set, show all fields regardless of role-based relevant expressions
  const roleValue = values.role || values.role || '';
  if (!roleValue && relevantExpressions.some(expr => String(expr).toLowerCase().includes('role'))) {
    return true;
  }
  
  // Otherwise evaluate all relevant expressions
  return relevantExpressions.every((expression) =>
