...
import {
  Settings2, MapPin, Package, Users, ChevronDown, ChevronRight,
  ToggleLeft, ToggleRight, RefreshCcw, Loader2, Search, ArrowLeft,
  Home, HardHat, Layers,
} from 'lucide-react';
...
const INIT_L: LotDef[] = [
  { key: 'A', title: 'Lot A \u{2014} Pr\u{E9}-c\u{E2}blage',        description: 'Pr\u{E9}-c\u{E2}blage des habitations pour le raccordement',       active: true, grappesCount: 9 },
  { key: 'B', title: 'Lot B \u{2014} Installation int\u{E9}rieure',   description: 'Installation \u{E9}lectrique int\u{E9}rieure des habitations',       active: true, grappesCount: 9 },
  { key: 'C', title: 'Lot C \u{2014} Raccordement',                   description: 'Raccordement au r\u{E9}seau \u{E9}lectrique SENELEC',              active: true, grappesCount: 9 },
];

const LOT_COLORS: Record<string, { card: string; icon: string; accent: string }> = {
  A: { card: 'border-cyan-500/20 bg-cyan-500/5',    icon: 'text-cyan-400 bg-cyan-500/10',    accent: 'text-cyan-400' },
  B: { card: 'border-violet-500/20 bg-violet-500/5', icon: 'text-violet-400 bg-violet-500/10', accent: 'text-violet-400' },
  C: { card: 'border-rose-500/20 bg-rose-500/5',     icon: 'text-rose-400 bg-rose-500/10',     accent: 'text-rose-400' },
};

const LOT_ICONS: Record<string, React.ReactNode> = {
  A: <Layers size={20} />, B: <Home size={20} />, C: <HardHat size={20} />,
};
...
function Toggle({ v, on }: { v: boolean; on: () => void }) {
  return (
    <button onClick={on} className={`relative w-11 h-6 rounded-full transition-colors ${v ? 'bg-emerald-500' : 'bg-slate-600'}`}>
...
function Badge({ n, cls }: { n: number | string; cls?: string }) {
  return <span className={`inline-flex items-center justify-center min-w-[20px] h-5 px-1.5 rounded-full text-[10px] font-bold ${cls || 'bg-slate-700 text-slate-300'}`}>{n}</span>;
}
...
const ConfigGrappes: React.FC = () => {
    ...
    (async () => {
      ...
      try {
        const [gR, lR, aR] = await Promise.allSettled([
          apiClient.get('/grappes'),
          apiClient.get('/grappes/lots'),
          apiClient.get('/carto-grappes/entrepreneurs'),
        ]);
        if (!cancelled) {
          if (gR.status === 'fulfilled' && gR.value.data?.grappes?.length) setGrappes(gR.value.data.grappes);
          if (lR.status === 'fulfilled' && lR.value.data?.lots?.length) setLots(lR.value.data.lots);
          if (aR.status === 'fulfilled' && aR.value.data?.assignments?.length) setAssignments(aR.value.data.assignments);
        }
  ...
  const byRegion = useMemo(() => {
    ...
    for (const g of grappes) {
      ...
      if (!list) { list = []; m.set(g.region, list); }
      list.push(g);
    }
    return m;
  }, [grappes]);
  ...
  const filtered = useMemo(() => {
    ...
    const m = new Map<string, GrappeRow[]>();
    for (const g of grappes) {
      if (g.key.toLowerCase().includes(q) || g.region.toLowerCase().includes(q) || g.label.toLowerCase().includes(q)) {
        let list = m.get(g.region);
        if (!list) { list = []; m.set(g.region, list); }
        list.push(g);
      }
  ...
  const toggleRegion = useCallback((r: string) => {
    setExpanded(p => { const n = new Set(p); n.has(r) ? n.delete(r) : n.add(r); return n; });
  }, []);

  const toggleLot = useCallback((k: string) => {
    setLots(p => p.map(l => l.key === k ? { ...l, active: !l.active } : l));
    toast.success(`Lot ${k} ${lots.find(l => l.key === k)?.active ? 'd\u{E9}sactiv\u{E9}' : 'activ\u{E9}'}`);
  }, [lots]);
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      const [gR, lR, aR] = await Promise.allSettled([
        apiClient.get('/grappes'), apiClient.get('/grappes/lots'), apiClient.get('/carto-grappes/entrepreneurs'),
      ]);
      if (gR.status === 'fulfilled' && gR.value.data?.grappes?.length) setGrappes(gR.value.data.grappes);
      if (lR.status === 'fulfilled' && lR.value.data?.lots?.length) setLots(lR.value.data.lots);
      if (aR.status === 'fulfilled' && aR.value.data?.assignments?.length) setAssignments(aR.value.data.assignments);
      toast.success('Donn\u{E9}es actualis\u{E9}es');
    } catch { toast.error('\u{C9}chec de l\'actualisation'); }
  ...
  if (!activeProjectId) return (
    <div className="min-h-screen bg-slate-950 flex items-center justify-center">
      <div className="text-center">
        ...
        <p className="text-xs text-slate-600 mt-1">S\u{E9}lectionnez un projet pour configurer les grappes.</p>
      </div>
    </div>
  );
    ...
    <div className="min-h-screen bg-slate-950 p-4 md:p-6">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
          <StatCard icon={<MapPin size={18} />} label="Grappes" value={grappes.length} color="emerald" />
          <StatCard icon={<Home size={18} />} label="M\u{E9}nages" value={totHH} color="blue" />
          <StatCard icon={<Layers size={18} />} label="Villages" value={totV} color="violet" />
          <StatCard icon={<HardHat size={18} />} label="Lots actifs" value={`${activeLots}/${lots.length}`} color="amber" />
        </div>
          ...
          {TABS.map(t => (
            <button key={t.key} onClick={() => setTab(t.key)}
              className={`flex items-center gap-2 flex-1 justify-center px-4 py-2.5 rounded-lg text-[11px] font-black uppercase tracking-[0.08em] transition-all ${
                tab === t.key ? 'bg-indigo-500/20 text-indigo-300 border border-indigo-500/30' : 'text-slate-500 hover:text-slate-300 hover:bg-slate-800/60'
              }`}>
              {t.icon} {t.label}
        ...
        {tab !== 'lots' && (
          <div className="relative mb-4">
            ...
            <input type="text"
              placeholder={tab === 'grappes' ? 'Rechercher une grappe...' : 'Rechercher une assignation...'}
        ...
        <div className="overflow-x-auto rounded-2xl border border-white/8 bg-slate-900/60">
          {tab === 'grappes' && <SectionGrappes data={filtered} expanded={expanded} toggle={toggleRegion} />}
        ...
        <div className="mt-4 text-center text-[11px] text-slate-600">
          {grappes.length} grappes \u{00B7} {lots.length} lots \u{00B7} {assignments.length} assignations
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div className="divide-y divide-white/5">
      {Array.from(data.entries()).map(([region, rows]) => {
        ...
        return (
          <div key={region}>
            <button onClick={() => toggle(region)} className="w-full flex items-center gap-3 px-5 py-4 hover:bg-white/[0.02] transition-colors">
              {isOpen ? <ChevronDown size={16} className="text-slate-500" /> : <ChevronRight size={16} className="text-slate-500" />}
              <span className={`w-2.5 h-2.5 rounded-full ${c.dot}`} />
              <span className="text-sm font-bold text-white">{region}</span>
              <span className={`text-[10px] font-semibold px-2 py-0.5 rounded-full ${c.bg} ${c.text}`}>{rows.length} grappes</span>
              <span className="text-[10px] text-slate-500 ml-auto">{hh} m\u{E9}nages</span>
            </button>
            {isOpen && (
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-white/5">
                      <th className="px-5 py-2 text-left text-[10px] font-black uppercase tracking-[0.2em] text-slate-500">Cl\u{E9}</th>
                      <th className="px-5 py-2 text-left text-[10px] font-black uppercase tracking-[0.2em] text-slate-500">Libell\u{E9}</th>
                      <th className="px-5 py-2 text-right text-[10px] font-black uppercase tracking-[0.2em] text-slate-500">M\u{E9}nages</th>
                      <th className="px-5 py-2 text-right text-[10px] font-black uppercase tracking-[0.2em] text-slate-500">Villages</th>
                    </tr>
                  </thead>
                  <tbody>
                    {rows.map(g => (
                      <tr key={g.key} className={`border-b border-white/[0.03] hover:bg-white/[0.02] transition-colors ${c.bg}`}>
                        <td className="px-5 py-3"><span className={`text-xs font-mono font-bold ${c.text}`}>{g.key}</span></td>
                        <td className="px-5 py-3 text-xs text-slate-300 font-medium">{g.label}</td>
                        <td className="px-5 py-3 text-right"><span className="text-xs font-bold text-white">{g.households}</span></td>
                        <td className="px-5 py-3 text-right"><span className="text-xs text-slate-400">{g.villages}</span></td>
                      </tr>
                    ))}
                  </tbody>
                </table>
      ...
      {data.size === 0 && <div className="p-8 text-center text-sm text-slate-500">Aucune grappe trouv\u{E9}e</div>}
    </div>
  );
}
        ...
        return (
          <div key={l.key} className={`rounded-xl border p-5 transition-all ${lc.card}`}>
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <div className="flex items-center gap-3 mt-2">
                    <span className="text-[10px] font-semibold text-slate-500">{l.grappesCount} grappes</span>
                    <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${l.active ? 'bg-emerald-500/20 text-emerald-300' : 'bg-slate-700/50 text-slate-500'}`}>
              ...
              <div className="flex items-center gap-3">
                <span className="text-[10px] text-slate-500 font-semibold">{l.active ? 'Activ\u{E9}' : 'D\u{E9}sactiv\u{E9}'}</span>
                <button onClick={() => onToggle(l.key)} className="transition-transform hover:scale-110">
                  {l.active ? <ToggleRight size={32} className="text-emerald-400" /> : <ToggleLeft size={32} className="text-slate-600" />}
                </button>
              </div>
            </div>
          </div>
...
function SectionAssignments({ rows, search }: { rows: AssignationRow[]; search: string }) {
  const filtered = useMemo(() => {
    ...
    return rows.filter(a =>
      a.grappeKey.toLowerCase().includes(q) || a.region.toLowerCase().includes(q) ||
      a.lotKey.toLowerCase().includes(q) || a.prestataire.toLowerCase().includes(q) || a.mode.toLowerCase().includes(q)
    );
  }, [rows, search]);

  if (filtered.length === 0) return (
    <div className="p-8 text-center text-sm text-slate-500">
      {rows.length === 0 ? 'Aucune assignation configur\u{E9}e' : 'Aucune assignation ne correspond \u{E0} la recherche'}
    </div>
  );
    ...
    <table className="w-full text-sm">
      <thead>
        <tr className="border-b border-white/8">
          {['R\u{E9}gion', 'Grappe', 'Lot', 'Mode', 'Prestataire'].map(h => (
            <th key={h} className="px-5 py-3 text-left text-[10px] font-black uppercase tracking-[0.2em] text-slate-500">{h}</th>
          ))}
        </tr>
      </thead>
      <tbody>
        {filtered.map((a, i) => {
          ...
          return (
            <tr key={`${a.grappeKey}-${a.lotKey}-${i}`} className="border-b border-white/[0.03] hover:bg-white/[0.02] transition-colors">
              ...
              <td className="px-5 py-3"><span className={`text-xs font-mono font-bold ${rc.text}`}>{a.grappeKey}</span></td>
              <td className="px-5 py-3"><span className={`text-xs font-bold ${lc.accent}`}>Lot {a.lotKey}</span></td>
              <td className="px-5 py-3">
                <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-bold ${MODE_CLS[a.mode] || 'bg-slate-700 text-slate-400'}`}>{a.mode}</span>
              </td>
              <td className="px-5 py-3">
                <span className="text-xs text-slate-300 font-medium">
                  {a.prestataire || <span className="text-slate-600 italic">Non assign\u{E9}</span>}
                </span>
              </td>
            </tr>
          );
